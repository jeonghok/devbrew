...
# (최종 리뷰 K4a). 실측(PR 3 Task 6, 이 락이 매 실행마다 `OWNFILE`·`SHARED` 줄로
# 다시 낸다): 소비자 여섯 중 «자기 파일만으로» 여덟 키를 다 읽는 것은 문서 리뷰
# 엔진 `docreview_route.py` **하나**다(8/8) — 두 호스트에 심볼릭 링크로 배포돼
# `OWNFILE` 줄로는 둘이지만 물리 파일은 하나다. 나머지 넷은 0/8·0/8·1/8·1/8 이고,
# 그 넷 «전부»가 `shared/adjudication/render_disposition.py` 를 import 하며 그 한
# 파일이 혼자 8/8 을 덮는다. 즉 이 락의 통과는 여섯 개의 독립 증거가 아니라
# **두 파일(엔진 · 공유 렌더 모듈)에 대한 단언 + 렌더 모듈을 import 한다는 사실의
# 복제 넷**이다. 이 락이 지키는 것은 「엔진과 공유 렌더 모듈이 여덟 키를 계속
# 부른다 + 나머지 소비자가 렌더 모듈을 계속 import 한다」이지 「소비자 여섯이 각자
# 여덟 키를 읽는다」가 아니다.
set -u
