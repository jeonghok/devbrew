...
# `shared/tests/test_no_new_duplication.sh:46` 과 **같은 도출**이어야 한다.
...
#
# 같은 파일의 다른 `ls-files` 호출은 **이 문제를 갖지 않는다**(2026-08-21 전수 측정):
#  · `:275` 는 `-s` 의 인덱스 모드 비트(`120000`)가 판정 근거라 미추적 항목엔 그 정보가
#    아예 없다(`-s --others` 를 붙여도 120000 행 8건 그대로). 그리고 이미 인덱스로
#    후보를 고르고 워킹트리에서 해석한다.
#  · `:392` 는 이미 `git ls-files` 와 워킹트리 글롭의 **합집합**이다 — 미추적을 덮는다.
#  · `:728`·`:901`·`:962`·`:965`·`:1015` 는 "추적된다 = 설치본에 실린다" 를 **단언의
#    대상으로** 삼는다(F4/H1: 사본을 untrack 하면 디스크엔 남아도 설치본에는 안 실린다).
#    여기에 `--others` 를 붙이면 그 단언이 재는 것이 사라진다.
#  · `:330`·`:530`·`:680`·`:683`·`:749`·`:853` 은 코퍼스가 아니라 **기대를 만드는**
#    도출원이다. 미추적 임시 파일이 배포 기대를 만들면 안 된다 — fixtures/mocks/harness
#    를 빼는 것과 같은 이유다.
# 열 지점 전부 clean 트리 카운트가 `--others` 유무에 무관하게 같다는 것도 함께 쟀다.
