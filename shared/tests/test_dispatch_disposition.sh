...
# `0건` 이 답이 되어 누락이 드러난다.
#
# 다만 ∀ 가 드러내는 것은 «어디서도 dispatch 되지 않는» agent 뿐이다. 이미 다른
# 표기로 불리는 agent 가 `NOTATION` 밖의 표기로 «추가» 호출되면 그 자리는
# dispatch 로 세어지지 않아 처분 앵커 검사를 조용히 빠져나간다.
#
# 그리고 «갈래마다» 이빨이 코퍼스 내용에 달려 있다 — 실례가 0 인 갈래는 좁혀도
# 이 스위트가 잡지 못한다. `NOTATION` 을 건드리는 PR 은 그 표기의 실례를 코퍼스에서
# 먼저 세라. 0 이면 그 편집은 이 스위트 밖에서만 검증된다.
#
