...
#          포함하며, 기본값 전개는 네 연산자(`:-` · `:=` · `-` · `=`)를 본다. 값이 cwd 에서 오는지는
#          **세 표기**로만 판정한다 — `$(pwd` · `$PWD` · 앞머리 `plugins/`. 가드를 지난 펜스에서는 한
#          걸음 더 엄격하다 — 우변이 치환 토큰 «그 자체»여야 한다. 그 엄격함의 대가는 거짓 RED 다:
#          `V="${CLAUDE_PLUGIN_ROOT}/하위경로"` 와, 값을 세우지 않는 전개(`${V:+…}`)도 함께 걸린다.
...
#          멀게 한다. 가드 줄 수와 가드를 지나는 펜스 수는 **오늘의 수로 핀한 하한**과 견준다(회계).
...
#
# **이 락은 셸을 파싱하지 않는다.** 규칙은 전부 한 «물리 줄» 위의 정규식이고, 구성을 그 줄 밖으로
# 옮기면 재지 않는다. 아래는 그 한 문장의 사례들이다 — 표기를 더 열거하는 것이 아니라 층위를 바꾸는
# 것(따옴표를 아는 토큰화 + 논리 줄 합치기)이 해법이고, 그 교체는 후속이다.
#
# 재지 못하는 것:
#  - 가드의 «표기». 정본 한 줄 · 큰따옴표 형태만 알아본다. 두 줄로 쪼개거나 홑따옴표 · 따옴표 없음 ·
#    `readonly` 접두로 쓰면 그 자리는 C3 · C4 · C5 · C6 · 축 2b · 가드 회계에서 통째로 빠진다.
...
#  - 이름의 전역성. 축 2b 가 쓰는 루트 변수 이름은 코퍼스 «전역»으로 예약된다 — 다른 뜻으로 같은
#    이름을 쓰는 미래 문서에 거짓 RED 가 난다.
...
# 가드 회계 — 가드 줄 수와 그 가드를 지나는 펜스 수. 판정은 아래 셸 쪽에서 «오늘의 수로 핀한 하한»과
# 견준다. 한때 `n_guard >= n_guard_fence` 라는 관계식을 썼는데, 펜스마다 앞이 k · 뒤가 min(k,1) 만큼
# 늘어나므로 그 부등식은 어떤 입력에서도 참이다 — 아무것도 재지 못했다.
...
# 그래서 두 수를 **오늘의 수로 핀한 하한**과 견준다. 관계식(`n_guard >= n_guard_fence`)은 구성상 항상
# 참이라 아무것도 재지 못했고, 실효는 하한 하나뿐이었다 — 그 하한이 실제보다 낮으면 차이만큼 조용히
# 사라진다(하한 20 · 실제 45 일 때 25개까지 통과, 실측).
# **가드를 정직하게 지우면 이 두 숫자도 같은 커밋에서 내린다** — 내리는 행위가 그 삭제의 공시다.
N_GUARD_MIN=45
N_GUARD_FENCE_MIN=45
n_guard="$(awk -F'\t' '$1=="N_GUARD" {print $2}' "$TMP/report.tsv")"
n_guard_fence="$(awk -F'\t' '$1=="N_GUARD" {print $3}' "$TMP/report.tsv")"
[ "${n_guard:-0}" -ge "$N_GUARD_MIN" ] && [ "${n_guard_fence:-0}" -ge "$N_GUARD_FENCE_MIN" ] \
  && ok "가드 ${n_guard}줄 / 가드를 지나는 펜스 ${n_guard_fence}곳 — 핀한 하한 ${N_GUARD_MIN} · ${N_GUARD_FENCE_MIN} 이상" \
  || no "가드 회계가 무너졌다: 가드 ${n_guard:-0}줄(하한 $N_GUARD_MIN) · 펜스 ${n_guard_fence:-0}곳(하한 $N_GUARD_FENCE_MIN) — 가드가 정규식 밖 형태로 다시 쓰였나(그 자리는 C3 · C4 · C5 · 축 2b 에서 빠진다). 정직한 삭제라면 두 하한도 같은 커밋에서 내려라."
