...
#          가드를 지난 펜스에서는 한 걸음 더 엄격하다 — 그 줄 **첫 대입**의 우변이 치환 토큰 «그 자체»
#          여야 한다. 그 엄격함의 대가는 거짓 RED 셋이다: `V="${CLAUDE_PLUGIN_ROOT}/하위경로"` · 값을
...
#          (오늘의 수로 핀한 하한). 계수 모집단은 추적되는 파일뿐이다.
...
#
# 한 줄만 보는 축과 그렇지 않은 축이 갈린다. 축 2b · C3 · C4 · C5 · C6 은 한 물리 줄만 본다.
# **축 1b 는 이미 `\` 이어쓰기를 논리 단위로 합치고 산문 코드 스팬을 개행 너머로 훑는다.** 축 3 은 한
# 줄을 헤딩 절 범위 안에서 본다. 그리고 한 줄 «안»에서도 축 2b 는 변수마다 맨 왼쪽 대입 하나만 본다.
#
...
#    «재작성»은 가드 회계가 잡는다(세어지는 수가 준다) — 회계가 잡지 못하는 것은 아래 「회계의 이빨」.
...
#    `PA` 이고, `PR` 은 pull request 번호로 쓰일 개연성이 높아 장래 거짓 RED 의 1순위다.
...
#    논리 줄의 나머지를 지운다(`--label "a # b" plugins/<자기>/x` 는 조용하고, 그 따옴표만 빼면 잡힌다).
...
#  - 회계의 «이빨». 핀한 하한은 보상되지 «않은» 제거·재작성만 잡는다. 못 잡는 것 셋: ① 한 자리를
#    비정본으로 재작성하면서 다른 자리에 정본 가드를 더하면 합계가 복구된다 ② 인식되지 않는 가드를 단
#    새 펜스의 «추가»에는 최소값이 결코 발화하지 않는다 ③ 가드를 지우며 같은 편집에서 핀 숫자를 내리면
#    통과한다 — 핀은 시험 대상 파일 «안»의 두 자리 숫자이고 독립된 증인이 없다. 계수에서 추적 전 파일을
#    빼는 것은 ①의 절반(로컬 임시 초안)만 닫는다 — 그 파일을 커밋하면 합계는 그대로 되사진다.
...
# 추적 전인 새 파일도 «스캔» 대상이다(`--others --exclude-standard`). 다만 가드 «계수»에서는 뺀다 —
# 추적되지 않은 로컬 초안이 핀한 하한을 되사 주면 안 된다(커밋하면 다시 세어진다).
MODE="${1:-}"
set -- 'plugins/*/skills/*.md' 'plugins/*/commands/*.md' 'plugins/*/references/*.md'
CORPUS="$(git ls-files --cached --others --exclude-standard -- "$@" | LC_ALL=C sort -u)"
TRACKED="$(git ls-files --cached -- "$@" | LC_ALL=C sort -u)"
if [ "$MODE" = "--emit-scanned" ]; then
  printf '%s\n' "$CORPUS"
  exit 0
fi
...
python3 - "$TMP" "$TMP/corpus.txt" "$TMP/tracked.txt" > "$TMP/report.tsv" <<'PY'
import os
import re
import sys

out_dir, corpus_file, tracked_file = sys.argv[1], sys.argv[2], sys.argv[3]
files = [l for l in open(corpus_file, encoding="utf-8").read().split("\n") if l]
# 가드 «계수»의 모집단. 스캔은 추적 전 파일까지 하되 세지는 않는다 — 추적되지 않은 로컬 초안이
# 핀한 하한을 되사 주면 안 된다.
tracked = {l for l in open(tracked_file, encoding="utf-8").read().split("\n") if l}
...
# 가드 회계 — 가드 줄 수와 그 가드를 지나는 펜스 수(추적되는 파일만). 판정은 아래 셸 쪽에서 «펜스 수
# 하나»를 오늘의 수로 핀한 하한과 견준다. 한때 `n_guard >= n_guard_fence` 관계식을 썼는데 펜스마다
# 앞이 k · 뒤가 min(k,1) 만큼 늘어나 어떤 입력에서도 참이었고, 그 뒤 두 하한을 같은 값으로 핀했더니
# 앞의 조건이 뒤에 지배당해 역시 발화할 수 없었다 — 그래서 단언은 하나다.
...
# **가드를 정직하게 지우면 이 숫자도 같은 커밋에서 내린다** — 내리는 행위가 그 삭제의 공시다. 다만 그
# 공시에는 독립된 증인이 없다(핀은 이 파일 «안»의 두 자리 숫자다) — 머리말 「회계의 이빨」 참조.
...
[ "${n_guard_fence:-0}" -ge "$N_GUARD_FENCE_MIN" ] \
  ...
  || no "가드 회계가 무너졌다: 가드를 지나는 펜스 ${n_guard_fence:-0}곳(하한 $N_GUARD_FENCE_MIN) · 가드 ${n_guard:-0}줄 — 가드가 정규식 밖 형태로 다시 쓰였나(그 자리는 C3 · C4 · C5 · 축 2b 에서 빠진다). 정직한 삭제라면 하한도 같은 커밋에서 내려라."
