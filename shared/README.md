...
`shared/tests/test_copy_of_contract.sh` — 세 계약을 검사한다: **심볼릭 링크**는 구조에서 도출된 모든 배포 지점이 링크여야 하고 존재하는 대상을 가리켜야 하고 그 대상이 기대한 정본과 일치해야 한다(내용이 갈라질 수 없으므로 바이트 비교가 필요 없다). **`copy-of:` 잔여**는 그 줄이 가리키는 파일과 그 줄만 제외하고 바이트가 같아야 한다. **import 형제 사본**은 `import` 로만 소비되는 정본이 소비자를 가진 **모든** 배포 디렉토리에 형제 사본을 가져야 한다(∃ 가 아니라 ∀ — 있는 사본이 같은지만 보는 앞의 두 계약은 빠진 자리를 못 잡는다).
