...
공유 렌더 모듈이 설계이므로). 그래서 `missing()==[]` 이 소비자 수만큼 나와도
**그만큼의 독립 사실이 아니다.** 실측(PR 3 Task 6, 호출자가 매 실행마다
`OWNFILE`/`SHARED` 로 다시 낸다): 자기 파일만으로 여덟 키를 다 읽는 소비자는
문서 리뷰 엔진 `docreview_route.py` 하나(8/8 — 두 호스트 배포 링크라 줄은 둘)이고
나머지 넷은 0/8·0/8·1/8·1/8 이며, 그 넷 전부가 `render_disposition.py` 를 import
하고 그 한 파일이 혼자 8/8 을 덮는다. 이 판정기가 지키는 것은 **「엔진과 그 공유
모듈이 여덟 키를 계속 부른다 + 나머지 소비자가 그 모듈을 계속 import 한다」**이지
소비자 각자의 독립 소비가 아니다.
"""
import ast
import io
