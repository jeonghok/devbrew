...
- [2026-08-23-dispatch-lock-mutations](2026-08-23-dispatch-lock-mutations.md) — 2026-08-23 · dispatch 처분 락(7축 A①②③④⑤·B·C) mutation M1~M18(+M8b) 전건 실행 기록. M8a/M8b 는 macOS bash 3.2 가 멀티바이트 문자 뒤 `set -u` 아래서 스크립트를 크래시시키는 셋째 결과(두 가설 모두 아님)를 냈다 — **락을 지금 고칠 사유는 아니다**: 정정된 것은 계획서의 footgun 서술이고, 전수 스윕 결과 이 패턴을 실행 코드로 갖는 인스턴스는 리포에 없다
- [2026-08-27-adjudication-topology-handoff](2026-08-27-adjudication-topology-handoff.md) — 2026-08-27 · **판정 지형 핸드오프**(읽기전용 발견 + 사용자 결정). 서브에이전트 18개 중 **12개가 판정 회계 없이 오케스트레이터·사람에게 직행**하고, 재비판이 붙은 5개 중 **2개는 codex 단독 의존이라 현재 사망**(`merge_review.py:508`). 정해진 방향 4건 — 받는 곳을 오케스트레이터로 통일 · 오케스트레이터가 **항상** 1차 재비판 · 사람은 자명하지 않은 결정에만 · 재비판 서브에이전트 가능하면 제거. brainstorming 급여용. 형제 [cross-skill-seam-handoff](2026-08-27-cross-skill-seam-handoff.md) 와 분리한 근거는 §5, 두 문서가 만나는 지점은 §4 T5
