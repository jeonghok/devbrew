...
### §7-5 공유 계약의 **의무**는 채택자에게 기계적으로 강제되지 않는다

Task 33 fix round 1 이 `proceed-gate.md` 에 새 의무를 넣었다 — *"각 skill 은 자기 degrade
채널을 자기 어휘 절에 이름으로 대야 한다"*. 두 채택자(`reviewing-spec` ·
`conducting-interview`)는 실제로 댄다. 그러나 **그 의무를 재는 검사는 없다** — 계약 산문뿐이다.

`test_proceed_gate_adopters.sh` 는 채택자를 포인터에서 도출해 **두 가드의 앵커**(정지 어휘 ·
polite stop)를 재지만, degrade 채널은 재지 않는다. 이유는 형태가 없기 때문이다: 채널의 이름은
skill 마다 다르고(`brief_review_degradations` 원장 vs `merge_review` 플래그 + `advisory:`),
"이름을 댔다"를 무엇으로 판정할지는 **세 번째 채택자가 나와야** 공통 모양이 보인다. 두 개로
일반화하면 두 개에 과적합한 정규식이 된다.

- **발동 조건**: 세 번째 skill 이 `references/proceed-gate.md` 를 채택할 때. 그 시점에
  `test_proceed_gate_adopters.sh` 의 채택자 루프에 한 단언을 더하면 된다(도출은 이미 있다).
- **그때까지의 상태**: 산문 의무. 채택자 두 곳이 지키고 있음은 사람이 읽어 확인했다.
- **오늘 만들지 않는 이유**: 대상이 둘뿐이라 지금 만든 검사는 측정이 아니라 추측이다.
