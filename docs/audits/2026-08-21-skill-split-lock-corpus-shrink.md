...
### §7-8 `test_copy_of_contract.sh` 축 1a 의 도출 앵커가 **산문 리터럴**이다

이 문서가 다루는 실패 클래스의 새 인스턴스다 — **리팩터가 락의 코퍼스를 줄이는데 락은
GREEN 을 유지한다.** 다만 여기서 줄어드는 것은 스캔되는 *파일* 이 아니라 **감사 대상으로
도출되는 배포 지점** 이다.

축 1a 는 어느 플러그인이 `prompt-preamble.md` 배포 지점을 가져야 하는지를, 그 플러그인
스크립트의 **산문에 리터럴 `scripts/prompt-preamble.md` 가 있는가** 로 도출한다.

〔2026-08-22 Task 35 실측〕 P21 로더를 `shared/codex/codex_prompt_common.py` 로 통합하면서
빌더 네 개에서 그 리터럴을 **정당하게** 걷어냈다. 그러자 spec-distill 의 도출이 3 → 2 로
줄며 축 1a 가 RED. 즉 **정당한 리팩터 한 번이 감사 대상 하나를 떨어뜨린다.** 이번엔 RED 가
나서 잡혔지만, 그것은 다른 축이 같은 지점을 세고 있었기 때문이지 도출 자체가 안전해서가
아니다. 세는 축이 없는 지점이라면 조용히 감사 밖으로 나간다.

Task 35 는 정본 주석에 리터럴 + 〔앵커 주의〕 경고를 남겨 복구했다
(`shared/codex/codex_prompt_common.py:33-42`). **이것은 완화이지 해소가 아니다** — 다음
사람이 그 주석을 다듬으면 같은 일이 반복되고, 경고는 주석을 지우는 사람만 읽는다.

- **오늘 상태**: GREEN. 도출 3건 유지.
- **발동 조건**: 누구든 `codex_prompt_common.py` 의 그 주석에서 리터럴 표기를 바꾸거나,
  같은 방식으로 도출되는 다른 배포 지점의 산문을 정리할 때.
- **선택지(그때 결정할 것)**: ⑴ 축 1a 가 **구조에서** 배포 지점을 도출 — 형제 사본/링크의
  실재나 `from <모듈> import` 의 실재로(가장 강하다: 대상이 피검자 산문의 통제를 벗어난다)
  ⑵ 리터럴을 코드 상수로 올려 주석이 아닌 실행 경로가 그것을 붙들게 하기 ⑶ 그대로 두고
  경고에 기대기(현 상태 — 자제에 의존).
  관련 원칙: 앵커가 피검자 통제 문자열이면 ∃-존재검사가 아니라 **∀-지배관계**로 가야 한다.
...
한다** — CPython(3.9.6 실측)이 스크립트의 `sys.path[0]` 을 **realpath 로** 잡아, 링크된
빌더가 형제를 *정본 옆* 에서 찾아내기 때문이다.

즉 이 테스트는 통과하지만 **설치본의 모양을 재고 있지 않다.** 설치본에서는 형제가 실제
물리 사본으로 옆에 있어 import 가 되고, 테스트에서는 realpath 우회로 import 가 된다 —
같은 GREEN, 다른 이유. 배포 결함은 아니다(양쪽 다 실제로 import 가 된다).

- **오늘 상태**: GREEN, 배포 결함 없음. 테스트 충실도 갭.
- **발동 조건**: FAKE_ROOT 가 링크가 아니라 **사본** 으로 빌더를 깔도록 바뀌거나, 형제
  해석이 `sys.path[0]` 이 아닌 다른 경로로 옮겨갈 때. 그때 이 테스트는 설치본이 깨져도
  통과하거나, 멀쩡한데 실패한다.
...
`shared/tests/test_no_new_duplication.sh` 는 **20줄 연속** 동일 블록을 본다. 사본 중간에
무관한 한 줄(주석이면 충분하다)이 끼면 연속이 끊겨 스캐너 밖으로 나간다.

〔2026-08-22 실측 — 가정이 아니라 실제로 일어났다〕 P21 로더 사본은 3벌이 아니라 **4벌**
이었다. `plugins/spec-distill/scripts/build_brief_codex_prompt.py` 의 로더 구간은 다른
셋과 **바이트 동일**인데(통합 전 원본 diff 로 확인) 중간에 무관한 주석 한 줄이 있어
무릎 측정의 3쌍에 잡히지 않았다. Task 35 구현자가 통합 중 눈으로 발견했다.

- **오늘 상태**: 그 4벌은 통합됐다. 락은 GREEN, 위반 0.
- **성격**: 이것은 결함이 아니라 **선택된 임계의 대가**다. 창을 낮추면 오탐이 들어오고,
  유사도 기반으로 바꾸면 결정론과 "무시해도 되는 경고 0" 을 잃는다(설계 §12.4 가 유사도
  퍼센트를 명시적으로 배제한 이유).
- **읽는 법**: 이 락의 GREEN 은 *"20줄 연속 미설명 동일 블록이 없다"* 이지 *"사본이 없다"*
  가 아니다. **사본 탐색을 이 락에 위임하지 마라** — 통합 여부는 사람이 읽어서 판단한다.
- **반대 방향도 같은 임계에 있다 — 정당한 중복이 창 바로 밑에 앉아 있다**
  〔2026-08-22 실측, PR6 whole-branch 리뷰 M1〕. 위 서술은 *"한 줄 끼면 통과한다"* 는
  쪽만 기록한다. 같은 임계의 다른 쪽에서, **이 통합이 만든 대체 블록이 18줄**이다 —
  창 20 의 두 줄 밑. 재측정(같은 코퍼스 445파일, min-chars 200 고정):
  w=21 → 0쌍 · **w=20 → 0쌍** · w=19 → **0쌍** · **w=18 → 5쌍**. 그 5쌍 중 3쌍이
  `build_artifact_codex_prompt.py` · `build_codex_prompt.py` ·
  `build_spec_codex_prompt.py` 세 빌더가 공유하는 **P21 import 서문**이다. 즉 오늘의
  GREEN 은 여유가 아니라 **2줄**이고, 그 서문의 주석을 두 줄 늘리는 편집만으로 락이
  RED 가 된다.
  - **그 RED 에는 해소 어휘가 없다.** 면제 술어는 `copy-of` 마커와 심볼릭 링크 둘뿐인데
    18줄은 파일 **전체**가 아니라 빌더 본문 안의 한 구간이다. 마커는 *"이 파일 전체가
    저 파일의 사본"* 을 뜻하고(`test_copy_of_contract.sh` 가 마커 줄 하나만 빼고 바이트
    동일을 요구한다) 빌더는 서로 다른 프로그램이라 그 단언이 거짓이 된다. 링크는
    더더욱 붙일 수 없다. 반대로 축 1c 의 ∀ 계약은 배포 지점마다 **형제 사본이 있을 것을
    요구**한다 — 이 중복은 제거 대상이 아니라 계약이 만든 것이다.
  - **읽는 법**: 다음에 이 락이 세 빌더에서 RED 를 내면 새 중복이 아니라 **이 임계와
    축 1c 계약이 만나는 지점**일 가능성이 높다. 코드를 흔들기 전에 위 곡선을 다시 재라.
  - 아래 선택지 ⑴~⑶ 은 전부 *"창을 낮추거나 정규화를 더한다"* 는 방향이라 이쪽 문제를
    악화시킨다(창을 낮추면 이 정당한 3쌍이 먼저 걸린다). 이 방향의 선택지는 따로다 —
    ⑷ 면제 술어에 **구간 단위 마커**(블록 시작/끝)를 더하기. 표기 비용과 오탐 증가를
    먼저 재야 한다.
- **선택지(그때 결정할 것)**: ⑴ 그대로 두고 이 한계를 헤더에 적기 ⑵ 공백·주석을 제거한
  정규화 위에서 한 번 더 스캔(오탐이 늘어나는지 재측정 필요) ⑶ 창을 낮추기(Task 34 곡선상
  15줄에서 오탐이 들어온다 — 비추천).
