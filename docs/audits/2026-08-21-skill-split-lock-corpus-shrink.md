...
### §7-11 `/plugin-audit` 의 자체 테스트 수집이 **접두로 도출**해 RED 를 못 본다
...
〔2026-08-22 실측〕 `plugins/agent-transparency/tests/` 의 **유일한** `.sh` 는 `ab_gate.sh`
다. 그 접두에 안 걸리므로 감사는 이 플러그인의 셸 테스트를 **0건**으로 보고 `why: null`
(=지적 없음)로 낸다. **`ab_gate.sh` 는 RED 다.** 즉 감사가 *"깨끗하다"* 고 말하는 근거가
*"그 파일을 안 봤다"* 이다.
