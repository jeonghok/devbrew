...
라운드에 `decide` 가 1건 이상이거나 **`blocks` 가 비어 있지 않은 `ask`** 가 1건 이상이면, 그 항목들
(결정 단위로 묶은 `decide` + 차단 `ask`, 렌더 순서)을 **`AskUserQuestion` 최대 4개씩 연속 호출**로
나눠 띄운다 — 도구가 한 호출에 실을 수 있는 질문이 4개뿐이고, 한 결정을 다른 결정의 질문에 묶으면
그 결정의 선택지가 사라지기 때문이다. 매 호출 첫 질문의 첫 줄은 degrade 공시(codex 부재 포함,
D7)다. 본문은
...
「추가 라운드 1회 열기」(D19, 개별 승인)가 된다. **상한 도달이면 열린 것이 0 이어도 이 두 단계
규칙이 앞에 선다** — 1단계가 「추가 라운드 1회 열기」를 싣고, 고르지 않을 때만 2단계(위 4옵션)로
넘어간다(AC7 과 4옵션 Non-goal 이 함께 성립하는 자리). 두 가드(AP2 polite stop 금지 · AC19
cross-compact 조기 진행 금지)는 정본을 그대로 따른다.
...
- AC7 — 최초 리뷰(라운드 1)에서 `rereview_count` 는 0 이고 수정 후 리뷰마다 +1 이다. 2 에 도달하면
  ...
  돌지 않는다(자동 연장 경로 0).
- AC8 — codex 부재 라운드의 라운드 게이트·승인 게이트 **매 호출 첫 질문** 텍스트 **첫 줄**이 codex
  부재 공시다(질문이 `AskUserQuestion` 여러 호출로 나뉘어도 호출마다 성립).
