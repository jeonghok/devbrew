...
- **AC8**: `reviewing-spec/SKILL.md` Phase 5 절이 단일 `AskUserQuestion` proceed 게이트를 명시한다. 옵션 의미가 다음 4종으로 문서화돼 있다 (label wording은 plan): ① /compact 후 writing-plans(권장), ② 바로 writing-plans, ③ 수정 필요(revise/interview/edit 후속 분기), ④ 멈춤(state 보존). approve 경로(①/②) 확정 후 *추가 AskUserQuestion 없음*이 명문화돼 있다. **이 '추가 AskUserQuestion 없음' 금지는 reviewing-spec Phase 5 그 턴 scope에 한정한다** — 사용자가 원래 거부한 것은 "approve 확인 후 또 묻는" 2차 게이트(R1)이며, 다음 턴에 진입한 `writing-spec`/`writing-plans` 등 *별개 skill*이 자체 게이트(예: writing-plans의 실행-방식 선택)를 띄우는 것은 그 skill의 scope이므로 AC8 위반이 아니다 (AC19의 cross-compact 경계와 정합).
...
- **AC19** (cross-compact 조기 진행 금지 — polite-stop의 *반대* 실패 모드): `reviewing-spec/SKILL.md`가 옵션 ① 선택 시 (a) `approve_handoff.sh` 실행 + verbatim `/compact` 노출 *후 그 턴을 종료*하고, (b) `writing-plans`를 같은 턴에서 호출하지 *않으며*, (c) `writing-plans` 진입은 사용자가 `/compact`를 실제 실행한 *다음 턴*에 **사용자 트리거로** 이뤄진다 — 고 명시한다. **(c)의 자동-진입 모호성 해소**: 모델은 다음 턴에 writing-plans로 *자동 진입하지 않는다*. /compact 노출과 함께 "compact 후 writing-plans 진입 준비됨"을 안내하고 사용자 신호를 기다린다. 다음 턴 진입은 사용자 트리거(예: `/compact` 뒤에 붙인 진행 인자 `/compact write plan`, 또는 명시적 진행 요청)에 의해서만 일어나며, 사용자가 redirect/취소하면 미진입(NG4·P17 — /compact 실행도 다음 단계 진입도 모두 사용자 주권). compact 전 writing-plans 진입(cross-compact 조기 진행)이 옵션 ①을 무력화하는 금지 동작으로 문구화돼 있다. polite stop("진행해야 할 때 멈춤")과 이것("멈춰야 할 때 진행")은 모두 게이트의 P17 사용자-주권을 우회하는 대칭 실패다. **prose-enforcement 근거**: AC11과 동일하게 LD5 rationale에서 확립된 패턴(메커니즘 강제 불가 영역은 SKILL.md 산문 + 리뷰 레이어로 강제) — AC19도 동일 적용하며 그 mechanical 검증 한계를 AC11과 같은 수준에서 인정한다. **verifiable (두-레이어, AC11 선례)**: (i) `grep -cE "턴 종료|다음 턴" reviewing-spec/SKILL.md` ≥ 1, **AND** (ii) 옵션 ① 서술 *블록 안에서* 'turn-ending(턴 종료)'이 'writing-plans 같은 턴 호출 금지' + '다음 턴 진입은 사용자 트리거'와 **함께** 명시됐음을 리뷰에서 확인 — grep 단독은 두 문구가 같은 블록에 공존함을 보장 못 하므로(false-positive: '턴 종료' 문구와 바로 아래 '같은 턴에 호출' 문구 공존 시에도 통과) 공존·정합 판정은 리뷰 레이어가 담당. 옵션 ②는 이 정지 요건의 *명시적 예외*(compact 없이 즉시 writing-plans)임이 대비로 명시된다. (근거: 본 2026-05-29 design 세션에서 실측된 실패 — Implicit context 및 Context/Why 참조.)
