...
- **V11** (AC17, self-reference): TTL guard mechanism verification. Two acceptable execution modes:
  - **Mode A (preferred)**: copy `plugins/spec-distill/tests/fixtures/spec-valid.md` to `docs/superpowers/specs/2026-05-16-v11-test-spec.md`, edit one line + save twice within 10s → confirm first fire dispatches reviewer, second fire is suppressed by `last_dispatched_at` TTL guard. After verification, delete the test file.
  - **Mode B (advanced)**: against this spec file itself, set `DEVBREW_SPEC_DISTILL_DESIGN_MODE_DISABLE=1` (kill switch) to bypass the design-mode hook, then run `DEVBREW_SPEC_DISTILL_REDISPATCH_TTL_SEC=10` while editing the spec body. Observe TTL guard behavior. Restore env after.
  - Note: literal ambiguity/placeholder keywords in this spec are *intentionally documented in the blacklist+escape syntax sections* (lines 131, 179, 195), so a raw second save without the kill switch would block on ambiguity hits — that's expected and confirms the validator works.
