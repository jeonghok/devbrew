...
표의 줄 번호는 **설계 시점**(base `84222ee1`)의 자리다 — 구현이 그 파일을 편집하면 밀린다. 현재 자리가 필요하면 plan 의 확정 표를 본다.
...
- ~~**`agent:` 갈래 유지 + 「실례 0」 공시**(처음 추천).~~ **기각이 뒤집혔다 — 이 대안이 최종안이다.**
  `/qg review` 가 지우는 대가를 실측으로 보였고 사용자가 게이트에서 복원을 택했다(T3 철회 행 참고).
  원래 기각 사유였던 「없던 세계의 락에는 이 갈래가 없다(D3 · D4)」는 그대로 참이지만, 대가가 기준을 이겼다.
