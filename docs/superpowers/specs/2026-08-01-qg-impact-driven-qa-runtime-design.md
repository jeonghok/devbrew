...
- **AC68′** (종료 코드 축 분리 — iter-8 리뷰) — `check_qa_ledger.py` 의 종료 코드가 **인자 모양(2)** 과 **내용(4)** 을 구분한다. 형제 스크립트의 규약과 같다(AC60: *"생략 시 exit 2, 빈 값은 exit 4"*). 앞 버전은 둘 다 2 라 *"부르는 법을 틀렸다"* 와 *"읽었는데 믿을 수 없다"* 가 같은 신호였다. 소비자(R8)는 어느 쪽이든 non-zero → PASS 불가로 라우팅하므로 **동작이 아니라 진단**을 고치는 것이다. 함께: 원장 파일 read 경로에 `UnicodeDecodeError` 핸들러가 없어 비-UTF-8 원장이 트레이스백이었다 — 형제 두 경로에서 이미 고친 **같은 버그의 세 번째 인스턴스**. `/qg` iter-8 코드 리뷰가 파서 축 넷을 더 닫았다: 같은 플래그 **반복**은 dict 가 마지막 값을 조용히 취해 집행을 통째로 껐다(→ exit 2) · `len(fields) != 3` 이 **>3 방향으로 미검사**여서 unit 경로의 탭 하나가 `unclaimed` 를 `fields[1]` 밖으로 밀어내 **진짜 PASS 유출**을 만들었다(실측) · `granularity` 는 이 설계가 고정한 **닫힌 집합**이라 검사한다(러너 이름은 소유자 것이라 검사하지 않는다 — AC38·AC52) · `splitlines()` 가 생산자보다 넓은 줄 모델이라 `split("\n")` 으로 좁혔다. (검증: T95 · T95′ · T95″ · T97.)
- **AC70** (배정 산출의 원자적 쓰기 + 실패 라우팅 — `/qg` iter-8) — R1b 는 `assign` stdout 을 `$assign_rows_file.part` 로 받고 **성공했을 때만** 최종 경로로 `mv` 하며 `pipefail` 을 켠다. 앞 버전은 최종 경로로 직접 리다이렉트했는데, 셸은 **명령이 돌기 전에** 대상을 만들고 절단하므로 인자 검증에서 즉사한 `assign`(0바이트)도 루프 중간에 죽은 `assign`(문법적으로 완전한 **접두 행**)도 R8 에게는 *"`unclaimed` 0건"* 과 **바이트 단위로 구분되지 않았다** — 형제 `--aggregate` 는 같은 입력에 `exit 4` 를 내는데 이쪽만 `exit 0` 이었다(실측). 이제 실패한 실행은 최종 경로에 파일을 남기지 않고 `--assign-rows` 가 부재로 `exit 4` 를 낸다. R1b 는 R6·R7 과 같은 모양의 **실패 라우팅 표**를 갖는다 — 이 SKILL 에서 **유일하게 라우팅이 없던** 결정론 호출이었다. **행 0개는 실패가 아니다**: 생산자가 완주하고 결과가 빈 것은 §11 ⑭ 의 축이며 이 AC 가 판정하지 않는다(0행을 오류로 만드는 수정은 ⑭ 를 부수효과로 닫아 정당한 빈 스코프에서 PASS 를 구조적으로 불가능하게 만든다). 남는 틈은 **정상 종료 + 잘린 stdout**(ENOSPC)이며 §11 ㉛ 에 등재했다. (검증: T94 ① · T97.)
- **AC69** — 오케스트레이터 소유 중간 파일 6종의 **위치가 R-init 에서 정의**된다. `mktemp -d` 로 만든 실행-스코프 디렉토리 하나에 살며, `$project_dir` 안도 `$evidence_dir` 안도 아니어야 한다. 앞의 금지는 실측이다: `create-sandbox` 는 `ls-files --others --exclude-standard` 로 **미추적·비-ignore 파일을 샌드박스로 복사한 뒤 커밋 `B` 로 봉인**하고 `.claude/quality-gates/worktrees/*` 만 건너뛴다 — 레포가 `.claude/` 를 ignore 하지 않으면 중간 파일이 기준선 커밋 안으로 들어간다. 뒤의 금지는 `$evidence_dir` 이 R5a³ 에서 verifier 에게 넘어가기 때문이다(피검자가 대조 원본을 쓰면 대조가 자기 자신을 대조한다). 어댑터별 4종은 러너 이름으로 가르고, **변수에 바인딩하지 않고 사용 지점에서 `$qg_run_tmp/<역할>-$runner.<확장자>` 로 전개한다** (iteration 2 정정): 앞의 두 판본은 R-init 에서 `$runner` 를 전개했다가(미바인딩 → 네 경로가 한 이름으로 붕괴) 셸 함수로 바꿨는데, **함수는 원리적으로 작동할 수 없다** — `Bash` 도구는 호출마다 새 셸이고 R-init 과 소비자 사이에 Agent dispatch 와 `AskUserQuestion` 이 끼어 있어 정의가 소비자에 도달하지 못하며, 실제로 그 판본은 정의 1건·**호출 0건**이었다. 정의 지점을 없애면 "정의를 부르는 것을 잊는" 실패 클래스도 없다. 같은 클래스의 네 번째 사례로 **`$baseline_wt` 이 사용 3건·대입 0건**이었던 것도 함께 닫았다(`create-baseline` stdout 을 잡는다 — 형제 `create-head` 는 같은 자리에서 이미 잡고 있었다). 이 요구가 SKILL `allowed-tools` 에 `Bash(mktemp:*)` 을 추가시키며 — 비-플러그인 명령 중 **항목을 가진 유일한 것**이지, 목록 밖 셸 명령이 없다는 뜻은 아니다(§11 ㉜) — 그 목록은 개수·순서 정확 린터(`check-allowed-tools-order.sh`)가 양쪽에서 잠근다. **custody 는 여전히 열려 있다**(§6.7 S1) — 이 AC 는 위치만 정한다. (§11 ㉕ 해소. 검증: **T96** + T94 ⓪ + 린터. 앞 버전은 T94 ⓪ 만 인용했는데 그것은 6종 중 하나의 *정의 순서*만 재고 (a)·(b)·(c) 어느 것도 재지 않았다 — iter-8 리뷰 high.)
...
**S1 에 대한 정정 (append — 열거 자체는 고치지 않는다).** S1 은 *"귀속 입력 파일 **4종**의 custody 부재"* 라고 적혀 있으나, `/qg` iter-8 이 오케스트레이터 소유 중간 파일을 **6종**으로 명시하면서 그 수가 stale 해졌다(`$assign_rows_file` · `$aggregate_yaml` · `$expected_units_file` · `$baseline_rows_file` · `$head_rows_file` · `$per_adapter_yaml`). 그리고 **노출 창이 파일마다 다르다는 사실이 어느 항목에도 적혀 있지 않았다** — iter-8 adversarial 이 다섯 리뷰어가 모두 놓친 축으로 지목한 것이다: `$assign_rows_file` 은 R1b(verifier 턴 **앞**) 기록 → R8(턴 **뒤**) 소비이고, `$aggregate_yaml` 은 R6(턴 **뒤**) 기록이라 창이 없으며, **`$baseline_rows_file` 이 가장 나쁘다** — R4(턴 앞) 기록 → R6(턴 뒤) 소비인데, `assign-rows` 변조가 degrade 를 *제거*만 하는 반면 baseline 행을 `pass`→`fail` 로 뒤집으면 모든 `NEW_REGRESSION` 이 `PRE_EXISTING` → `closed` → **degrade 신호 없는 PASS** 가 된다. §5.4/AC59 가 세운 방어(*"`fail` 은 적중으로 나오지 않는다 — 항상 재검증한다"*)는 **캐시**를 지키지 조립된 행 파일을 지키지 않는다. 닫는 모양은 `snapshot_digest` 선례(오케스트레이터가 봉인을 쥔다)이며 **여섯 전부에 걸거나 하나도 안 걸어야 한다** — 하나만 봉인하면 나머지에 대한 공시가 안심시키는 방향으로 거짓이 된다.

**S1 정정에 대한 재정정 (iter-8 리뷰 iteration 2 — 시계가 틀렸다).** 위 정정은 노출 창을 **R5a³ verifier 턴 앞/뒤**로 쟀는데 그 단위가 틀렸다. 피검자가 통제하는 코드는 그 턴 말고도 **호스트 권한으로 세 번 더** 실행된다 — R4 의 `setup_cmd`/`run`, R5b 의 `setup_cmd`/`run`, R6 의 flaky 재실행. 올바른 단위는 **"저장소가 통제하는 코드가 호스트 사용자로 실행되는 임의의 지점"** 이고, 그 시계로 다시 재면 위 정정의 분류 셋 중 **둘이 틀렸다**: `$head_rows_file`(R5b 기록 → R6 소비, 사이에 R6 의 flaky 재실행이 `$head_tree_dir` 에서 저장소 코드를 돌리고 같은 파일에 행을 다시 쓴다)과 `$per_adapter_yaml`(R6 어댑터별 기록 → R6 말미 `--aggregate` 소비, 사이에 같은 재실행)은 **창이 없지 않다**. 그리고 위 정정이 **셋만 분류하고 셋을 남긴 것 자체가** 안심시키는 방향의 누락이었다 — 특히 `$expected_units_file` 은 `$baseline_rows_file` 과 **같은 창 계급**이며(R1b 가 고른 unit 목록 → R6 소비) `SILENT_DROP` 백스톱의 원본이고, 재시도 경로(R5b→R6→R7→R8, R4·R1b 재실행 안 함)에서 **두 번의 verifier 턴을 건넌다**. 최종 분류: **창 있음 5종**(`assign-rows` · `expected-` · `baseline-` · `head-` · `per-adapter-`) · **창 없음 1종**(`aggregate` — R7 은 순수 git). SKILL R-init 에 같은 6분할 표를 실었다. **덧붙여, 배정 행 파일의 집행이 *부재*에 실려 있다는 사실**(AC70 의 fail-closed 는 "최종 경로에 파일 없음" 을 신호로 쓰는데, 부재는 피검자가 *채울* 수 있다)도 이 항목의 일부로 공시했다 — 다만 `assign_rc` 는 오케스트레이터 턴 값이라 1차 라우팅은 남으므로 **세 조건이 겹쳐야** 성립한다. **행 수를 모델이 전사해 두 번째 인자로 넘기는 봉쇄는 채택하지 않는다** — 오케스트레이터가 행 수를 배울 곳이 그 파일뿐이라 대조 양쪽이 같은 의심 산출물에서 나오고, 이 라운드가 닫은 전사 seam 을 다시 연다(iteration 2 adversarial 이 제안된 수정을 이 근거로 기각).
...
| T94 | **R1b→R8 `unclaimed` 집행 사슬** — ① R1b 의 **모든**(∀) `assign` 호출이 **fenced 코드 블록 안에서** 원자적 쓰기 3종(`.part` 리다이렉트 · 성공 시 `mv` · `pipefail`)을 갖는다 ② R8 의 게이트 호출 블록이 두 대조 인자를 **각각 정확히 1회** 넘긴다. 앞 버전은 세 축 전부에 뚫려 있었다(전부 실측): raw `index()` 라 **산문**이 코드 assert 를 만족했고(리다이렉트를 지우고 *"이제 화면으로만 읽는다"* 라고 써도 GREEN), ① 이 ∃ 라 리다이렉트 없는 두 번째 호출을 덧붙이면 통과했고, ② 가 *포함* 만 봐서 `--assign-rows A --assign-rows /dev/null` 중복이 통과했다. mutation 6축 RED + 위양성 대조 1축 GREEN | AC68 · AC70 |
...
| T95′ | **파서 축 4종** — 같은 플래그 **반복** → 2(dict 마지막 값이 조용히 이기는 축) · **4필드** 행 → 4(unit 경로의 탭이 `unclaimed` 를 `fields[1]` 밖으로 밀어내는 진짜 PASS 유출; `!= 3` → `< 3` mutant 를 죽인다) · `granularity` 어휘 밖 → 4 · **정상 3종**(file·package·bulk) → 0(과차단 방지) | AC68′ |
| T95″ | **잔여 반환 지점 + 음의 짝** — 미커버였던 3곳(positional 초과 → 2 · 원장 OSError → 4 · stdin 비-UTF-8 → 4) + 형제 두 read 경로의 **음의** 픽스처(비-UTF-8 집계·배정 → 4). 앞 버전은 세 read 경로 중 **하나만** 잠가 놓고 *"세 경로가 같은 모양"* 이라 적었다 | AC68′ |
| T97 | **생산자-소비자 계약** — 실물 `assign` stdout(워크트리 밖 unit → 결정론적 `unclaimed` 행)을 그대로 `--assign-rows` 에 먹여 집행 발화(1)와 `degraded` 원장 통과(0)를 함께 단언. 앞의 모든 픽스처는 포맷을 **손으로 다시 써서** 소비자의 파서가 아니라 *픽스처 작성자의 이해*를 검증했다 — 토큰이 한쪽에서만 바뀌면 개수가 조용히 0 이 되고 모든 락이 GREEN 을 유지한다(드리프트 방향이 fail-open) | AC68′ · AC70 |
| T98 | **§11 ⑭ 경계 핀** — 배정 **행 0개**·공백만 → **exit 0**. 양의 방향으로 못 박는다: 이 케이스가 없으면 *"0행 거부"* 수정이 스위트를 통과하며 ⑭(빈 스코프)를 조용히 닫고, 정당한 빈 스코프에서 PASS 가 구조적으로 불가능해진다 | AC70 |
...
| T96 | **중간 파일 custody** — ① R-init 의 `qg_run_tmp=` 대입이 **정확히 1개**이고 RHS 가 `$(mktemp -d)` ② Runtime 절 전체에 `TMPDIR=` 대입 **0건**(`qg_run_tmp=` 를 키로 하는 어떤 규칙도 볼 수 없는 축) ③ 여섯 역할 이름의 **모든** 대입(∀)이 `"$qg_run_tmp/` 로 시작하고 각 이름 ≥1건 — 창은 **Runtime 절 전체**다(R-init..R1a 로 좁히면 `$runner` 가 실제로 바인딩되는 곳으로 정의를 옮기는 **올바른 수정을 락이 막는다**, 실측 4줄 RED) ④ 런타임 `case` 담김 가드 존재(`pwd -P` 양쪽 + `PASS 불가`). 앞 버전은 ①③ 이 ∃ · ② 가 같은-줄 조건 + 하드코딩 열거라 **네 가지 mutant 가 전부 생존**했다 | AC69 |
...
**AC ↔ 검증 완전성.** **AC1–AC70 전부**가 위 T 또는 §8.3의 V에 대응한다.
...
25. **✅ 해소됨 — 오케스트레이터 소유 중간 파일의 위치가 정의되지 않았다.** (`/qg`
    ...
    **부수로 드러난 것:** 이 요구는 SKILL 이 `mktemp` 를 부를 수 있어야 한다는 뜻인데
    ...
    비-플러그인 명령 중 **항목을 가진 유일한 것**으로 `Bash(mktemp:*)` 을 등재했다 —
    필요한 명령을 선언하는 것이 allowlist 의 용도다. 린터가 SKILL 과 canonical 배열
    양쪽을 잠근다. **다만 "목록 밖 셸 명령이 없다"는 뜻이 아니다** — §11 ㉜ 참조.
...
30. **원장 파일은 피검자가 먼저 쓰고 오케스트레이터가 이어 쓴다 — floor 줄 선기입이
    fail-closed 인 것은 설계가 아니라 다른 검사의 부수효과다.** (`/qg` iter-8 —
    security-reviewer 와 silent-failure-hunter 가 각각 문서 쪽·코드 쪽에서 독립 관측,
    adversarial 이 SUGGESTION→IMPORTANT 로 격상.) **이 항목은 원래 ㉕ 본문에 있었고,
    iter-8 의 ㉕ ✅해소 재작성이 그 문장을 지웠다** — 아직 안 닫힌 seam 의 유일한 등재
    기록이 해소 도장과 함께 사라진 것이라, stale 선언보다 나쁘다(§8.1 선언문 3회 재발 ·
    AC67 미등록과 같은 클래스). 실체: `$evidence_dir/runtime-evidence.md` 는 R5a³ 에서
    verifier 가 쓰고 R8 에서 오케스트레이터가 floor 5줄을 이어 쓴다. verifier 가 floor
    줄을 **미리** 써 넣으면 `check_qa_ledger.py` 의 `seen` 이 first-wins 라 중복이 생기고,
    현재 그것이 막히는 이유는 오직 **중복-차원 오류**다 — 그 검사가 언젠가 경고로 완화되면
    `degraded` 뒤에 `closed` 를 붙인 두 줄이 그대로 우회로가 된다. 등급: **잔여 결함**.
31. **`assign` 이 `exit 0` 을 내면서 stdout 이 잘린 경우는 여전히 안 잡힌다.** (`/qg`
    iter-8 — C3 의 잔여.) iter-8 은 R1b 를 `.part` → `mv` 원자적 쓰기 + `pipefail` 로
    바꿔 **생산자가 죽은 경우**(인자 검증 즉사 · 루프 중간 `die` · 시그널)를 전부
    "최종 경로에 파일 없음" → `exit 4` 로 보냈다. 남는 것은 생산자가 **정상 종료했는데
    쓰기가 잘린** 경우다 — `run-test-selection.sh` 는 `printf` 실패를 종료코드로 올리지
    않으므로 ENOSPC 등에서 도달 가능하다. 그러면 문법적으로 완전한 **접두 행**이 전량으로
    취급된다. 닫는 모양은 리뷰가 지정한 대로 생산자가 **행수 포함 완료 선언**을 마지막
    줄로 내고 소비자가 그것을 요구하는 것이며, `assign` 출력 형태를 바꾸므로
    `test_run_test_selection.sh` 의 정확-일치 케이스 다수를 함께 고쳐야 한다 — 이번
    라운드 범위 밖으로 두고 등재한다. 등급: **갭**(지배적 실패 모드는 닫혔다).
...
㉜ **`allowed-tools` 가 fenced 블록의 맨 셸 유틸리티를 덮는지 미측정** (iter-8 리뷰
    ...
    **다만 그 falsifier 는 이 브랜치보다 오래됐다**:
    ...
    (막힌 `mv` → 오케스트레이터가 `> "$assign_rows_file"` 로 즉흥 → AC70 이 닫은 fail-open
    재개방)은 **철회한다**. 남는 것은 **거짓 주장 하나**뿐이고 그것은 문장을 고쳐서 닫았다.
    grant 는 넓히지도 좁히지도 않는다 — 넓히면(`Bash(mv:*)`) 실측되지 않은 위험을 대가로
    도구 표면을 양보하는 것이고, 좁히기는 앞선 라운드가 이미 해로운 수정으로 기각했다.
    한 가지 비대칭은 남긴다: `pwd` 는 **대입 안의 명령 치환**이라 선행 명령 토큰이 없고
    `&&` 뒤의 `mv -f` 는 **독립 명령**이라 접두 매처가 실제로 보는 모양이다. 이 브랜치의
    Runtime 절은 아직 한 번도 실행된 적이 없으므로(§12 의 `claude -p --plugin-dir` 실측이
    미결) **이미 예정된 관측에 한 줄로 접어 넣는다** — 새 작업을 만들지 않는다.
    등급: **갭**(거짓 주장은 닫혔고, 사실 자체가 열려 있다).

㉝ **T96 ③a 의 값-기반 금지는 `=` 없는 경로 구성과 한 단계 간접을 못 본다** (iteration 2,
    LT7 잔여). ③a 는 "대입 문법 열거"(`export`·대문자·줄 중간·창 밖 4종 실측 생존)에서
    "목적지 값 금지"로 술어를 뒤집어 그 넷을 한 술어로 닫았다. **닫지 못하는 것을 성과로
    팔지 않는다**: `printf -v name "%s/.qg" "$project_dir"` 나 `read -r name <<< …` 는 `=`
    없이 같은 일을 하고, **새 변수를 경유한 한 단계 간접**은 텍스트로 원리적으로 못 본다.
    무한 열거(대입 문법)를 훨씬 작고 부자연스러운 열거(`=` 없는 경로 구성 idiom)로 바꾼
    것이며, 실제 집행자는 ④ 의 런타임 담김 가드다. **정규식을 더 넓히는 수정은 세 번째
    열거가 되므로 채택하지 않는다** (iteration 2 리뷰어·adversarial 양쪽이 같은 근거로
    거부). 등급: **갭**(④ 가 커버).
...
| `/qg branch` iter-8 Review 게이트 | 리뷰어 5(security-reviewer · **codex** · pr-review-toolkit:code-reviewer · silent-failure-hunter · pr-test-analyzer) + adversarial. **39건 → 4 CRITICAL / 18 IMPORTANT / 17 SUGGESTION.** 판정 한 줄: *"1·2 를 안 하면 이 커밋은 `unclaimed` 집행자를 추가한 것이 아니라 `assign` 실패를 '`unclaimed` 0건' 으로 세탁하는 새 경로를 추가한 것"*. ★ **세 리뷰어가 독립적으로 같은 뿌리에 수렴** — `> "$assign_rows_file"` 가 명령 실행 **전에** 파일을 절단하므로 죽은 생산자의 0바이트 파일이 정상 결과와 **바이트 단위로 구분되지 않았다**(형제 `--aggregate` 는 같은 입력에 exit 4, 이쪽만 exit 0, 실측). ★ **내 락 5개가 실측으로 뚫렸다**: T94 ① 이 raw `index()` 라 **산문**이 코드 assert 를 만족(리다이렉트를 지우고 *"이제 화면으로만 읽는다"* 고 써도 GREEN) · ① 이 ∃ 라 두 번째 호출 추가로 통과 · ② 가 *포함* 만 봐서 중복 인자 통과 · T96 ①③ 이 ∃ 이고 ② 가 같은-줄 조건 + 하드코딩 열거라 **4종 mutant 전부 생존**. **그리고 내가 ② 의 이빨 증거로 보고한 mutation 은 실제로 ① 이 잡은 것이었다** — ② 의 주장 축은 사실상 미측정이었다. ★ **adversarial 이 해로운 수정 6건을 막았다**: C1 의 "0행 거부"(§11 ⑭ 를 부수효과로 닫고 정당한 빈 스코프에서 PASS 를 구조적으로 불가능하게 만든다) · M2 의 bulk 봉쇄(§11 ⑫ *"공시하되 봉쇄 안 함"* 정면 위반) · M4 의 `create-sandbox` skip 목록 확장(Law 2 표면 양보) · SR1 을 한 파일에만 적용(공시가 안심시키는 방향으로 거짓) · F2 를 락보다 먼저 수정(락이 4줄 RED) · `Bash(mktemp:*)` 좁히기. ★ **adversarial 이 다섯 리뷰어가 안 본 축을 찾았다**(4번째 CRITICAL): 이 diff 가 `$baseline_rows_file` 에도 고정·열거 가능한 집을 줬는데 **그쪽이 더 나은 공격**이다 — R4(verifier 턴 앞) 기록 → R6(턴 뒤) 소비의 같은 창인데, `assign-rows` 변조는 degrade 를 *제거*만 하는 반면 baseline 행을 `pass`→`fail` 로 뒤집으면 모든 `NEW_REGRESSION` 이 `PRE_EXISTING` → `closed` → **degrade 신호 없는 PASS** 가 되고, §5.4/AC59 의 방어는 *캐시*를 지키지 조립된 행 파일을 지키지 않는다. 수정(근본 클러스터 3): ⓐ R1b 원자적 쓰기(`.part`→`mv`+`pipefail`) + 실패 라우팅 표 = C1·C2·C3·CX1·SR2 · ⓑ 락 3종 재작성(∀ · fenced · 열거→도출) = F1·G1·G2·G3·G7 · ⓒ R-init 런타임 담김 가드(`pwd -P` 양쪽) = CX3·SR5·H3·H4·H5. 함께: `$runner` 미바인딩(F2/CX2)을 `qg_paths_for()` 함수로, 파서 축 4종, §11 ㉚(㉕ 재작성이 지운 seam 재등재)·㉛(ENOSPC 잔여) 등재, §6.7 S1 개수·순서 사실 정정. 신규 AC70 · T95′ · T95″ · T97 · T98. **mutation 16축 RED**(리뷰어가 실측으로 뚫은 mutant 를 그대로 재현). ★ 그중 **2개가 처음엔 생존**했다 — `seventh_file_in_tree`(열거→도출로 닫음)와 `fields_lt3`(**같은 커밋에 내가 추가한 granularity 검사가 길이 축을 가려 내 4필드 케이스가 엉뚱한 이유로 통과**하고 있었다; `fields[2]` 를 정상값으로 두어 축을 격리). |
| `/qg` iter-8 Review 게이트 **iteration 2** (Retry 재검증) | 리뷰어 3(**codex** · security-reviewer · pr-test-analyzer) + adversarial 2패스. **18건 → 6 CRITICAL / 8 IMPORTANT / 4 SUGGESTION.** ★ **내가 iteration 1 에서 고친 것이 다시 결함이었다** — codex: `$runner` 미바인딩을 고치려 만든 `qg_paths_for()` 가 **정의 1건·호출 0건**. 그리고 **adversarial 이 codex 의 처방을 기각**했다: `Bash` 도구는 호출마다 새 셸이고 R-init 과 소비자 사이에 Agent dispatch·`AskUserQuestion` 이 끼어 있어 **함수는 원리적으로 작동할 수 없으며**, "호출이 존재함"을 락으로 걸면 **작동 불가능한 메커니즘을 못 박는다**. 세 판본이 같은 실수의 세 얼굴이었다(매번 *정의*를 고치고 *배선*을 안 고쳤다) → 정의 지점 자체를 제거. ★ **락 이빨 재측정: 16 mutant 중 11 생존.** 진단 한 줄: *"fence 인식이 경계를 닫은 게 아니라 옮겼다 — 산문에 대한 `index()` 가 셸 **주석**에 대한 `index()` 가 됐을 뿐이고, fencing 을 **반대 극성**에 적용했다"*(요구는 코드 안이어야 하고 금지는 산문까지 덮어야 하는데 정확히 반대로 걸려 있었다). 특히 **T96 ④ — 이 커밋이 산문-vs-코드 클래스의 답으로 추가한 assert 자신이 그 클래스의 가장 순수한 사례**였다(가드를 통째로 지우고 산문 한 줄만 남겨도 GREEN). ★ **adversarial 이 다시 해로운 수정을 막았다**: SR 의 `--assign-rows-expected N` 은 **공허**(오케스트레이터가 행 수를 배울 곳이 그 파일뿐이라 대조 양쪽이 같은 의심 산출물에서 나오고, 이 라운드가 닫은 전사 seam 재개방) · SR 의 §6.7 재분류는 **셋 중 둘이 틀림**(창의 시계가 verifier 턴이 아니라 "피검자 코드가 호스트 권한으로 도는 임의 지점" — R4·R5b·R6 flaky 재실행 셋이 더 있다) · LT8 의 (3a) 창 넓히기는 **면제 목록이 편집마다 자라는** LT7 반론의 재현 · `Bash(mv:*)` 로 grant 넓히기는 도구 표면 양보. ★ **내 측정이 adversarial 의 CRITICAL 하나를 철회시켰다**: `project_dir=$(pwd)` 가 merge-base 의 Step P0 에 이미 있어 *모든* 실행에서 여러 릴리스간 permission stop 없이 돌았다 → 운영 위험은 미발생 증거로 기각, **남는 것은 거짓 주장 하나**(§11 ㉜). ★ **adversarial 의 가설 + 내 검색이 아무도 안 낸 결함을 찾았다**: `$baseline_wt` 사용 3건·대입 0건(형제 `head_tree_dir` 은 같은 자리에서 제대로 잡는다). 수정: T94 ①(주석 제거·`&&` 연결·pipefail 줄 앵커) · T94 ②(`\|` 0건) · **T94 ③ 신규**(라우팅 문장 양+음, 같은-줄 공기 앵커 — `PASS 로 올리지 않는다` 단독은 이 창에서 body-unique 가 아니다) · T96 ①(요구/금지 분리: req=fenced R-init, proh=절 전체 1건) · T96 ③a(문법 열거 → **목적지 값 금지**) · T96 ③b(변수 대입 → **파일 이름 ∀**, 러너 판별 축을 처음으로 잼) · T96 ④(fence + 주석 제거 + **대조 probe** 추가) · LT9 픽스처(나쁜 바이트를 정상 필드 *안*으로 — masking 재발) · T98(바이트 모양 → **실물 생산자 출력 + 전제 가드**, §11 ㉛ 을 랜딩 가능하게) · T97 stderr 축 고정. **mutation 17축 전부 기대대로**(16 RED + 1 GREEN 정직 측정). ★ 그중 **T96 ④ 가 처음엔 생존** — 해소(`pwd -P`)만 재고 **대조(`case`)를 안 쟀다**: 경로를 정규화해 놓고 아무것도 비교하지 않는 가드가 남는다. 신규 §11 ㉜·㉝·㉞. |
| 다음 단계 | **`/qg branch` 재실행(iter-9)** — iter-8 리뷰 수정분(위 행)은 아직 재검증을 안 받았고, 직전 다섯 라운드가 모두 그 자리에서 새 CRITICAL 을 냈다. **이 브랜치는 여전히 병합 불가.** 남은 잔여(병합 차단): §11 **⑭**(빈 스코프 무집행 — B1 선결) · **⑰**(mode provenance) · **㉑**(bulk green 이 0건 인증 — 설계 결정 필요) · **㉒**(도말 인자 순서) · **㉘**(영향분 적절성 기준 부재 — codex block; 닫으려면 선택기 회귀 fixture 층이 필요해 이 브랜치 범위 밖) · **㉚**(㉕ 재작성이 지운 원장 선기입 seam — 재등재됨) · §6.7 **15 항목**(S1 은 6종·순서 사실로 정정). 열린 갭: §11 **⑲**(GC scope-creep) · **⑳**(트리 누수) · **㉖**(이빨 갭 6종 + 원장 table→test) · **㉙**(AC31·AC49 의 "취지" 문구) · **㉛**(`assign` 정상 종료 + 잘린 stdout — terminator 는 `assign` 출력 형태를 바꿔 `test_run_test_selection.sh` 다수 동반 수정 필요). 브랜치 밖으로 분리(선행, 병합 비차단): **㉔**(`.git/info/exclude` 영구 소실 — v2.2.0) · **㉗**(spike 테스트가 live codex 호출 + 추적 픽스처 덮어쓰기). 그 밖: 수동 V-행 · Runtime 게이트 실측(`claude -p --plugin-dir` 별도 세션) · **V10**(§6 AC id 집합 ⊆ §8 인용 집합을 세는 검사 — 같은 stale 이 세 번 났으므로 사람이 기억할 일이 아니다). **의도적으로 열어 둔 것**: B1(§6.7 F5) · C5. A1/C2 · U3 · ㉓ · ㉕ 는 닫혔다. |
