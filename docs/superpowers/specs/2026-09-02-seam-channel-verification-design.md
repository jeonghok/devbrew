...
- [7. 검증](#7-검증)
  ...
  - [7.2 삭제의 오라클 — 대체 계약의 행동 테스트](#72-삭제의-오라클--대체-계약의-행동-테스트)
...
- **N5 (전수)** — 격리 에이전트 검사는 **두 락의 짝**이다. ⑴ **이름 목록의 각 원소**가
  `tools: []` 를 선언한다 — 목록이 열거이므로 피검자가 값을 바꿔도 집합에서 이탈하지 못한다.
  ⑵ `agents/*.md` 중 `tools: []` 인 파일 집합이 **그 목록과 같다**(등식) — 다섯째가 추가돼도,
  넷 중 하나가 빠져도 RED 다. **선택자를 술어와 같은 값으로 두지 않는다**: 대상을 `tools: []`
  에서 «도출» 하면 `tools: Read` 로 넓히는 변이가 대상 집합을 벗어나 락이 공허참으로 통과한다
  (§10 이 「검사 대상이 자기 앵커를 쥐는 문제」로 기각한 형태의 재발이다).

## 2. 묶음 1 — 채널 오배정

**측정 근거 — 이 자리는 실측했다.** `shared/tests/fixtures/seamprobe/` 의 **M6**: `PostToolUse`
...
이 설계의 두 훅(matcher `Bash`)에 그대로 적용되지 않았다 — M6 이 그 간극을 메운다. 같은 표가
`:349-352` 에서 *"앞선 판의 ✅ 가 네 라운드의 리뷰를 통과하고도 틀렸다"* 고 스스로 경고하므로,
그 표를 근거로 삼는 것만으로는 부족했다.

**M6 의 한계** — 1회 · 단일 변형 · 헤드리스 `claude -p` · 이 하니스 버전. 모델의 출처 서술은
...
| 사문이 되는 서술 | `.../quality-pipeline/references/state-file-format.md:67` — sentinel 을 companion file 로 계속 참조 |
...
| `tests/test_brief_agents.sh:100-108` | verdict 따라 `tools: []` ↔ `tools: Read` | **무조건 `tools: []`** + §4.2 의 L1·L2 |
| `tests/test_reviewing_brief_skill.sh:162-175` | probe 절 전체에 종속 — 토큰 존재 락(`:162-163`) · 미실행 금지 서술(`:165-166`) · `WFAIL` 윈도우 ≥9줄(`:167`) · hard-gate 부재/advisory/D2 단언(`:168-172`) · `WFAIL_BASH` 펜스(`:173+`) | 전부 삭제 |
...
그대로 물려받고 전수성 단언(L2)만 더하는 이유가 이것이다 — 열거를 도출로 바꾸면 선례에서
...
| 락 | 형태 | 무엇을 잡나 |
|---|---|---|
| **L1** | `ISOLATED` 를 네 이름으로 확장하고, **각 원소**가 `tools: []` 인지 단언 | 값을 넓히는 변이 — 이름이 열거돼 있으므로 이탈 불가 |
| **L2** | `agents/*.md` 중 `tools: []` 인 파일 집합 **== `ISOLATED` 집합** (등식) | 다섯째 추가(목록 갱신 강제) · 넷 중 하나의 이탈 |

L1 만으로는 신규 격리 에이전트가 락 없이 들어온다. L2 만으로는 넷이 **동시에** 넓혀지면 등식이
유지돼 통과한다. 둘이 함께여야 두 변이 방향이 다 막힌다 — 「음의 락엔 양의 짝이 필요하다」의
같은 구조다. 두 락은 `test_brief_agents.sh` 에 두고 `test_seed_agents.sh` 의 열거는 그대로 둔다
(L2 가 그 넷을 이미 덮으므로 중복 확장이 필요 없다).
...
| 그 중복을 지키던 락 | `tests/test_arm_ledger.py:512-519` — `resolve_mode.py` **소스**에 `^PATH_PREFIX\s*=\s*"([^"]+)"` 정규식을 걸어 두 리터럴의 등식을 확인 | **삭제** — 아래 |
...
**`test_arm_ledger.py:512-519` 는 고치는 것이 아니라 지운다.** 그 락의 docstring 이 존재 이유를
스스로 적는다 — *"두 파일에 리터럴이 따로 있으므로 락이 없으면 한쪽만 바뀌어도 아무도 모른다"*.
통합하면 리터럴이 하나가 되어 **지킬 등식이 사라진다.** 소스에 정규식을 거는 형태라 그대로 두면
`assertIsNotNone` 이 RED 가 되는데, 그것을 「고쳐서 통과시키는」 것은 없어진 위험에 락을 남기는
것이다. 삭제가 맞고, 그 삭제는 §7.2 의 「예상된 RED」 목록에 오른다.
...
| M3 | project-init `validate_branch` 동일 (M1·M2 대칭) | 동일 | RED |
| M3b | `validate_commit` 은 `systemMessage` 에 **남는다** | 그것을 `additionalContext` 로 옮긴다 | RED (§2.2 의 재진입 비대칭을 락이 지킨다) |
| M4 | **L1** — `ISOLATED` 네 이름의 각 원소가 `tools: []` | 하나를 `tools: Read` 로 | RED |
| M5 | **L2** — `tools: []` 파일 집합 == `ISOLATED` 집합 | 다섯째 `tools: []` 에이전트를 픽스처로 추가(목록은 그대로) | RED |
| M5b | 〃 | 넷 **모두**를 `tools: Read` 로(등식은 유지, 양쪽이 함께 빔) | RED — **M4 가 잡는다** |
| M6 | 핸드오프가 네 인자 | `$AUDIT` 를 뺀다 | RED |
| M7 | 목적지 상수가 한 자리에서 온다 | 상수 **값을 바꾼다** | 런타임 메시지 6곳이 전부 새 값 |
| M8 | `next_phase` 형태 계약 | 값을 `not-a-pair` 로(콜론 없음) | RED |
| M8b | 〃 | 값을 `otherplugin:otherskill` 로 | **GREEN** — 외부 플러그인 이름을 요구하지 않는다는 것의 양성 짝 |

**M4·M5·M5b 가 함께여야 N5 가 성립한다.** 세 변이가 각각 다른 방향을 흔든다 — 값 넓히기(M4) ·
신규 추가(M5) · 동시 이탈(M5b). 앞 판본은 대상을 `tools: []` 에서 **도출**했는데, 그러면 M4 의
변이가 대상 집합을 벗어나 단언이 공허참이 되어 **GREEN 이 나온다.** 즉 앞 판본의 M4·M5 는
기대값을 산출할 수 없었다. 이것이 §4.2 를 도출에서 열거+등식으로 되돌린 이유다.

**M7 은 값 변경 변이다.** 삭제 변이로는 안 잡힌다 — 상수를 지우면 import 에러로 전부
죽으므로 「함께 죽었다」가 「한 자리에서 온다」의 증거가 되지 못한다.

**M1~M3 은 형태 락이고 그것으로 충분하지 않다.** 문자열이 어느 JSON 키에 담겼는지만 보므로,
하니스가 그 필드를 무시하게 바뀌어도 GREEN 이다. 그 빈칸을 메우는 것이 **M6 도달 측정**
(`shared/tests/fixtures/seamprobe/`)이며, 그것은 스위트가 아니라 재현 자산이다 — 하니스 버전이
바뀌면 다시 태워야 한다는 뜻이고, 그 사실을 §11 에 남긴다.

### 7.2 삭제의 오라클 — 대체 계약의 행동 테스트

**부재 락은 만들지 않는다.** 지운 것들에 대해 「없음」을 확인하는 락은 대상을 통째로 지워도
통과하므로 스위트의 GREEN 을 완료로 오독하게 만든다.

**그리고 「기존 스위트가 깨지지 않는 것」도 오라클이 될 수 없다.** 이 변경은 기존 락이 **깨지기를
요구**한다 — 아래 넷은 예상된 RED 다. 오라클과 대상이 반대 방향이면 오라클이 아니다.

| 예상된 RED | 왜 |
|---|---|
| `test_qg_publish_offer.sh` | offer 자체가 사라진다 |
| `test_setup_qg.sh:107-147` · `test_qg_gc.py:165-176` · `test_skill_orchestration_behavior.sh:538-620` | sentinel 배선이 사라진다 |
| `test_reviewing_brief_skill.sh:162-175` | probe 절이 사라진다 |
| `test_arm_ledger.py:512-519` | 지킬 등식이 사라진다(§5.2) |

**대신 대체 계약의 행동을 단언한다** — 삭제가 «불완전하게» 됐을 때(옛 동작이 살아남았을 때)
잡히는 것은 이쪽뿐이다:
...
| B3 | 파이프라인이 `publish-eligible.md` 를 **더는 쓰지 않는다** | 실행 후 세션 폴더에 그 파일이 없다 — 단 `publish-active.md` 는 그 단언의 대상이 **아니다**(§3.1) |
| B4 | `check_brief.py` 가 외부 플러그인 이름을 요구하지 않는다 | M8·M8b |
...
검증 실패 차단, `:752` dispatch 강제)를 자기 범위로 열어 두었다. 이쪽은 **런타임 메시지 문자열
6곳과 상수**이고(§5.2) 줄이 겹치지 않는다.
...
- **OQ-I 도달 측정의 수명** — 묶음 1의 근거인 M6 은 스위트가 아니라 **재현 자산**이다. 하니스가
  `additionalContext` 계약을 바꾸면 형태 락(M1~M3)은 조용히 GREEN 을 유지하고 도달만 사라진다.
  하니스 버전이 오를 때 M6 을 다시 태우는 것을 무엇이 촉발할지 정하지 않았다.
...
- **OQ-K `systemMessage` 의 반대 명제 미측정** — M6 은 `additionalContext` 의 도달만 쟀다.
...
| D2 | **baseline 결과 파일의 경로와 형식** — 「예상된 RED」와 선재 RED 를 구별할 수 있는 형태여야 한다 | §7.2 · §7.3 |
| D3 | **B1~B4 행동 테스트의 실행 방식** — 특히 B1(설치본에서 감사 파일 없이 도는 것)은 격리 설치가 필요할 수 있다 | §7.2 |
| D4 | **`next_phase` 형태 계약의 정확한 정규식과 픽스처 쌍** — 유효·무효 각각 | §4.3 · M8·M8b |
| D5 | **L2 등식 락의 구현 형태** — `agents/*.md` 스캔을 셸로 할지 python 으로 할지, 다섯째 픽스처를 어디 둘지 | §4.2 · M5 |
