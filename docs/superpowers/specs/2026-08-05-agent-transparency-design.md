...
- **탐지 (약함)** — 사람 발화 본문 · `AskUserQuestion` 라벨 · 거절 발화는 사용자가 직접 쓴 것이라
  버릴 수 없고 패턴·엔트로피 마스킹에 의존한다. **패턴이 못 알아본 비밀은 통과한다.** 이 잔여
  위험은 수용된 것이며 [§12 OQ-K](#12-미해결)에 남긴다.
...
되돌림(거절된 도구 사용 · 턴 도중에 끼어든 교정 발화)은 1절의 한 종류로 들어간다. 턴 도중의
발화는 대부분 *"이게 아니야"* 류의 교정이고, 사용자가 가장 되짚고 싶어 하면서 순진한 필터가
놓치는 항목이다.
...
| # | 기준 | 검증 |
|---|---|---|
| **AC29** | **A/B 측정 통과** — [§10-6](#10-검증-계획)의 네 작업 × 3회 반복을 켠/끈 두 조건으로 돌려 **이분법 임계치 4개**를 모두 만족 | 아래 |
...
6. **A/B 측정 (AC29, 머지 게이트)** — 시스템 프롬프트를 건드리는 변경이라 주장 대신 측정한다.
   ...
   **게이트 — 넷 다 기계로 판정된다 (사람 눈대중 없음)**

   | # | 작업 | 통과 조건 | 무엇을 지키나 |
   |---|---|---|---|
   | 1 | (a) | 켠 조건 **3/3**에서 최종 응답에 마크다운 표 행(`^\|`)이 **0개** | 과잉 방지 — 오타 하나에 표가 나오면 실패 |
   | 2 | (b) | **양 조건 3/3** 모두 `tests/test_calc.py` 통과(종료 코드 0). 두 조건의 통과 여부가 다르면 fail | 능력 비저하 (K1) |
   | 3 | (c) | 켠 조건 **3/3**에서 `Agent` 도구 결과 **이후** 응답에 마크다운 표 행이 **1개 이상** | G2 — 에이전트 결과 표면화 |
   | 4 | (d) | 켠 조건 **3/3**에서 `AskUserQuestion` 호출 **직전** assistant 텍스트에 굵은 라벨(`**…**`)이 **4개 이상** | **G1 · 브리프 C3** — 사용자가 유일하게 "확실하다"고 한 순간 |

   게이트 4가 이번에 추가된 것이다. 앞선 판(작업 3종)은 C3을 **하나도 유발하지 않아서**, 설계에서
   가장 확실했던 요구사항에 정적 텍스트 검사(AC3·AC4)밖에 없었다(리뷰 적발).
