...
실패한다.
   ...
   **러너**: `plugins/agent-transparency/tests/ab_gate.sh` 하나가 아래를 전부 수행하고 **머지 게이트
   ...
   # ★ **줄 단위로** 본다. 여러 줄을 한 문자열에 담아 case 로 태우면 `*disabled*` 가 먼저 걸려
   #    **활성 사본이 함께 있어도** '끈 조건 성립'으로 통과한다 — 이 검사가 막으려던 바로 그
   #    상태다(세 리뷰어가 각각 적발). 매치 0건(미설치)은 머지 전 정상 경로이므로 통과시킨다.
   ...
   if [ -n "$at_lines" ]; then
     while IFS= read -r ln; do
       ...
       case "$ln" in
         ...
         *disabled*|*비활성*) : ;;                      # 이 줄은 꺼져 있음
         *) echo "plugin list 의 상태 표기를 읽을 수 없다: [$ln] — OQ-AC 확인 후 재실행" >&2; exit 1;;
       esac
     done <<EOF
$at_lines
EOF
...
| **OQ-AF** *(신규)* | **`agent:` 를 지정한 skill의 `SKILL.md` **본문**이 그 fork 모델에 실제로 전달되는가** | spec 리뷰가 적발. 이 문서의 다른 플랫폼 사실은 전부 실측 라벨을 다는데(훅 payload · 명령 충돌 · `context: fork` 발화) 이 전제만 측정도 등재도 없었다. 전달되지 않으면 「할 일」·「무엇을 읽고 무엇을 읽지 않나」·「쓰는 방식」이 미도달이고 AC28 파리티가 재는 대상이 사라진다 — **그리고 그보다 크게, `` !`…` `` 로 주입되는 인벤토리 자체가 본문에 실려 가므로 [§5.2](#52-데이터-흐름) 데이터 흐름이 성립하지 않는다**(리뷰가 파급 과소 서술을 적발). 영향받는 계약: AC35 · AC16① · AC43 · AC46(가용성 센티널) · AC28. **구현 착수 시 임시 플러그인으로 실측**하고 결과를 `tests/probe/skill_body.txt` 에 AC48④와 **같은 네 줄 형식**으로 남긴다(AC35⑥이 그 파일을 필수 입력으로 요구한다 — OQ-AE에는 fail-closed 락이 있는데 이쪽에 없던 비대칭을 리뷰가 적발). 실측은 **두 값**으로 나눈다: ⓐ 본문 텍스트 도달 ⓑ `` !`…` `` 주입 결과 도달. 반증되면 규칙을 agent 정의로 옮기는 것만으로는 부족하고 **인벤토리 전달 경로를 되살리는 재설계**가 필요하다 — `agent:` 지정을 포기하고 기본 fork로 되돌리는 갈래를 택하면 그것이 무효화하는 AC27(*"`agent` 가 `Explore` 면 red"*)·AC48·AC50을 함께 개정해야 하며(OQ-AE ②와 묶인다), [§6.3](#63-standup--명령--skill--준비-스크립트)의 *"세 번째 사본을 만들지 않는다"* 결정과 AC28의 좌우변도 같은 커밋에서 고친다 |
