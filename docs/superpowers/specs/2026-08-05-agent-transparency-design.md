...
6. **A/B 측정 (AC29, 머지 게이트)** — 시스템 프롬프트를 건드리는 변경이라 주장 대신 측정한다.
   ...
   **루브릭 C — `/recap` 답변 (게이트 5)**
   ...
   ```
   아래는 /recap 명령에 대한 어시스턴트 응답이다. 각 질문에 yes 또는 no 한 단어로만 답하라. 설명하지 마라.
   Q1. 일어난 일이 시간순으로 나열돼 있는가? (종류별로 다시 묶이지 않았는가)
   Q2. 사용자가 골랐던 선택지의 문구가 요약되지 않고 그대로 인용돼 있는가?
   Q3. 접히거나 일부만 실린 항목이 있을 때, 그 사실과 원래 분량이 밝혀져 있는가? (접힌 것이 없으면 yes)
   Q4. 이 프로젝트에서만 통하는 말이 설명 없이 쓰인 곳이 없는가?
   ```
