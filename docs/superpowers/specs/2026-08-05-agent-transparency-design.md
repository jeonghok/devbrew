...
[읽는 쪽 — 사용자가 /standup 칠 때만]
   ...
   /standup ──▶ skill (context: fork, agent: agent-transparency:transcript-reader)
                   ...
                   ▼
                prepare_standup.py ──▶ 인벤토리 헤더(37~104줄, §6.3의 크기 표) + git 상태
...
| **`agent:` 를 지정한 skill의 fork에서 나오는 `agent_type`** | **`<플러그인>:<agent name>` — 네임스페이스 형태** (2026-08-08 실측, Claude Code 2.1.225. 임시 플러그인 `atprobe` 로 재서 `atprobe:transcript-reader` 관측) |
| **`SKILL.md` 본문과 `` !`…` `` 주입 결과가 그 fork 모델에 도달하는가** | **둘 다 도달한다** (같은 실측. 본문에 심은 canary와 주입 canary가 fork 답변에 그대로 나왔다) |
...
**이 플러그인의 전용 read-only agent**(`agents/transcript-reader.md`)를 쓴다. 그러면 `agent_type` 이
`agent-transparency:transcript-reader` 가 되어 훅이 **자기 플러그인의 fork만** 정확히 제외할 수
있다 — **2026-08-08에 실측으로 확인했다**(위 표):
...
**이 값이 바뀌면 무엇이 red가 되나.** 값은 2026-08-08에 쟀지만 **문서화되지 않은 플랫폼 동작**이라
바뀔 수 있다. 합성 payload로만 검사하면 플랫폼이 다른 라벨을 주어도 테스트는 green인 채로 이 절의
자기모순이 되살아난다. 그래서 관측값을 `tests/probe/agent_type.txt` 에 **네 줄 형식**(관측값 · probe
명령 · 원출력 · `claude --version`)으로 기록하고 AC48④가 그 파일을 **필수 입력**으로 삼는다 —
파일이 없거나 첫 줄이 훅이 제외에 쓰는 상수와 다르면 **red**. 실측이 라벨 형태만 확정하므로,
플러그인 이름이 확정되면 상수는 `agent-transparency:transcript-reader` 다. **그래도 바뀔 때를 위해**
[§12](#12-미해결)의 OQ-AE가 대안 갈래를 열어 둔다 — red만 걸어 두고 대안을 안 적으면 구현이 그
자리에서 멈춘다(리뷰가 적발).
...
않았다"* 를 적발했고, 2026-08-08에 임시 플러그인으로 쟀다 — **본문과 `` !`…` `` 주입 결과가 둘 다
도달한다**([§6.2](#62-subagentstop-훅)의 실측 표). 전달되지 않았다면 규칙을 `SKILL.md` 한 곳에 둔 이
...
└── tests/
    ...
    │                                    #   원출력 · claude --version) —
    │                                    #   agent_type.txt (AC48④) · skill_body.txt (AC35⑥) ·
    │                                    #   command_name.txt (AC39)
...
21 + 7 + 6 + 1(AC47) + 2(AC48·AC49) + 1(AC50) + 1(AC51) = **총 39건**. 삭제된 번호는 재사용하지 않는다.
AC50은 2026-08-08 라운드 3에서 추가됐다 — 같은 라운드가 만든 **무출력 두 갈래**를 재는 AC가
하나도 없었다. AC51은 구현 계획에서 신설됐다 — `commands/standup.md` 본문(스킬 이름·
...
| **AC50** | **훅 내부 예외 경로가 계약대로 동작한다** — 예외면 **`systemMessage` 만 담긴 JSON**(`additionalContext` 키 **부재**)을 내고 exit 0, stderr에 사유가 남는다 | 예외를 **직렬화 단계에서** 강제로 일으켜(`json.dumps`/`sys.stdout.write` 패치) 확인한다. **손상된 stdin은 이 경로가 아니다** — 그것은 AC36의 *"`에이전트` 로 대체하고 정상 출력"* 경로이며, 두 자극을 섞으면 같은 입력에 두 AC가 상반된 출력을 요구하게 된다(리뷰가 적발). **이 AC가 없으면** 2026-08-08에 메운 *"조용한 강등"* 구멍([§7](#7-에러-처리--강등))이 회귀해도 red가 나지 않는다. *전용 agent 무출력 갈래는 여기 넣지 않는다* — AC48③이 이미 양방향으로 잰다(라운드 3이 중복 규범을 만들었다) |
...
| **AC48** | **`/standup` fork 전용 agent가 있고 fail-closed `tools:` allowlist를 선언하며, 훅이 그 `agent_type` 에는 stdout 없이 exit 0 한다** | 넷 — ① 전용 agent 파일 존재 + skill frontmatter의 `agent` 가 그것을 가리킴 ② **`tools:` 가 (a) 키로 존재하고 비어 있지 않으며 (b) 그 집합이 `{Read, Glob, Grep}` 의 부분집합이다** — 두 조건의 **곱**이고, 검증 열에 **`tools:` 줄 삭제 mutation에서 red**를 명시한다. (a)가 load-bearing이다: 부분집합만 요구하면 키가 없거나 빈 값일 때 **공집합이라 공허하게 참**이 되어 green인데 플랫폼 의미는 정반대(미선언 = 전 도구 허용)이고, `disallowedTools` 단독 금지 문구는 그 갈래를 덮지 못한다 — 라운드 3이 열거의 구멍을 막으면서 공집합이라는 더 큰 구멍을 열었다(리뷰가 적발). `Glob` 은 **필수 원소**다 — [§12](#12-미해결) OQ-AD의 잔여위험 논증이 그 보유를 전제한다. **부재 열거가 아니라 지배관계로 판정하는 이유**: 금지 도구를 열거하는 검사는 내일 추가될 쓰기 도구를 오늘 담을 수 없어 시간축으로 fail-open이고, 그것이 [§4](#4-제약)가 `disallowedTools` 단독을 기각한 바로 그 근거다 — 앞선 판의 이 AC가 같은 함정을 밟고 있었다(리뷰가 적발). `disallowedTools` 단독이면 red ③ 훅 실행 **양방향**(그 `agent_type` 이면 stdout 비고, `Explore` 면 상수 A) ④ **`tests/probe/agent_type.txt` 가 계약 형식대로 존재하고 그 첫 줄이 훅이 제외에 쓰는 상수와 정확히 일치한다** — 파일은 **네 줄**이다: 플랫폼이 실제로 준 `agent_type` 문자열(앞뒤 공백 제거, 네임스페이스 포함 여부는 관측한 그대로) · 실행한 probe 명령 · 그 원출력 · `claude --version`. **뒤 세 줄이 없으면 red.** **다만 이 검사는 위조 불가능성을 주장하지 않는다** — 네 줄 모두 구현자가 손으로 적을 수 있으므로 이것은 *"실행했다"* 의 증명이 아니라 **재현 메모**다(리뷰가 라운드 3의 과장을 적발). 실제 측정을 요구하는 것은 이 AC가 아니라 [§12](#12-미해결) OQ-AE의 머지 전 확인이며, 이 AC가 막는 것은 *"아무 기록도 없이 넘어가는 것"* 까지다. 비교 대상은 agent frontmatter의 `name:`(bare `transcript-reader`)이 아니라 **훅 상수**다 — 둘의 표기가 다르므로 술어를 여기서 못박는다. **단위 테스트가 probe 를 직접 실행하지는 않는다** — 스위트 안에서 실물 CLI 실행을 요구하면 [§10-1](#10-검증-계획)이 방금 고친 실패(의존 미설치 환경에서 게이트가 구조적 통과 불가)를 재생산한다. **왜 ④가 필요한가**: 값은 **2026-08-08에 측정됐지만**(네임스페이스 형태 — [§6.2](#62-subagentstop-훅)) 그것은 **문서화되지 않은 플랫폼 동작**이라 드리프트할 수 있다. ①②③만 있으면 합성 문자열 검사라 플랫폼이 라벨을 바꿔도 green인 채로 [§6.2](#62-subagentstop-훅)의 자기모순이 되살아난다. ④는 그 드리프트를 red로 바꾸는 유일한 지점이다. *(앞선 판은 이 자리를 「재지 않았다」로 적어 두었는데, 같은 커밋이 §6.2에서 그 사실을 뒤집고도 이 문장을 갱신하지 않아 두 절이 서로 다른 최신성을 주장했다 — 리뷰가 적발.)* |
...
| **AC35** | **`SKILL.md`가 세 가지 트랜스크립트 사실과 「읽지 않는 것」 목록을 담는다** — ① 사람 발화 세 레코드 타입 ② `last-prompt` 제외 ③ 텍스트 없는 어시스턴트 레코드 건너뛰기 ④ `Bash` 문자열·파일 내용·`tool_result`·에이전트 반환값·서브에이전트 파일 ⑤ **표본 하한(1-0)의 두 항목** ⑥ **`tests/probe/skill_body.txt` 가 AC48④와 같은 네 줄 형식으로 존재하고, 첫 줄이 *"본문 미도달"* 이 아니다** | 여섯 항목 각각 + mutation(하나를 지우면 red). ⑤는 2026-08-08 라운드 4에서 추가됐다 — 하한을 신설해 놓고 그것을 재는 AC도 루브릭 문항도 없어, AC50이 자기 신설 근거로 든 *"락이 없으면 회귀해도 red가 없다"* 가 한 문단 옆에서 재현되고 있었다(리뷰가 적발). 앞선 판에서 이것들은 추출기 **코드**에 있었고 AC12–AC15가 검사했다 — 코드가 사라졌으므로 검사 대상이 지시문으로 옮겨간다 |
...
2. **mutation** — 통과가 정답인 assert는 모양으로 이빨을 판별할 수 없다. 대상 바이트를 **표기·값·
   ...
   **문서 안의 특정 문구·항목이 사라지면 red** 를 요구하는 AC **전부**다. 현재로는 **AC2 · AC3 ·
   AC4 · AC5 · AC16① · AC25 · AC28 · AC31 · AC33 · AC35 · AC38 · AC44 · AC46 · AC48④**.
   (라운드 4에서 AC25·AC28·AC33·AC48④를 더했다 — 규칙 (ii)를 만족하는데 열거에 없어, *"규칙과
   열거를 같은 값으로 맞춘다"* 는 라운드 3의 선언이 반대 방향으로 어긋나 있었다.)
...
| **OQ-AE** *(축소)* | **플랫폼이 `agent_type` 라벨 형태를 바꾸면 무엇을 하나** | 2026-08-08 실측으로 **현재 값은 확정됐다**(네임스페이스 형태 — [§6.2](#62-subagentstop-훅)). 남은 것은 *미측정*이 아니라 *미문서화 동작에 대한 의존*이며, 방어는 AC48④의 probe 파일 락이다. 값이 바뀌면 아래 두 갈래. **경로 둘**: ① 플랫폼이 **고유한 안정 라벨을 주면** 그 값을 훅 상수로 쓴다(AC48④의 probe 파일이 정본) ② **일반 `Explore` 와 구분 불가하면**, 이름 분기를 포기하고 *"`/standup` fork의 결과에도 설명 자리가 한 번 붙는다"* 를 수용한다 — 중복이지 오류가 아니며 K1에 걸리지 않는다. **경로 ②를 택하면 함께 개정해야 하는 것**(리뷰가 적발 — 안 적으면 탈출구가 탈출시키지 못한다): **AC48③을 삭제**하고 · **AC48④의 「첫 줄이 훅 상수와 일치」 절만 면제**(probe 파일 존재·네 줄 형식 요구는 **유지** — 면제를 AC 전체로 넓히면 *"재지 않고 넘어가는 경로를 없앤다"* 가 무너진다) · 훅을 **두 갈래**(상수 B / 상수 A)로 되돌리고 [§6.2](#62-subagentstop-훅) 불변식의 *"세 갈래"* 와 [§8](#8-파일-목록) 트리 주석을 함께 고친다. AC7·AC36의 *"상수 A·B 갈래에서"* 스코프는 경로 ②에서도 참이므로 **개정 대상이 아니다.** **`/standup` 을 메인 대화에서 실행하는 갈래는 열지 않는다** — [§11](#11-기각된-대안)이 *"발췌가 메인 컨텍스트에 영구히 쌓여 `/compact`를 앞당긴다"* 로 이미 기각했다 |
| **OQ-AF** *(축소)* | **플랫폼이 프롬프트 구성 방식을 바꿔 `SKILL.md` 본문·주입 결과가 fork에 안 닿게 되면** | spec 리뷰가 미측정을 적발했고 **2026-08-08에 쟀다 — 본문과 `` !`…` `` 주입 결과가 둘 다 도달한다**([§6.2](#62-subagentstop-훅) 실측 표). 그러므로 아래는 *지금 모르는 것*이 아니라 *바뀌면 무너지는 것*의 서술이다. 전달되지 않으면 「할 일」·「무엇을 읽고 무엇을 읽지 않나」·「쓰는 방식」이 미도달이고 AC28 파리티가 재는 대상이 사라진다 — **그리고 그보다 크게, `` !`…` `` 로 주입되는 인벤토리 자체가 본문에 실려 가므로 [§5.2](#52-데이터-흐름) 데이터 흐름이 성립하지 않는다**(리뷰가 파급 과소 서술을 적발). 영향받는 계약: AC35 · AC16① · AC43 · AC46(가용성 센티널) · AC28. 관측을 `tests/probe/skill_body.txt` 에 AC48④와 **같은 네 줄 형식**으로 남긴다(AC35⑥이 그 파일을 필수 입력으로 요구한다 — OQ-AE에는 fail-closed 락이 있는데 이쪽에 없던 비대칭을 리뷰가 적발). 관측은 **두 값**이다: ⓐ 본문 텍스트 도달 ⓑ `` !`…` `` 주입 결과 도달 — 2026-08-08에는 **둘 다 도달**. 반증되면 규칙을 agent 정의로 옮기는 것만으로는 부족하고 **인벤토리 전달 경로를 되살리는 재설계**가 필요하다 — `agent:` 지정을 포기하고 기본 fork로 되돌리는 갈래를 택하면 그것이 무효화하는 AC27(*"`agent` 가 `Explore` 면 red"*)·AC48·AC50을 함께 개정해야 하며(OQ-AE ②와 묶인다), [§6.3](#63-standup--명령--skill--준비-스크립트)의 *"세 번째 사본을 만들지 않는다"* 결정과 AC28의 좌우변도 같은 커밋에서 고친다 |
