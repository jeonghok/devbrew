...
앞선 판에서 여기 있다가 **본문으로 옮겨 확정된 것**(리뷰 지적): A/B 측정의 작업 프롬프트 4종과
픽스처 정책은 [§10-6](#10-검증-계획)에 고정됐고, 테스트는 합성 픽스처만 쓴다([§10-4](#10-검증-계획)).
...
- **탐지 (약함)** — 사람 발화 본문 · `AskUserQuestion` 라벨 · 거절 발화 · 에이전트
  `subagent_type` 과 `description` 은 버릴 수 없고 패턴·엔트로피 마스킹에 의존한다. **패턴이 못
  알아본 비밀은 통과한다.** 이 잔여 위험은 수용된 것이며 [§12 OQ-K](#12-미해결)에 남긴다.
  이 다섯 필드는 [§6.3](#63-recap--명령--skill--추출기) 「출력에 들어가는 것」 표의 마스킹 대상과
  **정확히 같은 목록이어야 하며**, AC30이 그 일치를 검증한다.
...
| AC20 | 3절이 트랜스크립트가 아니라 `git log` · `git diff --stat` 에서 온다 | git 없는 픽스처에서 3절이 비고 그 사유가 표시된다 |
...
| **AC30** | **§3 「탐지(약함)」의 필드 목록과 §6.3 「출력에 들어가는 것」의 마스킹 대상이 같은 집합이고, 그 다섯 필드가 실제로 마스킹된다** | 두 목록을 파싱해 집합 비교(한쪽에만 있으면 red) **AND** 다섯 필드 각각에 비밀을 심은 픽스처에서 마스킹 확인 |
...
| # | 기준 | 검증 |
|---|---|---|
| **AC29** | **A/B 측정 통과** — [§10-6](#10-검증-계획)의 네 작업 × 3회 반복을 켠/끈 두 조건으로 돌려 게이트 4개를 모두 만족 (1·2는 구문, **3·4는 루브릭 3표 다수결**) | §10-6 |
| **AC32** | 루브릭 A·B의 **전문이 이 문서에 있고** 각각 4문항이며, 게이트 3·4가 개수 검사가 아니라 그 루브릭으로 판정한다 | 루브릭 블록 존재 + 4문항 + 게이트 표가 "루브릭"을 판정 방식으로 명시. 개수 기반 문구가 남아 있으면 red |
...
6. **A/B 측정 (AC29, 머지 게이트)** — 시스템 프롬프트를 건드리는 변경이라 주장 대신 측정한다.
   ...
   **실행**: 두 조건 × 네 작업 × 3회. 모델·effort는 두 조건에서 동일하게 고정하고 그 값을 결과에
   기록한다. 출력은 트랜스크립트 파일에서 읽는다(터미널 캡처는 렌더 아티팩트가 섞인다).

   ```bash
   for t in a b c d; do for i in 1 2 3; do
     git -C fixtures/ab-project checkout -- .
     claude -p "$(cat prompts/$t.txt)"                                        # 끈 조건
     git -C fixtures/ab-project checkout -- .
     claude -p --plugin-dir plugins/agent-transparency "$(cat prompts/$t.txt)"  # 켠 조건
   done; done
   ```

   **네 작업의 프롬프트 (문서에 고정 — 구현 계획으로 미루지 않는다)**

   | | 프롬프트 |
   |---|---|
   | (a) | `README.md 3번째 줄의 오타 'teh' 를 'the' 로 고쳐줘` |
   | (b) | `src/calc.py 의 add 함수가 음수도 처리하도록 고치고 기존 테스트가 통과하는지 확인해줘` |
   | (c) | `이 저장소에 정의된 함수가 몇 개인지 서브에이전트를 써서 세고 결과를 보고해줘` |
   ...
   **게이트 — 사람 눈대중 없이 판정된다. 다만 판정 방식이 둘로 갈린다**
   ...
   | 3 | (c) | 켠 조건 **3/3**에서 `Agent` 도구 결과 직후 응답이 **루브릭 A를 통과** | **루브릭** | G2 — 에이전트 결과 표면화 |
   | 4 | (d) | 켠 조건 **3/3**에서 `AskUserQuestion` 호출 직전 응답이 **루브릭 B를 통과** | **루브릭** | **G1 · 브리프 C3** — 사용자가 유일하게 "확실하다"고 한 순간 |
   ...
   *"버린 선택지와 그 이유를 밝혔는가"* 는 자연어 요구사항이고, 굵은 라벨을 세는 검사는 **무관한
   ...
   해당 응답 본문을 별도 모델 호출에 넘기고 아래 문장을 **그대로** 프롬프트로 쓴다. 판정자는
   ...
   **루브릭 B — 결정 요청 직전 (게이트 4)**
   ...
   판정 호출 수: 산출물 6개(게이트 3·4 × 3회) × 3표 = **18회**. 머지 전 1회성이다.
...
| **OQ-L** | 게이트 3·4의 루브릭 판정에 **모델이 들어온다** — 판정자도 틀릴 수 있다 | 두 리뷰어가 세 라운드에 걸쳐 지적한 "구문 검사가 내용을 못 본다"의 해법으로 채택. 3표 다수결이 흔들림을 줄이지만 없애지는 못한다. 루브릭이 본문에 있어 **재현·반박 가능**한 것이 개수 검사와의 차이다 |
