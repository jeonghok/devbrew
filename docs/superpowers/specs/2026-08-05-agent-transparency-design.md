...
5-1. **1-0 하한에 못 미치면 그 사실과 빠진 항목을 답변 첫 줄에 함께 적는다** — 예를 들어
   ...
   (I4 — [§7](#7-에러-처리--강등)의 "1-0 하한 미달" 행이 요구하는 것을 지시문에 실제로 심는다).
...
실패한다.
   ...
   **러너**: `plugins/agent-transparency/tests/ab_gate.sh` 하나가 아래를 전부 수행하고 **머지 게이트
   ...
   ```bash
   #!/usr/bin/env bash
   # ★ 이 기계의 bash 는 3.2 뿐이다(macOS 기본, 유일 실행 환경) — mapfile · readarray ·
   #   연관배열 같은 bash 4 전용 구문을 쓰지 않으므로 버전 가드를 **두지 않는다**.
   #   가드를 남기면 게이트가 한 번도 못 돈다(I5 — 앞선 판은 여기 `${BASH_VERSINFO[0]} -ge 4`
   ...
   #   ::test_no_bash4_only_constructs` 가 `${BASH_VERSINFO` 부재를 락으로 고정한다).
