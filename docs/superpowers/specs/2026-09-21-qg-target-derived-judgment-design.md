...
```
C   = git log --all --grep='^Spec: <경로>\(#[^ ]*\)\?$' --format='%H'   # 선언 커밋(조각 포함)
...
1. **살아 있는 ref** — `git for-each-ref refs/heads` 중 `C` 의 원소를 포함하면서 **`base_ref` 의
   조상이 아닌** 것. `main` 과 무관한 브랜치는 `C` 를 포함하지 않으므로 자동으로 빠진다.
2. **ref 가 사라진 머지분** — `C` 의 원소 `c` 가 어느 살아 있는 ref 에도 안 걸리면, `c` 를 처음
   담은 머지 커밋 `m`(= `git rev-list --ancestry-path --merges c..base_ref | tail -1`)의
   **주제 쪽 부모**를 그 브랜치의 끝으로 쓴다.
3. 둘 다 안 나오면 그 선언 커밋은 **고아**다 — `not-certified (declaration-invalid)`.
