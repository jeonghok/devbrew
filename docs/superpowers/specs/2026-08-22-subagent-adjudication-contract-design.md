...
**「링크를 쓸 수 없는」 사례로 기록된 것은 하나뿐이고 이 모듈에 닿지 않는다.**
`plugins/quality-gates/CHANGELOG.md:261-264` 의 사유는 `P21_PREAMBLE_PATH` 가
`Path(__file__).resolve().parent` 로 **형제 데이터 파일**을 찾는다는 것이다 — 링크면
`.resolve()` 가 그 형제 프레임을 `shared/codex/` 로 옮긴다. `adjudication.py` 는 **형제
경로를 하나도 해석하지 않는다**(순수 회계 클래스). 그 선례가 이 모듈을 덮지 않는다.

따라서 배포는 **상대 심볼릭 링크**이고, 계약은 축 1a(심볼릭 링크 ∀)가 진다.

✎ **앞선 세 판본이 여기서 연속으로 틀렸고, 마지막 것은 결론까지 틀렸다.**
①*"`shared/` 에 문서가 없다"* — 거짓(`shared/README.md` 가 load-bearing).
②*"import 전용이라 축 1a 참조 도출이 0건 → `:447` RED"* — 거짓(배포 지점 도출은
`test_copy_of_contract.sh:369-378` 의 구조 도출이 주(主)이고 워킹트리 갈래가 `[ -L "$_p" ]`
로 링크를 명시 인정한다).
③*"exec 이냐 import-only 냐가 두 축을 가른다"* — 거짓(`:775-784` 가 `n_canon == n_consumed`
를 단언하며 exec 필터가 떨어뜨린 정본을 RED 로 잡는다. 그 필터는 **가드된 no-op** 이고,
축 1a 의 소속은 `:309-323` 의 **실재하는 심볼릭 링크**에서만 도출된다 — 어디에도 exec
판별이 없다).
④ 그리고 ③의 근거로 인용한 `kill_switch_active.py:16-19` 의 *"설치본에는 `shared/` 가
없으므로 형제 사본이어야 한다"* 는 **위 `:764-769` 실측이 뒤집은 믿음을 적어 둔 주석**이다.

**네 번 모두 같은 실패다 — 주석이나 부분 코드를 실제 동작으로 읽고, 이미 존재하는 결정
기록을 찾지 않은 것.** 이 문서에서 배포·집행에 관한 주장은 **결정 기록 또는 실행 관측**을
인용해야 하며, 락 소스의 주석은 근거가 아니다.
...
✎ 앞선 두 판본이 이 자리에서 연달아 틀렸다. ①*"축 1c 는 PR1 에서 이 정본을 안 보니 안전"*
— `CONSUMED_PY` 경로만 보고 M6 앵커를 못 봤다. ②*"그러니 PR1 에서 사본을 빼자"* — 문제를
사본 방식이 만든 것이었고, 배포 형태가 링크로 바로잡히자(§2.1) **그 제약 자체가 사라졌다.**
틀린 전제 위에서 최적화한 대가다.
...
앞선 판본의 A② 는 *"바로 **위** 40줄"* 이라 이 파일에서 qualifying 앵커가 0개가 되어
**PR3 가 배달 즉시 RED** 였다. `---` 위에는 아무것도 놓을 수 없으므로 규칙을 바꿀 쪽은
앵커 거처가 아니라 방향이었다. 그래서 A② 는 「바로 **아래**」다 — 다른 세 표기도 전부
...
- **`dispatch 줄 수 == 앵커 수` 등식을 단언한다**(축 A①). 구조적 근거가 있다 — 1:1 계약.
  ✎ **이 문장은 다른 단언을 배제하지 않는다.** 앞선 두 판본이 여기서 *"단언은 하나다"* →
  *"수를 비교하는 단언은 하나뿐이다"* 로 적었는데, 둘 다 **§5.1⑤(에이전트별 dispatch ≥ 1)와
  위 두 vacuity bullet(0이면 RED) 까지 무효화**한다 — 그것들도 수 비교다. 문자 그대로
  구현하면 M6·M7 의 RED 기제가 사라진다. **배제 문장을 쓰지 않는 것이 수정이다**
...
| **M16** | `consumer=` 를 **어휘 밖 5번째 값**으로 교체(예: `consumer=maybe`, 서식은 유효) | **축 A④ RED**(닫힌 어휘) | 그 값이 실패 메시지에 등장. ✎ 앞선 판본의 §14 는 이 자리에 *"M2 변형"* 이라고 적었는데 **그런 mutation 은 §10.2 에 없었다** — 계획된 계측기가 0인 채 커버리지를 주장한 행이다. 검출은 느슨하므로(§5.2) 이 앵커는 검출되고 A④ 에서 걸린다 |
| **M17** | **락에서 ⑤축별 대상 수 인쇄를 삭제** | **§5.3 인쇄 단언 RED** | 인쇄값 6종 중 하나가 없다는 메시지. ✎ 이것이 없으면 M9·M10 의 관측 근거가 조용히 사라져도 둘 다 green-expected 라 통과한다 |
...
| **PR3** | 앵커 18줄 + `test_dispatch_disposition.sh`(축 A①②③④ · B · C) + mutation M1~M17 | 축 B 의 대상이 PR2 에서 이미 전환됐으므로 단독 GREEN |
...
| §5.3 의 여섯 인쇄값 (§5.3) | §5.3 인쇄 단언 | **M17** |
