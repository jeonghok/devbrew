...
### 2.1 배포 형태 — 규칙은 「사본이냐 링크냐」가 아니라 「exec 이냐 import-only 냐」다

`shared/README.md:10` 은 상대 심볼릭 링크를 기본으로 적지만, **실제 판별 기준은 그 문장이
아니라 락에 인코딩돼 있다.** `test_copy_of_contract.sh` 는 `CONSUMED_PY` 에서
`^if __name__ == "__main__"` 을 가진 것을 빼 `IMPORT_ONLY_CANONICALS` 를 만들고, 두 축이
그 분류를 따라 갈린다 — **exec 되는 정본은 축 1a(심볼릭 링크), import-only 정본은
축 1c(형제 사본)**. 실측이 이를 뒷받침한다: `codex_findings_to_yaml.py`(실행 지점 있음)는
심볼릭 링크로, `kill_switch_active.py`·`gc_common.py`·`codex_jsonl.py`(import-only)는
물리 사본으로 배포돼 있다.

이 모듈은 실행 지점이 없는 **import-only 정본**이므로 형제 사본이다. 그 이유는 이 리포에
이미 두 곳에 적혀 있다:

- `plugins/quality-gates/CHANGELOG.md:259-264` — *"링크로 배포하면 `.resolve()` 가 링크를
  따라가 그 형제가 `shared/codex/` 로 해석된다 … 리포에서는 통과하고 **설치본에서만**
  P21 이 빠진 프롬프트가 나가는, 관측되지 않는 실패가 된다."*
- `plugins/spec-distill/scripts/kill_switch_active.py:16-19` — *"설치본에는 `shared/` 가
  없으므로 **형제 사본이어야 import 가 풀린다**."*

✎ **앞선 두 판본이 여기서 연속으로 틀렸다.** ①*"`shared/` 에 문서가 없다"*(거짓 —
`shared/README.md` 가 load-bearing) ②*"import 전용이라 축 1a 참조 도출이 0건 → `:447` RED"*
(거짓 — 배포 지점 도출은 `:369-378` 의 **구조 도출(인덱스 ∪ 워킹트리 글롭)** 이 주(主)이고
워킹트리 갈래가 `[ -L "$_p" ]` 로 링크를 명시 인정한다. 인용했던 `:441-443` 은 산문 단독
도출 시절의 낡은 주석이다). **근거를 지어내지 말고 리포에서 찾을 것** — 두 번 다 이미
적혀 있었다.
...
**정본과 사본은 같은 PR 에 실리지 않는다 — 사본은 첫 소비자와 함께 온다.**
`test_copy_of_contract.sh:801-823`(축 1c 의 M6 앵커)이 *"배포 트리에 형제 사본이 실재하는데
축 1c 집합에 없다 — 소비자 탐지가 이 정본을 통째로 잃었다"* 를 RED 로 잡는다. 그 집합은
`CONSUMED_PY`(`:753`) → `IMPORT_ONLY_CANONICALS` 로 도출되는데 **소비자가 있어야** 들어간다.
따라서 *정본 + 사본은 있고 소비자는 없는* 상태는 그 앵커가 겨냥한 바로 그 상태다.

그러므로:

| PR | 정본 | 사본 | 축 1c |
|---|---|---|---|
| PR1 | 만든다 | **안 만든다** | 사본이 없으므로 `n_sib ≥ 1` 이 거짓 → M6 앵커 미발화 |
| PR2 | — | 첫 소비자와 **같은 PR** 에서 만든다 | 소비자가 생기며 집합에 들어감 → ∀ 발효 |

✎ 앞선 판본은 *"축 1c 는 PR1 에서 이 정본을 보지 않으니 안전하다"* 고 적었다. `CONSUMED_PY`
경로만 보고 **소비자 스캔과 무관한 세 번째 앵커(M6)를 못 봤다.** 「그 축이 나를 안 본다」는
「그 축이 나를 통과시킨다」가 아니다.
...
**무는 자리는 축 A④ 이고 분리 mutation 은 M15 다**(§14) — 앞선 판본은 이 요구를 여기 적어
놓고 §5.2 의 어느 칸에도 걸지 않아, 요구가 있는데 아무도 재지 않는 상태였다.
...
| | **A②** | **위치 규칙(결정론)** — 각 dispatch 줄에 대해, 그 **바로 위** 40줄 안의 앵커 중 **그 사이에 다른 dispatch 줄이 없는** 것이 정확히 하나 | 전체 | **M14b (앵커를 다른 dispatch 의 창으로 «옮기기» — 총계 18 유지)** |
...
| **C** | **C** | `consumer=` 가 `.js`·`orchestrator`·`human` 인 앵커: `disclosure=` 리터럴이 **코퍼스**에 실재한다 | 나머지 앵커 | M11 |

**A② 는 「∃ 완전매칭」이 아니라 결정론 배정이다.** 앞선 판본은 *"1:1 매칭이 존재한다"* 라고
썼는데 그것은 이분 완전매칭 **존재 명제**여서 배정 규칙이 없으면 구현할 수 없고,
...
요구하는데 앞선 판본의 축 표에는 그것을 무는 칸이 없었다 — 축 B 는 `.py` 한정이고 축 C 는
`disclosure=` 만 본다. 그래서 `consumer=plugins/x/scripts/없는파일.js` + 실재하는 리터럴이
세 축을 그대로 통과했다. **요구를 산문에 적고 집행 자리를 지정하지 않는 것**이 라운드 2~3 이
반복해서 잡은 모양이라, 이 문서는 이제 §14 의 요구→집행 표로 그것을 강제한다.
...
- **수(數)를 비교하는 단언은 하나뿐이다: `dispatch 줄 수 == 앵커 수`**(축 A①). 구조적 근거가
  있다 — 1:1 계약. ✎ 앞선 판본은 이것을 *"단언은 하나다"* 라고 써서 **§5.1⑤ · 축 A②③④ ·
  축 B · C · 위 두 vacuity bullet 을 전부 무효화**했다(그것들도 전부 단언이다). 문자 그대로
  구현하면 M6·M7 의 RED 기제가 사라진다. 여기서 「하나」인 것은 **등식**이지 단언 일반이 아니다
...
| **verdict** | **항목이 소실됐거나 그 축의 주(主) 판정자가 죽었을 때만 막는다.** 보조(모델 다양성) 손실은 공시하되 막지 않는다 | 아래 |
...
✎ **앞선 판본이 이 배너를 blocking 의 증거로 인용했다 — 범주 오류였다.** 그리고 무조건
`blocks()` 를 구현하면 `test_merge_review.py:130-135`(AC10) · `:144-148` · `:154-158`
**세 테스트가 깨진다** — 셋 다 `combined_verdict == "approved"` 와 `codex_degraded == "true"`
를 동시에 단언한다. 화석이 아니라 계약이다. 게다가 codex 가 지금 불가이므로(§R9) 그 구현은
**모든 라운드를 `needs_revise`** 로 만든다.
...
- **배포 정합성 (∀, 도출 집합 위에서) — PR2 부터 유효하다.** PR0 이 확정한 배포 집합의
  **모든** 플러그인에 대해 `plugins/<p>/scripts/adjudication.py` 가 존재하고, 1행
  `# copy-of:` 마커가 정본을 가리키며, 그 줄을 뺀 바이트가 정본과 같다. **실재하는 사본만
  순회하면 안 된다** — 그것은 ∃-검사라 빠진 자리에 침묵한다. 기대 집합을 먼저 도출하고
  그 전부를 검사한다.
  ✎ **PR1 에서는 이 단언을 두지 않는다** — 배포 집합이 공집합이라 ∀ 루프가 0회 돌고
  vacuous GREEN 이 된다(라운드 3 N7). PR1 의 산출물은 정본 하나뿐이고 그것은 위 여섯
  bullet 이 직접 잰다. 축 1b·1c 가 사본을 이미 재므로 이 단언과의 중복 여부는 PR2 착수 시
  판정한다
...
| **M15** | `consumer=` 를 실재하지 않는 `.js` 경로로 교체(`disclosure=` 는 실재 리터럴 유지) | **축 A④ RED**(경로 실재) | 그 경로가 실패 메시지에 등장. ✎ 이 mutation 이 없으면 §4.1 의 경로 실재 요구가 집행되는지 아무도 모른다 — 라운드 3 N6 가 그 구멍이었다 |
...
| **M14b** | 앵커 하나를 **다른 dispatch 의 창 안으로 옮긴다 — 총계는 18 유지** | **축 A② 단독 RED** — 원래 dispatch 위 40줄에 「사이에 다른 dispatch 가 없는」 앵커가 0개, 옮긴 쪽은 2개 | 매칭 실패한 dispatch 의 줄 번호. ✎ **A① 만 구현한 락은 이것을 GREEN 으로 통과시킨다** — 그래서 이것이 A② 의 유일한 분리 계측기다. 앞선 판본의 「합치기」는 총계가 깨져 A① 단독으로 RED 가 났고, 그래서 A①만 있는 락과 A①② 락을 **구별하지 못했다**(M13 의 ✎ 가 명명한 실패와 같은 모양) |
...
| **PR1** | `shared/adjudication/adjudication.py` 정본 + `test_adjudication_behavior.sh`. **배포 사본 없음** | 사본이 없으므로 축 1c 의 M6 앵커(`n_sib ≥ 1`)가 발화하지 않는다(§3). 정본만으로 모듈 테스트가 단독 GREEN |
| **PR2** | 결함 수리 9건 + **앵커가 지목할 모든 `.py` 소비자를 모듈로 전환** + **그 소비자가 사는 플러그인의 형제 사본** + 회귀 테스트 | 사본과 소비자가 **같은 PR** 에 들어오므로 축 1c 가 정상 발효하고 §10.1 의 ∀ 도 공집합이 아니다 |
| **PR3** | 앵커 18줄 + `test_dispatch_disposition.sh`(축 A①②③④ · B · C) + mutation M1~M15 | 축 B 의 대상이 PR2 에서 이미 전환됐으므로 단독 GREEN |
...
| **R7** | **#6 의 잔여는 verdict 의미가 아니라 「findings 폐기」다 — 이 사이클에서 닫지 않는다.** `CHANGELOG.md:1634-1638` 이 실측한 것은 mixed 라운드에서 **findings 전량이 사라지는** 것이다: `merge_review.py:490` 의 `codex_findings if codex_avail else []` 가 원장에서, `build_codex_findings_display`(`:241-242`)가 표시 채널에서 각각 지운다. §7 #6 의 수리(「개수를 advisory 에 포함」)는 **개수만 복원하고 사라진 `severity: high` finding 자체는 복원하지 않는다** | 올바른 수리는 **보존 + 라벨**이다 — 형제 `merge_brief_review.py:175` 가 이미 그 형태(부분적으로 읽히는 지적을 버리지 않는다). 그것은 회계가 아니라 데이터 흐름 변경이라 이 설계 범위 밖이고 별건 원장으로 넘긴다. ✎ **앞선 두 판본이 여기서 연속으로 틀렸다**: ①「§9.1 이 닫는다」— 그 전제가 반증됐다(§9.1 표) ②「기존 verdict 테스트가 이 결함의 **화석**인지 판정한다」— `test_merge_review.py:130-135`(AC10)·`:144-148`·`:154-158` 은 화석이 아니라 **주석에 근거까지 적힌 계약**이다. 남의 테스트를 화석으로 전제하지 말 것 |
...
라운드 2~3 이 반복해서 잡은 실패 모양이 하나다 — **요구를 산문에 적고 집행 자리를 지정하지
않는 것.** 그래서 이 문서의 모든 요구는 아래 표에 «무는 자리»와 «그것만 분리해 무너뜨리는
mutation» 두 칸을 갖는다. **빈 칸은 결함이 아니라 정직한 미집행 선언이다** — 빈 칸을 남기지
않으려고 없는 집행을 적는 것이 이 문서가 세 번 한 실수다.
...
| 앵커 서식 (§4) | 축 A④ | M2 |
| `consumer=` 닫힌 어휘 (§4.1) | 축 A④ | M2 변형(5번째 값) |
...
| 배포 사본 정합성 (§3) | 기존 축 1b·1c + §10.1 (**PR2 부터**) | — (기존 락의 mutation 이 이미 있다) |
