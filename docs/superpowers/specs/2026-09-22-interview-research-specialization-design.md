...
- 연결 표기는 `[RC3 → OQ1]`(레포) · `[RC3 → 없음]`(레포, 닿는 결정 없음) · `[→ OQ1]`(웹) · **`[→ 없음]`**(웹, 닿는 결정 없음) 넷이다.
  레포 주장은 연결 안에 **항상** `RC<n>` 을 싣는다 — 넷째 형식은 최종 리뷰(I-4)가 더했다: 셋만 있으면 결정 없는 레포 주장이
  `[→ 없음]` 으로 쓰여 `RC<n>` 을 잃고 ④⑤ 를 빠져나간다.
- ∀ 는 「넷 중 하나가 줄 끝에 있는가」다. `[→ 없음]` 은 정직한 답이고 red 가 아니다.
...
| # | 검사 | 술어 | 발동 조건 · 0건일 때 |
|---|---|---|---|
| ① | 연결 ∀ | 위 정의의 조사 항목마다 `[RC<n> → OQ<n>]` · `[RC<n> → 없음]` · `[→ OQ<n>]` · **`[→ 없음]`** 중 하나가 줄 끝에 | `contract: v2` 없거나 항목 0건이면 공허 통과 |
