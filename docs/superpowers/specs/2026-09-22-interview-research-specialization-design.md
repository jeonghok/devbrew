...
| | 수 | 무엇 |
|---|---|---|
| 새 필드 | **2** | `decides: [D<n>]`(둘 다) · `id: R<n>`(레포만) |
| 새 state 키 | **2** | `touched_decisions: []` · `blind_spot_dispatches: int`(개명) |
| 새 파일 | **2** | `references/research-claims.md`(계약 정본) · `tests/test_research_claims_contract.sh`(락) |
| 새 게이트 술어 | **5** | 연결 ∀ · 연결 대상 실재 · 역참조 · 이름 있는 derived(조건부) · 확인 줄 ∀ |
| **새 agent** | **0** | 배선으로 채운다 |
...
evidence:                      # 외부(웹) 주장
  - url: "https://..."
    ...
    decides: [D3]              # ★신설 — 이 주장이 닿는 «열린 결정». 빈 배열 허용
repo_claims:                   # 내부(레포) 주장
  - id: R3                     # ★신설 — payload·audit 을 잇는 id
    path: "<repo 상대경로>"
    anchor: "<심볼 | 헤딩 | 원문 인용>"
    line: 123                  # 선택 — 보조 정보
    claim: "<주장>"
...
- `<open_decisions>${OPEN_DECISIONS}</open_decisions>` — 지금 열린 결정 목록(`D<n>` + 한 줄).
...
| 장치 | 변경 | 웹 스위치 |
|---|---|---|
| C43 경로 (a) 자동확인 | `SKILL.md` 의 그 행이 리터럴 마커 대신 **계약(`repo_claims`)을 산출한다**고 적는다. 대량 레포 스윕은 하니스 기본 탑재 read-only 탐색 subagent 에 같은 계약을 인라인해 배선한다 | **무관** — 내부 축의 주 생산자다 |
...
**처분** — consumer=orchestrator · fail-closed(계약 펜스 실패 시 dispatch 중단) · disclosure=loud
advisory + audit §2 의 `unavailable` 사유. **현행 세 dispatch 자리의 처분 줄은 `fail-open` 이므로
셋 다 `fail-closed` 로 고친다**(라운드 1 `96c277ea#r1.1`).

fan-out 은 선언한다(CLAUDE.md 처분 규약). 네 자리 전부 dispatch 당 1이고 §F 의 자격·예산이 총량을
사용자 답에 묶는다.
...
> 레포 주장(`R<n>`)이 ≥1 인 payload 는 audit §1 에 `derived:internal_research` 라는 **이름의** 행이
> 있고 그 행의 상태가 **`closed`** 여야 한다.

`closed` 를 요구하면 `coverage_anchor_failures` 가 그 행의 evidence 에서 실재하는 `S<N>` 앵커를
요구하므로 차원이 **사용자 발화로 닫혔다**는 것까지 기계가 본다. 이름을 요구하면 무관한 derived
행으로 갈음할 수 없다. 레포 주장이 0건이면 이 요구가 발동하지 않으므로 `derived: N/A` sentinel 을
쓰는 기존 픽스처는 red 가 되지 않는다(⟨C10⟩).
...
| **V2** | `finishing.md` Step A | 종료 직전, 무조건 | payload 의 모든 `R<n>` 에 대응하는 확인 줄이 audit 에 있는가 — **누락 대조**만 한다 | **없음** |
...
확인하지 않는다 — Step 2 가 V1 의 특수 경우이고 V2 가 그 산출을 누락 대조한다.

**확인 줄의 거처**(라운드 1 `8891bd29#r1.2`) — audit `## 5. 프로세스 로그`. `AUDIT_SECTIONS` 가
이미 요구하는 절이라 **무조건 존재하고** 새 절을 만들지 않는다(⟨C10⟩ · 압축 규약). 형식:

```
- 확인 R3 — 확인 — plugins/spec-distill/scripts/check_brief.py#coverage_anchor_failures — 주장과 일치
...
- **결정 연결 (§4 · §5 → 결정)** — 조사 항목 줄 **끝**에 `[R3 → D3]`(레포) 또는 `[→ D3]`(웹).
  레포 항목의 `R<n>` 이 audit 확인 줄과 맞물리는 id 이고, 웹 항목은 기존 `«출처키»` 가 그 역할을
  이미 한다(N2 가 audit §7 에 결속).
- **역참조 (§3 → 근거)** — OQ 줄이 그 근거의 **id** 를 포함한다: `→ 근거 R3`. §3 항목은 **그 자체가
  열린 결정**이라 연결의 *대상*이고 출처가 아니다 — 거기에 `[→ D<n>]` 을 걸면 자기지시가 되어
  술어가 공허해진다.
- **요약 상호참조 (§0 → 근거)** — 「한눈에」의 결정 목록도 같은 id 를 쓴다.

**해결된 결정의 보존**(라운드 1 `4ceba241#r1.1`). 조사 당시 열려 있던 `D3` 이 사용자 답으로
해결되면 그 조사는 ⟨C11⟩ 의 성공 사례인데, 열린 목록에서 빠지면 §E ② 의 실재 검사가 red 가 된다.
성공을 red 로 만드는 것은 Goal 의 반전이다. 그래서:

> §0 의 결정 목록은 **해결된 것도 남기고** 상태 토큰을 붙인다 —
> `D3 [해결 ⟨S10⟩] — 면제 상한 → 근거 R3`. 「열림」은 상태 토큰이 말하고 목록에서의 부재가 말하지
...
하류가 보는 것은 「D3 을 정하려면 R3 를 읽어야 한다」다. **하류에 건 지시는 없다** — X2 가 그렇게
...
- payload §5 의 항목 줄 중 **`R<n>` 리터럴을 가진 줄**

§5 의 `기각`·`보류`·`검토`·`위험` 네 모양 중 어느 것인지는 묻지 않는다 — `R<n>` 이 있으면 레포
...
| # | 검사 | 술어 | 발동 조건 · 0건일 때 |
|---|---|---|---|
| ① | 연결 ∀ | 위 정의의 조사 항목마다 `[→ D<n>]` 또는 `[R<n> → D<n>]` 가 줄 끝에 | 항목 0건이면 공허 통과 |
| ② | 연결 대상 실재 | 그 `D<n>` 이 payload §3 또는 §0 의 결정 목록에 실재 | 연결 0건이면 공허 통과 |
| ③ | 역참조 | 연결에 쓰인 `R<n>` 이 그 `D<n>` 줄에 되나타난다 | 같음 |
| ④ | 이름 있는 derived (조건부) | `R<n>` ≥1 이면 audit §1 에 `derived:internal_research` 행이 있고 상태가 `closed` | `R<n>` 0건이면 요구 미발동 — sentinel 통과 |
| ⑤ | 확인 줄 ∀ | payload 의 모든 `R<n>` 마다 audit §5 에 `확인 R<n> — {확인\|반증\|미확인} — …` 줄 | `R<n>` 0건이면 공허 통과 |
| advisory | — | `R<n>` 0건이면 「내부 조사 0건」 한 줄 | — |

③ 이 라운드 1 `e361710c#r1.2` 의 답이다. 원래 AC9 는 「`D<n>` 이 §3·§0 에 실재하는가」만 봤고
그것으로 §3 역참조가 닫힌다고 적었는데 **거짓이었다** — §3 의 `→ 근거 R3` 를 지워도 통과한다.
③ 은 방향을 뒤집어 **`D<n>` 줄이 그 근거 id 를 포함하는가**를 보므로 역참조의 삭제가 red 가 된다.

⑤ 가 `e361710c#r1.1` 의 답이다. 두 파일을 잇는 것은 `R<n>` 이고, 이것은 이 게이트의 기존 교차 술어
...
```yaml
orchestration:
  touched_decisions: []     # ★신설 — 이미 조사가 닿은 결정 id 목록. orchestrator 가 append.
```

- **산출자**: orchestrator. 라운드 안에서 조사 산출을 «지금 이해»에 실을 때 그 산출의 `decides`
  원소를 이 목록에 append 한다. **같은 라운드 안에서 append 가 먼저이고 계수가 나중이다.**
- **소비자**: C44 계수 규칙. 면제 조건:

  > 산출 항목의 `decides` 가 **열린 결정 중 `touched_decisions` 에 없는 것**을 하나라도 담으면
  > `non_user_streak` +0. 그렇지 않으면 +1. 열린 결정이 0이면 면제도 0이다.

- **초기화**: 없다. 인터뷰당 누적이고 재개방으로도 비우지 않는다 — 비우면 면제가 무한해진다.
  단 그 결정이 `[해결 …]` 로 바뀌면 열린 집합에서 빠지므로 면제 후보에서도 빠진다.

그러면 면제 예산이 `|열린 결정 − touched_decisions|` 이 되어 **유한**하다. 이 상한은 열린 결정
...
- `touched_decisions` 는 부재 시 `[]` — 기본값 채우기의 정상 경로다.
...
C43 경로 (a) 자동확인  ──┴────────────────────┘   ← 웹 스위치 무관 · 내부 축 주 생산자
        ...
        V1 — 라운드 규약 안, 무조건 (SKILL.md)
          ...
          {확인 | 반증 | 미확인} → audit §5 프로세스 로그에 `확인 R<n> — …`
          `반증` → §5 에 *원래/재결정/근거* + 그 차원 재개방 가능
                    │                              │
                    │                              └→ 재개방이 §F ② 예산을 늘린다 → 다시 조사
                    ▼
        payload §4 · §5 항목 줄 끝  [R3 → D3] / [→ D3]
        payload §3 OQ               → 근거 R3
        payload §0 결정 목록        D3 [열림|해결 ⟨S10⟩] — … → 근거 R3
        audit  §1 derived:internal_research — closed — ⟨S<N>⟩
        audit  §5 확인 R3 — 확인 — <경로>#<앵커> — …
                    │
                    ▼
        V2 — finishing.md Step A, 무조건 : R<n> 누락 대조
                    │
                    ▼
        V3 — check_brief.py (fail-closed)
          ① 연결 ∀  ② 연결 대상 실재  ③ 역참조  ④ 이름 있는 derived(조건부)  ⑤ 확인 줄 ∀
          R<n> 0건 → advisory → Step B 게이트 question 본문
                    │
                    ▼
...
- **AC1** `references/research-claims.md` 가 실재하고 `evidence[]`·`repo_claims[]` 두 모양의 계약
  **정본**을 담으며 신설 필드 `decides`(둘 다)와 `id: R<n>`(레포만)을 포함한다.
...
- **AC6** `SKILL.md` 라운드 규약에 **V1** 이 있다 — 주장을 «지금 이해»에 싣기 전에 ① 경로 실재
  ② 앵커 실재 ③ 주장이 그 자리와 맞는가 ④ 결과 {확인, 반증, 미확인} ⑤ audit §5 에 `확인 R<n> — …`
  한 줄. **steelman trigger 와 `DEVBREW_SPEC_DISTILL_DISABLE_WEB` 어느 것에도 종속되지 않음**을 명시.
- **AC7** `finishing.md` 에 **V2**(payload 의 `R<n>` 누락 대조)가 있고, V1 판정이 `반증` 일 때
...
- **AC9** 「조사 항목」의 순회 정의가 코드 주석과 이 설계에 같은 문면으로 있다 — §4 의 모든 항목 줄 +
  §5 에서 `R<n>` 리터럴을 가진 줄, §3 제외. 연결 위치는 **줄 끝**이고 하위 불릿은 red 다.
- **AC10** `R<n>` ≥1 이면 audit §1 에 `derived:internal_research` 행이 있고 상태가 `closed` 여야
  한다. **이름이 다른 derived 행이나 `open` 행으로는 만족되지 않는다.** `R<n>` 0건이면
  `derived: N/A` sentinel 로 통과한다.
- **AC11** `R<n>` 0건이면 `gate()` 의 `advisories` 에 「내부 조사 0건」 한 줄이 실리고
  `finishing.md` Step B 가 그 줄을 proceed 게이트 `question` 본문에 싣는다고 적는다.
- **AC12** payload 의 모든 `R<n>` 에 대응하는 `확인 R<n> — {확인|반증|미확인} — …` 줄이 audit §5 에
...
- **AC16** 새 락 `tests/test_research_claims_contract.sh` 가 **다섯 축**을 갖는다 —
  ...
  rc 1 + 빈 stdout) · **D 정합**(정본의 필드 이름 집합 ⊇ steelman-builder 사본의 것, 사본에 정본에
  없는 필드 없음).
...
- **AC19** `templates/interview-brief-template.md` 가 세 방향을 예시로 보인다 — §4·§5 의
  `[R3 → D3]`/`[→ D3]` 줄 끝 연결 · §3 OQ 의 `→ 근거 R3` 역참조 · §0 의
  `D3 [열림|해결 ⟨S10⟩] — … → 근거 R3`. `templates/interview-audit-template.md` 가 §1 의
  `derived:internal_research` 행과 §5 의 `확인 R<n> — …` 줄을 예시로 보인다. 두 템플릿의 예시가 곧
...
- **AC20** D6(검증 축)의 집행 손잡이 — `SKILL.md` 의 coverage-mapper 첫 dispatch 절이, seed 의
  «다시 검증할 것» 문단이 담은 항목마다 **`repo_claims` 를 산출해 V1 을 태울** 의무를 적는다.
  Phase 0(`framing-requests`)은 건드리지 않는다(⟨C4⟩) — 의무는 받는 쪽에 있다.
- **AC21** `## 결정 기록` 의 이 문서 결정은 `X<n>` 접두를 쓰고 리뷰 엔진의 `D<n>.<m>` 과 섞이지
  않는다.
- **AC22** 세 dispatch 자리의 처분 줄이 `fail-closed` 이고, agent 파일 둘의 옛 상한 문구
  (`fan-out 1: 인터뷰당 1회 dispatch(C8)` · `재dispatch 금지 — fan-out 1` · `bounded dispatch: …
  상한 2`)가 자격+예산 서술로 갱신된다.
...
| `plugins/spec-distill/templates/interview-brief-template.md` | 세 방향 예시 (AC19) |
| `plugins/spec-distill/templates/interview-audit-template.md` | §1 `derived:internal_research` · §5 `확인 R<n>` 예시 (AC19) |
| `plugins/spec-distill/scripts/check_brief.py` | 술어 다섯 + advisory + 「조사 항목」 정의 주석 (§E) |
...
7. **픽스처 회귀 0 을 명시 확인** — `derived: N/A` 를 쓰는 기존 픽스처와 `R<n>` 없는 §5 위험 항목이
   AC8·AC10 으로 red 가 되지 않는지 파일 단위로 확인한다(⟨C10⟩ 준수 증거다).
...
- **`R<n>` 번호의 유일성 범위** — 인터뷰 하나 안에서만 유일한지, 코퍼스 전체에서 유일해야 하는지.
...
- **OQ-C** (brief OQ11) 첫 사이클 레포에서의 수익률. **부분 해소** — 조건부 발동이 이 문제의 답이기도
  하다: 건질 것이 없는 레포에서는 `R<n>` 이 0건이고 술어가 공허 통과하며 advisory 만 뜬다. 필수
...
- **L1 조건부 발동이 피검자 산출물에 앵커돼 있다.** `R<n>` 을 0건 내면 ④⑤ 와 advisory 가 발동하지
...
- **L3 「열려 있음」은 검사하지 않는다.** ② 는 `D<n>` 이 §3·§0 의 목록에 **있는가**만 보고 상태
...
| X3 | derived 하한은 **조건부** · `R<n>` 0건은 **advisory** 로만 공시 | 사용자 (OQ2·L1) |
