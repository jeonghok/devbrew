...
- Claude Code의 `AskUserQuestion`은 tool call이므로 응답이 같은 어시스턴트 턴 안의 tool result로 돌아옴 — 별도 턴 boundary를 만들지 않는다. 이게 Stop hook fake-user-message가 불필요해진 핵심 이유.
...
- `hooks/post-tool-use.py`(gh pr create 안내) 및 `session-start-advisor.py`의 코드 레벨 단순화 범위 — 본 spec은 "유지"로 결정했으나 사이드 이펙트 점검과 advisor의 in-flight-pipeline 안내 로직 제거 PR 분리 여부는 plan 단계.
- SKILL이 Gate 2 fix-loop에서 모델이 inline 수정을 시도할 때의 정확한 tool 권한 (orchestrator-as-writer; reviewer agent는 별도 subagent라 Law 2 무결).
- 새 spec test fixture 구성 (Agent dispatch mocking 방식).
- 사용자가 AskUserQuestion 응답으로 "Stop" 선택 시 partial pipeline의 final summary 포맷.
...
- **G5**: `plugin.json`을 v2.0.0으로 bump하고 `CHANGELOG.md`에 Added/Changed/Removed/Breaking을 명시. README의 Hook 표·게이트 흐름·state 다이어그램을 신규 구조로 교체.
...
- **C1**: 본 재설계로 인해 어떤 reviewer agent도 `Write`/`Edit` 권한을 얻으면 안 됨 (Law 2 — Writer ≠ Reviewer). 오케스트레이터(SKILL)는 writer 역할로 분류되며, 모든 review/verify는 별도 subagent 격리 유지.
...
- **C4**: Gate 2 fix-loop의 `max_gate2_iterations=5` cap은 유지. AskUserQuestion이 매 iter 경계에서 fire하지만, 사용자가 5회 연속 "Retry"를 선택해도 5회에서 강제 종료(또는 max-iter용 별도 AskUserQuestion fire).
...
- **AC5**: SKILL의 Gate Execution 섹션이 Gate 1 → Gate 2 → Gate 3을 명시적으로 시리얼 호출하는 의사코드/실제 instruction을 포함 (수동 검수 + grep으로 "Gate 1" / "Gate 2" / "Gate 3" 순서 등장 확인).
- **AC6**: Gate 2 fix-loop가 매 iter 끝에서 `AskUserQuestion` 호출을 지시하며, 3-옵션(`Retry` / `Proceed to Gate 3` / `Stop`)을 명시 (수동 검수).
- **AC7**: Gate 1 FAIL 시 AskUserQuestion 호출 + 3-옵션(`Continue anyway` / `Stop` / `View detail`) 명시 (수동 검수).
- **AC8**: Gate 3 NEEDS_RESOLUTION 시 AskUserQuestion 호출 + 3-옵션(`Yes, retry` / `Skip with evidence` / `Stop`) 명시 + P21 secret-not-in-prompt 정책 재확인 문구 포함 (수동 검수).
...
- **AC12**: README의 "설치된 Hook" 표에서 `stop-hook.py` 행이 제거되고, "파이프라인 흐름" 다이어그램이 단일-턴 시리얼 디스패치로 교체됨. "Principles Instantiated"에 P22 일반화(consent gate as progression primitive) 문구 추가.
...
- **AC14**: 기존 hook 중 다음은 유지: `hooks/post-tool-use-session-tracker.py`, `hooks/post-tool-use.py`, `hooks/session-start-advisor.py`, `hooks/session-end-cleanup.py`. (`session-start-advisor.py`는 in-flight pipeline 감지 분기 단순화 — 자세한 범위는 plan에서).
...
- **AC17**: `/cancel-qg`, `/qg --reset`, `/qg --gc` 명령이 v2.0.0 state file 포맷에서 동작 (orphan 정리 유틸로서의 역할 유지).
- **AC18**: 사용자가 happy path 시나리오(trivia가 아닌 3-게이트 PASS)를 실행했을 때 AskUserQuestion이 단 한 번도 호출되지 않음 — 수동 시뮬레이션 검증.
...
│   ├── session-start-advisor.py                   [MODIFY] in-flight pipeline 감지 분기 제거; legacy state file 1회 안내 emit
...
└── tests/                                         [MODIFY-LARGE]
    ...
    └── test_skill_orchestration.sh                [NEW] Agent dispatch mock 기반 — happy path 0-AskUserQuestion 시나리오, Gate 2 fix-loop AskUserQuestion 호출 검증, Gate 1 FAIL AskUserQuestion 호출 검증, Gate 3 NEEDS_RESOLUTION AskUserQuestion + P21 secret 무주입 검증
```
...
- **V1**: AC1–AC4, AC9, AC10 일괄 검증 — `bash` 스크립트로 grep/test 명령 실행:
  ...
  ```
- **V2**: AC5–AC8 — `skills/quality-pipeline/SKILL.md`를 사람이 읽고 (a) Gate 1 → Gate 2 → Gate 3 시리얼 호출 의사코드 존재, (b) AskUserQuestion 4개 호출 지점(Gate 1 FAIL, Gate 2 iter boundary, Gate 2 max-iter, Gate 3 NEEDS_RESOLUTION) 명시 + 옵션 라벨 정확, (c) P21 secret-not-in-prompt 정책 재확인 문구 존재 확인.
...
- **V4**: AC12 — `plugins/quality-gates/README.md`의 "설치된 Hook" 표에서 `stop-hook.py` 행이 없고(`! grep 'stop-hook.py' plugins/quality-gates/README.md`), "Principles Instantiated"에 P22 일반화 라인 신규 추가.
- **V5**: AC13 — 모든 agent 파일의 frontmatter에 `disallowedTools` 키가 변경 없이 존재:
  ...
  ```
- **V6**: AC15 — `ls plugins/quality-gates/tests/`에서 deleted 패턴이 없고, `test_skill_orchestration.sh`가 존재하며 4개 시나리오(happy path / Gate 2 iter / Gate 1 FAIL / Gate 3 NEEDS_RESOLUTION+P21) 모두 PASS.
- **V7**: AC18 — 수동 dry-run: 작은 PR(50 LOC 미만, trivia 아닌)로 `/qg` 실행 시 어시스턴트 턴 안에서 3 게이트가 PASS하고 AskUserQuestion이 한 번도 호출되지 않으며 single turn에서 final summary 출력되는지 확인.
- **V8**: AC16 — `.claude/quality-gates/<some-old-session>/pipeline.md`에 legacy 키(`consecutive_no_signal: 2`, `current_gate: 2` 등)를 fixture로 두고 SessionStart 시 advisor가 `/cancel-qg` 안내 stderr 출력 emit하는지 검증.
- **V9**: Regression — `tests/test_branch_worktree.sh`, `tests/test_discover_plan.sh`, `tests/test_no_secret_prompts.sh`(있다면), `tests/test_agent_frontmatter_keys.sh` 모두 PASS.
...
- **R5 — Subagent orchestrator 패턴** (parent → long-running subagent가 게이트 시리즈 진행): 서브에이전트 비용 + 가시성 손실(중간 verdict가 사용자에게 안 보임) + AskUserQuestion이 subagent 안에서 native 호출 불가 가능성(미검증). 본 spec의 단순함 우선 원칙(devbrew lightness)과 충돌.
...
- **OQ1**: `skills/quality-pipeline/SKILL.md`의 신규 `allowed-tools` frontmatter에 `AskUserQuestion`을 명시 선언해야 하는지(현재는 deferred tool로 ToolSearch 통해 로드됨). plan 단계에서 확인 — 명시 선언이 일반적인 best practice라면 추가.
- **OQ2**: SKILL이 Gate 2 fix-loop의 "Retry" 선택 시 모델이 직접 `Edit`/`Write`로 fix를 적용할지, 별도 fix-applier subagent로 위임할지. 본 spec은 orchestrator-as-writer 패턴(Law 2 무결 — reviewer 분리만 강제)을 가정하지만, 구현 단계에서 fix 품질/격리에 대한 마찰이 발견되면 재고. plan에서 결정.
- **OQ3**: `pipeline.md` minimal schema의 정확한 필드 — `started_at`, `worktree_path`, `gate2_iteration` 외 GC를 위한 mtime anchor가 별도로 필요한가(파일 자체의 mtime으로 충분한가). plan에서 확정.
