...
| **R1** — 락이 「모듈을 쓰는가」까지만 재고 「출력을 렌더하는가」는 못 잰다 | Task 3~6 의 회귀 테스트가 출력을 직접 보지만, 정적 락으로는 여기까지가 한계다 |
| **R8** — 산문 소비 자리가 실제로 버리는지 미측정 | 앵커가 그 자리들에 처분을 *적게* 만든다. 실제 행동 측정은 skill 실행이 필요하며 이 계획에 없다 |
| 앵커의 `fail-` 과 모듈 `items=` 의 동일성 | **미집행.** 축 B 는 「그 파일이 import 하는가」까지이고 인자로 무엇을 넘겼는지는 안 본다 |
