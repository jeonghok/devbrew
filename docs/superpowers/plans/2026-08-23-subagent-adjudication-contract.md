...
- **락 메시지의 변수 뒤 한글** — `printf "${tot}개"` 처럼 **반드시 중괄호**. `"$tot개"` 는 macOS bash 3.2 가 한글 `개` 의 **선두 바이트**를 변수명에 포함시켜, `set -u` 아래서 존재한 적 없는 변수를 참조해 **죽는다**.
...
> **`${a1_anch}` 의 중괄호는 장식이 아니다.** `"$a1_anch개"` 로 쓰면 macOS bash 3.2 가 한글 `개` 의 **선두 바이트**를 변수명에 포함시켜, `set -u` 아래서 스크립트가 **죽는다**(M8a·M8b 실측).
