...
# 그래서 **본문 고유** 단언을 함께 둔다. 줄머리 blockquote 마커를 먼저 떼고 flatten 하는 이유:
# 그 문장은 인용 블록 안에서 줄바꿈되므로 마커를 남기면 저자가 어디서 접었는지에 따라 리터럴이
# 깨진다(무해한 재줄바꿈이 거짓 RED 가 된다). 실측으로 네 칸을 확인했다 — 원본 O · 본문 반전 X ·
# 한 글자 X · 무해한 재줄바꿈 O.
rhythm_body="$(sed 's/^[[:space:]]*> \\{0,1\\}//' <<<"$rhythm_block" | tr '\\n' ' ' | tr -s ' ')"
grep -qF '계수 **뒤에** 그 원소들의 `touched` 를 `true` 로 올린다' <<<"$rhythm_body" \\
  && ok "AC14: 면제의 순서 본문 — 계수 뒤에 표시한다 (본문 고유)" \\
...
Expected: `✗` **정확히 18건**. 내역 — covmap 자격·자격우선·예산식 3 + covmap 예산식·카운터 1 +
covmap 옛 `상한 2` 부재 1 + covmap 옛 하드 카운트 셋 3 + prober 자격·예산·옛키·옛하드 4 +
C44 면제 **여섯**(순서 라벨 · 순서 **본문** · 대상 집합 · 공집합 경로 · 산출자 · 거처) 6 = 18.
