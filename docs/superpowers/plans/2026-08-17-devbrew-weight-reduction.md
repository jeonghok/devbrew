...
echo "=== [4] 이 스텝 본문이 Step 4c 를 담는가 (기대: 1 이상) ==="
sed -n '/^- \[ \] \*\*Step 7: Task 17이 이 락을/,/^---$/p' "$CORPUS" | grep -c 'Step 4c'
echo "=== [5] 이 스텝 본문이 Step 5 를 담는가 (기대: 1 이상) ==="
sed -n '/^- \[ \] \*\*Step 7: Task 17이 이 락을/,/^---$/p' "$CORPUS" | grep -c 'Step 5'
echo "=== [6] 백스톱 — 커밋 스텝을 가리키는 죽은 참조 (기대: 0) ==="
grep -c 'Task 16 Step 6' "$CORPUS"
rm -f "$CORPUS"
```

Expected: `[0]` 세 줄이 각 기대대로 · `[1]`·`[2]`·`[3]` 이 **1** · `[4]`·`[5]` 가 **1 이상** · `[6]` 이 **0**.

하나라도 어긋나면 세 자리 중 하나가 드리프트한 것이다 — **문서를 고치기 전에 어느 쪽이 참인지부터 정한다.** 어느 줄이 어디를 지키는지: `[0]` 은 이 검사가 자기를 세고 있지 않다는 증거(여기가 깨지면 아래 전부가 무의미하다) · `[1]` 은 권위 있는 자리 자체 · `[2]`·`[3]` 은 나머지 두 자리가 **역할 문장으로** 그 이름을 부르는지 · `[4]`·`[5]` 는 그 스텝이 **두 호출부를 다** 갚는지 · `[6]` 은 커밋 스텝을 가리키는 죽은 참조가 새로 생겼는지.
...
>
> 그러므로 census 시점의 읽는 법(*"불일치 → 그 행의 조치를 다시 읽는다"*)을 그대로 적용하면 **설계대로 일어난 변화를 결함으로 신고**하게 되고, 실행자는 사이클이 성공했다는 바로 그 증거를 보고 수십 행의 조치를 되읽는다. **그래서 모집단을 셋으로 가르고, 가운데 것은 읽는 방향을 뒤집는다.** 어느 행이 어느 버킷인지는 원장의 **위치란과 조치란만 보면** 정해지므로 여기에 목록을 복제하지 않는다(수는 한 곳에 산다):
>
