...
#    **이것은 오늘 열려 있는 결함이 아니라 잠재 결함이다** — `shared/` 아래에 마크다운
#    정본이 하나도 없고, 사본으로 배포되는 마크다운도 없다(2026-08-19 실측). 이 태스크의
#    findings 는 그런 배포가 이미 있다고 적었는데 그 파일은 한 플러그인의 `scripts/` 한
#    곳에만 있고 `shared/` 정본이 없다 — findings 저자가 확인해 준 착오다. 좁힘 자체는
#    **값을 바꾸지 않는다**(`n_sb`·`n_cand` 불변, 실측) — 값이 안 변한다는 것을 먼저 재고
#    좁혔다. 남는 위험은 basename 우연 충돌(`README.md` 류)인데, 그것은 **좁히는 방향**의
#    실패라 거짓 RED 로 시끄럽다.
