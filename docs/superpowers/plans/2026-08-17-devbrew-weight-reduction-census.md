...
|---|---|---|---|---|---|---|---|
| 27 | `setUp` (py) | 77/51 | ◐ | (다수, 무작위 표본 3건 실측: `test_arm_ledger.py`·`test_qg_gc.py`·`test_hook_output_schema.py`) | 우연 | 조치 없음 | 〔seed〕 unittest API 컨벤션 — 표본 3건 실측 확인: repo fixture 생성/tempdir 생성/temp-repo 생성으로 각기 다름. 부분 동일(27곳)은 표준 boilerplate(`tempfile.mkdtemp()` 등)의 우연 중첩, 책임 공유 아님 |
...
| 34 | `tearDown` (py) | 15/10 | ◐ | (다수, 무작위 표본 3건 실측: `test_session_end_cleanup.py`·`test_run_audit_codex_reviewer.py`·`test_arm_ledger.py`) | 우연 | 조치 없음 | 〔seed〕 unittest 컨벤션, `setUp`과 동일 이유 — 표본 3건 실측: `shutil.rmtree(self.tmp)` / `self.td.cleanup()` / `os.chdir`+`shutil.rmtree(self.repo)`로 각기 다름 |
...
| 54 | `cleanup` (sh) | 3/3 | ⚠ | agent-transparency `tests/ab_gate.sh`, quality-gates `test_resolve_baseline.sh`·`test_qa_ledger.sh` | 우연 | 조치 없음 | 실측: qg 2곳은 `cleanup() { rm -rf "$TARGET"; }` 형태의 1줄 관용구지만 대상 변수가 다름(REPO/TMP) — 너무 사소해 추출 가치 없음 |
...
| 66 | `read_records` (py) | 2/2 | ⚠ | agent-transparency `tests/ab_judge.py`, `scripts/prepare_standup.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: `ab_judge.py`의 docstring이 `prepare_standup.py`의 같은 이름 함수를 **명시적으로 지칭**하며 "같은 형식에 대해 정반대 결정을 내린다"고 적음 — 둘 다 JSONL 레코드를 읽고 파싱 실패 줄 수를 함께 반환하는 같은 shape이되, 손상 줄 처리 정책만 의도적으로 분기. 우연이 아니라 문서화된 분기 — 공통 리더 + 정책 파라미터로 추출 가능 |
...
| 74 | `read` (py) | 2/2 | ⚠ | agent-transparency `tests/test_plugin_contract.py`, plugin-audit `scripts/tests/test_agents_generic.py` | 진짜 사본 | shared/ 정본 + copy-of (Task 15·17·18·19) | 실측: 양쪽 다 `(BASE_DIR / x).read_text(encoding="utf-8")` 한 줄짜리 — 파라미터명(`rel`/`name`)과 베이스 디렉터리 상수(`PLUGIN_DIR`/`AGENTS`)만 다르고 베이스 디렉터리를 인자로 빼면 바이트 동일. 〔seed는 범용 이름으로 우연 처리했으나 실측 결과 뒤집음〕 |
...
| 76 | `section_of` (py) | 2/2 | ⚠ | agent-transparency `tests/test_plugin_contract.py`, `tests/test_readability_parity.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 둘 다 "`## <heading>` 부터 다음 `## ` 까지" 같은 docstring, 같은 알고리즘. 게다가 **`test_plugin_contract.py`의 이 본문은 #70 `section`(agent-transparency `test_ab_runner_contract.py`)과 변수명까지 사실상 동일** — 이름 기반 그룹핑(funcs.py)이 놓친 3-way 교차이름 중복(§discrepancy 참조). `test_readability_parity.py` 쪽만 `.find()`+가드로 에러 처리가 다름 |
...
| 78 | `git` (py) | 2/2 | ⚠ | agent-transparency `tests/test_prepare_standup.py`, plugin-audit `scripts/tests/test_check_integrity.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 둘 다 `subprocess.run(["git", ...], cwd=..., check=True, ...)` 얇은 래퍼 — 인자 순서(`git(cwd, *a)` vs `git(*args, cwd)`)와 출력 억제 방식(`capture_output=True` vs `stdout/stderr=DEVNULL`)만 다름. 판단 갈림, 무거운 쪽 |
...
| 82 | `_read` (py) | 2/2 | ⚠ | plugin-audit `scripts/check-shape-completeness.py`, spec-distill `scripts/brief_review_state.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 같은 목적(UTF-8 파일 읽기)이나 에러 계약이 다름 — plugin-audit는 3종 예외를 흡수해 `None` 반환(fail-soft), spec-distill은 무가드 1줄(fail-loud). 판단 갈림, 무거운 쪽 |
...
| 96 | `_verbose` (py) | 2/2 | ⚠ | qg-gc.py↔spec-distill-gc.py | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 〔seed〕 `qg-gc.py ↔ spec-distill-gc.py` 부분 사본의 구성 함수 — gc 패밀리(#95/97/98/99/116/136)와 동일 파일쌍 |
...
| 101 | `_norm_sev` (py) | 2/2 | ⚠ | quality-gates `scripts/synthesize_findings.py`, `scripts/synthesize_artifact_findings.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 둘 다 같은 fail-closed 정책(어휘 밖 severity는 SUGGESTION이 아니라 CRITICAL로 승격)을 문서화 — 같은 플러그인 내 두 렌더러가 같은 정책을 각자 구현 |
| 102 | `assert_not_grep` (sh) | 2/2 | ⚠ | — | 부분 사본 | Task 14 | 〔seed〕 assert_* 패밀리 |
| 103 | `write_agent` (sh) | 2/2 | ⚠ | quality-gates `test_agent_tools_lock_mutation.sh`, `test_agent_tools_lock_differential.sh` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 둘 다 `printf -- '---\nname: probe\ndescription: fixture\nmodel: inherit\n...\n---\n\nbody\n'`로 같은 probe-agent frontmatter를 씀 — 대상 경로와 printf 포맷(`%b`/`%s`)만 다름. `bad`/`expect`/`verdict`(#104/105/107)와 같은 mutation-lock 하네스 계열 |
...
| 121 | `parse_iso` (py) | 2/2 | ⚠ | spec-distill `hooks/review-dispatch.py`, `hooks/pending-review-reminder.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | 실측: 축4 #149(블록축, 두 훅이 8개 블록 공유)와 동일 파일쌍의 구성 함수 |
...
**없음 — 0건.** 분류표 150행 중 진짜 사본 27행 + 부분 사본 73행 = 100행 전부가 조치 열에 shared/ 통합(Task 15·17·18·19) · 공통 조각 추출(Task 20·21) · Task 14 · Task 22 · 기존 통합에 흡수 중 하나를 명시하고 있다 — "조치 없음"이 찍힌 행은 전부 우연(47행)·템플릿-인스턴스(3행, 재적용+동일성 검사가 조치)뿐이다. 기계적 확인: 진짜/부분 사본 행을 뽑아 조치 열에 "조치 없음"이 하나도 없음을 파이썬 스크립트로 grep-검증(fix round 1에서 재실행, BAD ROWS: [] 확인).
