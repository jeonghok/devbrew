...
| 66 | `read_records` (py) | 2/2 | ⚠ | agent-transparency `tests/ab_judge.py`, `scripts/prepare_standup.py` | 부분 사본 | 공통 조각만 추출 (Task 20·21) | **정정(fix round 2)**: 이전 판이 "파싱 실패 줄 수를 함께 반환하는 같은 shape"이라 적었는데 반환 계약은 실제로 다르다 — `ab_judge.py`는 `records`(list)만 반환하고 손상 줄 수는 지역 변수 `broken`으로만 세어 stderr 경고 후 버림(OSError 가드 없음), `prepare_standup.py`는 `(records, unparsed, found)` 3-tuple을 반환하고 `except OSError: return [], 0, False` 가드가 있음. 실제로 같은 것은 **알고리즘 골격**(JSONL을 줄 단위로 순회, `json.loads` 실패를 카운트)과 `ab_judge.py`의 docstring이 `prepare_standup.py`를 명시적으로 지칭하며 "같은 형식에 대해 정반대 결정을 내린다"고 서술한 **문서화된 형제 관계**다 — 분류(부분 사본)는 이 문서화된 관계 때문에 유지, "같은 shape" 서술만 삭제 |
...
| 101 | `_norm_sev` (py) | 2/2 | ⚠ | quality-gates `synthesize_findings.py:365`, `synthesize_artifact_findings.py:49` | 부분 사본 | 공통 조각(시그니처·어휘 셋 검사) 추출은 가능하나 **fallback 방향은 절대 통합하지 말 것** — §15.1 / Task 28 Step 7 참조 | **정정(fix round 2)**: 이전 판이 "같은 fail-closed 정책"이라 적었는데 코드와 반대다. 실측: `synthesize_findings.py:365`는 미지 severity를 **SUGGESTION**으로 강등(fail-open, 주석 "Normalize to SUGGESTION… so counts == rows"), `synthesize_artifact_findings.py:49`는 **CRITICAL**로 승격(fail-closed, 주석 "Treat anything off-vocab as CRITICAL"). 두 함수는 시그니처·어휘-셋 검사 구조는 근접하지만 **동일 시나리오에 정반대 fallback 방향**을 갖는, plan §15.1/Task 28 Step 7에 이미 "머지 차단 동작을 바꾸는 별개 결정"으로 out-of-scope 기록된 **의도적** 분기다. Task 20·21이 이 행을 집더라도 fallback 분기(CRITICAL/SUGGESTION)는 절대 통합하지 말 것 — 우연으로 낮추면 이 경고 자체가 표에서 사라져 무조건 조치-없음으로 스킵될 위험이 있어, 부분 사본으로 유지하되 조치란에 금지 문구를 명시하는 쪽을 택했다 |
