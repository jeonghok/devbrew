...
>
> **축 1·2·4(31행)에는 위치란이 아예 없다** — 그 행들은 **5열**(`# · 후보 · 분류 · 조치 · 근거`)이고 후보란 자체가 파일 경로다(`detect_codex.sh ×3`). 축 3의 8열에만 위치란이 있다(행 모양 분포 실측: `{5: 31, 8: 119}`). **틀릴 칸이 없으므로** 축 3과 같은 재도출을 하지 않았다 — 이 판단을 적어 두는 이유는 "안 했다"와 "필요 없다"를 구별하기 위해서다.
>
> 이쪽의 대응 위험은 다른 것이다: **`×N` 의 N 이 틀리는 것.** 31행 전량의 N 을 확인했고 전부 맞다. 그리고 **배정된 행은 태스크 Files 대조가 한 겹 더 받지만, 유예된 행은 그것이 없다** — 축 1·2·4 중 유예는 **#13 하나뿐**이라 그 두 파일(`plugins/quality-gates/tests/test_session_end_cleanup.py` · `plugins/spec-distill/tests/test_session_end_cleanup.py`)은 **존재를 직접 확인**했다. 〔앞 판이 "그 경로들은 각 태스크 Files 와의 대조에서 이미 확인됐다"고 적었는데, 유예 행에는 태스크 Files 가 없으므로 #13에는 그 논거가 성립하지 않는다 — 정정.〕
>
...
>
> **재검토가 한 일**: 100행 각각에 대해 조치란이 지목한 태스크의 **Files 블록과 스텝 본문을 열어** covered / 오귀속 / 절반 커버 / 미커버로 판정하고, 오귀속은 실제로 하는 태스크로 고치고, 절반 커버는 그 태스크의 스텝을 확장하고(Task 14 Step 4b · Task 17 Step 4b · Task 20 Step 3b · Task 21 Step 2b · Task 22 Step 2b·2c · Task 19의 `_disabled` 7곳), 나머지는 아래 규칙으로 **명시 유예**했다.
