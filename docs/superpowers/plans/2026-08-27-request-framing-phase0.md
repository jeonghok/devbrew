...
- **이 계획서의 사실 주장은 전부 대상 파일에서 재확인한다 — 세 축.** 계획(과 그 출처인 설계)이 쓴 것 중 **본문만 정본**이고, 아래 셋은 넣기 전에 대상 파일을 열어 확인한다. 계획과 파일이 어긋나면 **파일이 이긴다** — 그리고 그 사실을 보고서에 적는다.
  - **① 서식** — 이웃 항목의 모양(헤딩 레벨 · 필드 순서 · 포인터 표기)을 읽고 따른다. *실측 근거: Task 1 이 fix 라운드 하나를 태웠다 — 설계 §6.1 이 philosophy 항목 서식을 재보지 않고 blockquote 로 단정했고 실제 관례는 `### P## — Title` 헤딩이었다.*
  - **② 경로의 실재** — 계획이 서술하는 제어 흐름이 그 파일에 **실제로 존재하는가.** 특히 결정론 표·라우팅 표가 있는 파일에서는 그 표를 읽고 계획의 문장이 도달 가능한 상태를 말하는지 본다. *실측 근거: Task 2 가 fix 라운드 하나를 태웠다 — 계획이 `needs_revise` 를 Phase 5 로 보낸다고 썼으나 라우팅 표는 `rereview_count < 5` 를 author 회귀로 보내고 Phase 5 에 도달시키지 않는다.*
  - **③ 시제** — 계획이 「~를 재는 락이 있다」고 쓴 것이 **오늘 참인가.** 뒤 태스크가 만들 락을 현재형으로 서술하면 그 문장은 착지 전까지 거짓이다.
...
- **★ 변이 집합은 «축 목록»이 아니라 «그 단언이 주장하는 것»에서 도출한다.** 고정된 네 축(삭제·값·표기·반전)을 훑는 것으로는 부족하다 — 실측으로 **세 번** 같은 결함이 그 목록을 빠져나갔다(Task 4 의 `AND`, Task 5 의 부정문·문단분할, Task 7 의 산술 부호). 절차는 이렇다:
  1. **그 단언이 무엇을 묶는다고 주장하는지 한 문장으로 쓴다.**
  2. 그 문장에서 **묶인 요소를 전부 열거한다** — 각 토큰 · 각 연산자 · 각 피연산자 순서 · 각 조건절 · 각 인접성(같은 줄/같은 문장/같은 단락).
  3. **요소마다 하나씩**, 나머지를 고정한 채 바꾼다. 특히 **단어는 한 글자도 안 바꾸고 연산자만 뒤집는** 변이를 반드시 포함한다 — 리터럴 검사는 그것을 절대 못 잡고, 그것이 실제 드리프트의 모양이다.
  4. 반대 방향으로 **의미보존 변이**(rewrap · 문장 순서 · 문단 분할)를 최소 둘 돌려 GREEN 을 확인한다.
  단언이 **공식**(산술·비교·조건)을 묶는다고 주장하면 그 공식의 연산자와 피연산자 순서가 묶임의 일부다. 단어만 검사하는 단언은 그 주장을 못 지킨다.
- **「반전」 축은 연산자 하나가 아니다.** 조건식을 흔들 때 **비교 연산자**(`!=` → `==`)만 뒤집고 멈추면 **논리 연결사**(`AND` → `OR`)가 통째로 남는다. 게이트를 실제로 무력화하는 쪽은 후자인데도 락이 GREEN 을 유지한다. *실측 근거: Task 4 가 `!=` 반전은 잡았지만, 같은 조건식의 `AND` 를 `OR` 로 바꾸고 피연산자 순서를 뒤집은 변이는 두 단언 모두 통과했다 — 그 형태에서는 새 에피소드 하나만으로 재dispatch 가 나서 밀도 게이트가 사라진다.* 조건식의 **모든 연산자**를 축으로 세고, **각 conjunct 를 하나씩 삭제**하는 축도 함께 돌린다.
  **대칭 축(양성 방향) — 의미를 바꾸지 않는 재배치는 GREEN 이어야 한다.** 줄바꿈 위치를 옮기는 rewrap 같은 무해한 편집에 거짓 RED 가 나면 그 락은 다음 편집자를 막는다. 관계를 조이는 것과 배치에 관대한 것은 **함께** 만족시킨다 — 한쪽을 다른 쪽과 맞바꾸지 않는다.
- **변이 전에 커밋한다 — 라운드마다가 아니라 «매 변이마다».** `git checkout -- <path>` 는 «마지막 변이»가 아니라 HEAD 로 되돌린다. 커밋하지 않은 채 변이하면 복원이 그 앞의 편집까지 지우고, 복원 후 clean tree 가 성공처럼 보인다. **실측: 이 계획에서 같은 풋건이 두 번 터졌고 둘 다 같은 모양이었다** — baseline 커밋 → 변이 → 복원 → *수정(미커밋)* → 다음 변이 → 복원 → **그 수정이 사라짐.** 라운드 첫머리에 한 번 커밋하는 것으로는 못 막는다. **`git checkout --` 직전에 `git status --porcelain` 이 비어 있는지 확인**하고, 비어 있지 않으면 먼저 커밋한다. 값싸고 이 실패 전체를 덮는다. **이미 clean tree 면** `git add -A && git commit` 이 «nothing to commit» 으로 비-zero 를 내는데, 그건 실패가 아니라 *HEAD 가 이미 baseline* 이라는 뜻이다 — 멈추지 말고 그대로 변이로 진행한다. *실측 근거: Task 3 의 브리프는 미커밋 상태를 전제한 stash 레시피를 담았고, 앞 태스크가 이미 커밋돼 있어 그 스텝이 literal no-op 이었다.*
...
`P22` 항목 바로 아래, **그 파일의 실제 서식으로.**

> **정정 (Task 1 실행 중 실측)** — 이 계획의 첫 판본은 blockquote(`> **P23 …**`) 형식을 적었다. **틀렸다.** 파일의 실제 관례는 `### P## — Title` 헤딩 + 굵은 집행 절 + `Load-bearing:` 절 + `코드: <경로>` 포인터 줄이다(`P17`·`P18`·`P21`·`P22` 전수 확인). **서식을 먼저 재고 본문을 넣어라** — 형식 단정은 산문이라 틀려도 소리가 안 난다.
...
**`코드:` 포인터 줄은 이 태스크에서 달지 않는다** — 이웃 항목은 전부 갖고 있지만 P23 의 집행 표면(`proceed-gate.md` 의 「재결정 규약」 절)이 Task 2 에서야 생긴다. 없는 파일을 가리키는 포인터를 미리 쓰는 것은 거짓 인용이다. **Task 2 가 그 줄을 단다.**
...
- Produces: `docs/philosophy/…` 의 P23 항목에 붙일 `코드:` 포인터 대상(이 태스크가 만드는 절)

> **Task 1 이 남긴 것 — 이 태스크가 닫는다.** philosophy 의 P23 항목에는 `코드:` 포인터 줄이 **없다**. 이웃 항목(`P17`·`P18`·`P21`·`P22`)은 전부 갖고 있고, Task 1 이 안 단 것은 집행 표면이 이 태스크에서야 생기기 때문이다. **이 태스크의 마지막 편집으로 그 줄을 단다:**
>
> ```markdown
> 코드: `plugins/spec-distill/references/proceed-gate.md` 「재결정 규약」 절 · `skills/reviewing-spec/SKILL.md`
> ```
>
> 달지 않으면 P23 만 이웃과 다른 모양으로 남고, 「이 원칙은 어디서 집행되나」에 답이 없다.
...
네 축으로 흔든다(삭제 · 값 · 표기 · 반전). **「반전」의 범위는 §Global Constraints 를 따른다** — 비교 연산자 하나로 끝내지 않는다. **먼저 커밋한다** — `git checkout --` 는 HEAD 로 되돌리므로 미커밋 편집이 함께 사라진다.
