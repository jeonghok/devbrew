...
### 34. **선행기술 조사의 산출물은 "훔칠 것"이 아니라 "안 훔칠 이유"였다**
...
- **가장 값진 답은 "없음"이다.** 멈춤 조건("실행을 깨뜨리거나·사용자를 해치거나·거짓 결과를 내는 것만 고친다")이 있었기에 **스펙 변경 0건**으로 끝났다. 조건이 없었다면 이 라운드는 **r13**이 됐을 것이다 (원장 29).
...
### ✅ 2.5. ~~레퍼런스 코퍼스 라운드~~ — **DONE. 스펙 변경 0건** (2026-07-13)

사용자 지시로 `~/Downloads/reference/`(9개 리포)에 **선행기술 조사 라운드**를 돌렸다. **다시 돌리지 마라 — 결론은 원장 31·33·34에 있다.**

- **채택 0건.** 후보 3건 전부 기각 (이미 있음 / 대상에 없는 클래스 / 우리가 방금 반증한 수치 스코어링). → 원장 34
...
- **선행기술은 전부 결정론 채점기다** — 우리가 판단이 필요하다고 본 3축(훅 안전성·컴포넌트 격리·철학 준수)에 **아무도 답을 갖고 있지 않다.** 조사자가 REFUTE를 최대한 시도한 뒤 인정했다.
