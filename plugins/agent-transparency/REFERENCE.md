...
> 각각 이 절과 같은 레벨의 독립된 헤딩이다 — 셋이 아니라 정확히 둘인 해시
> 뒤에 이름이 곧바로 오는 형태를 절 경계로 쓴다(더 깊은 레벨로 중첩된
> 헤딩은 경계로 인식되지 않는다). 위 JSON 포맷 접두 줄도 이 절에만 있고
> 네 루브릭 절 본문에는 반복되지 않는다 — 소비자는 이 절을 부모로 자동
> 상속하지 못하니 별도로 읽어 프롬프트 앞에 붙여야 한다.
