...
# max_rounds clamp
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS= mr)" = "5" ] && ok "default 5" || no "default should be 5"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=3 mr)" = "3" ] && ok "env 3 honored" || no "env 3"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=99 mr)" = "10" ] && ok "clamp >10 to 10" || no "clamp high"
...
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=10 mr)" = "10" ] && ok "boundary 10 -> 10 (at cap)" || no "boundary 10"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=11 mr)" = "10" ] && ok "boundary 11 -> 10 (just over cap)" || no "boundary 11"
...
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=999999999999999999999999999999 mr)" = "10" ] && ok "overflow (30-digit) clamps to 10" || no "overflow must clamp (F-F)"
# F-F: leading-zero values must be base-10 normalized, not octal/raw passthrough.
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=010 mr)" = "10" ] && ok "leading-zero 010 -> 10 (not octal 8)" || no "010 normalize (F-F)"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=08 mr)" = "8" ] && ok "leading-zero 08 -> 8 (not invalid-octal error)" || no "08 normalize (F-F)"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=0 mr)" = "0" ] && ok "0 allowed (floor)" || no "0 floor"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=abc mr)" = "5" ] && ok "non-integer -> default 5" || no "non-integer default"
[ "$(DEVBREW_QUALITY_GATES_CRITIQUE_MAX_ROUNDS=-4 mr)" = "5" ] && ok "negative -> default 5" || no "negative default"
