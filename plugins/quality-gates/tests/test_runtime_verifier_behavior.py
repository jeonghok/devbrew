...
class RuntimeVerifierBehaviorTests(unittest.TestCase):
    def test_AC45_runtime_verifier_verdict_enum(self):
    ...
    def test_AC46_runtime_verifier_missing_key_raises(self):
        bad = "verdict: PASS\n"
        parsed = run_agent_stub("runtime-verifier", "p", bad)
        with self.assertRaises(AssertionError):
            assert_yaml_schema(parsed, ["verdict", "evidence_log"])

    def test_AC47_runtime_verifier_invalid_yaml_raises(self):
        with self.assertRaises(AssertionError):
            run_agent_stub("runtime-verifier", "p", "verdict: : : :")

    def test_v220_runtime_verifier_functional_assertions_schema(self):
        ...
        self.assertTrue(isinstance(fa, list) and len(fa) >= 1,
                        "functional_assertions must be a non-empty list")
        entry = fa[0]
        for k in ("ac_id", "flow", "expected", "observed", "evidence_refs", "verdict"):
            self.assertIn(k, entry, f"functional_assertions entry missing '{k}'")
