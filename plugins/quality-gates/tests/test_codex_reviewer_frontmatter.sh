...
# AC9 Layer 2 / AC11 — allowed-tools is the narrow whitelist
allowed = data.get("allowed-tools") or []
# Required entries
required_allowed = ["Bash(codex exec*)", "Bash(python3 *)"]
for tool in required_allowed:
    assert_eq(tool in allowed, f"{tool} present in allowed-tools")

# Banned entries — must NOT be in allowed-tools
banned_in_allowed = ["Bash(cat *)", "Bash(echo *)", "Bash", "Write", "Edit", "MultiEdit"]
for tool in banned_in_allowed:
    assert_eq(tool not in allowed, f"{tool} NOT in allowed-tools (would weaken isolation)")
