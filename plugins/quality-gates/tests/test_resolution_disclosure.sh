...
case_per_adapter_present() {
  ...
  # **비용 — 이 엄격함은 공짜가 아니다.** 이 문장을 뜻은 그대로 두고 무해하게
  # 다듬기만 해도(조사·띄어쓰기·어순 등) 이 단언은 깨진다 — 그 실패는 회귀가
  # 아니라 「고쳐 써서 그렇다」는 기대된 결과다. 다음에 이 줄이 RED 면 먼저
  # `diff-test-results.py` 의 실제 문구를 읽고, 문구가 의미상 그대로인데 리터럴만
  # 바뀌었으면 이 단언의 기대 문자열을 그 문구로 갱신하면 된다(로직 결함이 아니다).
