#!/usr/bin/env bash
# reconstruct-skill.sh — quality-pipeline SKILL.md 를 references/differential-test.md
# 포인터 자리에 그 파일 본문을 되접어, 분할 전과 논리적으로 동일한 단일 문서로
# 재구성한다. `source`해서 `reconstruct_skill_md` 함수를 쓴다.
#
# 왜 필요한가 (Task 31, 무게 감축): `## Differential test` 절차 전문이
# skills/quality-pipeline/references/differential-test.md 로 옮겨지고 SKILL.md 에는
...
# 분석해 차등 테스트 절차(Step R-init..R8)의 순서·근접성·본문을 검증한다 —
...
#     echo "FAIL: SKILL.md ↔ references/differential-test.md 재구성 실패"; exit 1
#   fi
#   trap 'rm -f "$SKILL_MD"' EXIT
#
# **실패는 침묵하지 않는다.** 포인터(`## Differential test` 헤딩)를 못 찾거나 참조
...
reconstruct_skill_md() {   # $1 = SKILL.md 절대경로
  ...
  if awk -v skill_dir="$skill_dir" '
    ...
    /^## Differential test$/ && !spliced {
      spliced = 1
      ref = skill_dir "/references/differential-test.md"
      n = 0
      while ((getline line < ref) > 0) { print line; n++ }
      close(ref)
      if (n == 0) {
        print "reconstruct-skill.sh: references/differential-test.md 부재 또는 빈 파일 (" ref ")" > "/dev/stderr"
        exit 3
      }
    ...
    END {
      if (!spliced) {
        print "reconstruct-skill.sh: SKILL.md 에 \"## Differential test\" 포인터 헤딩이 없다 — 재구성 대상을 못 찾음" > "/dev/stderr"
        exit 3
      }
    }
  ' "$skill" > "$out"; then
    ...
    #
    # 위 awk 는 **하드코딩된 한 섹션**(`## Differential test`)만 되접고, 그 헤딩이 없을 때만
