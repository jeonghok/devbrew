...
#   사본이 이 파일 안에 두 곳(리팩터 전) 있었다**는 것뿐이다. 그래서 이
...
# 위 정규화 문단의 계약("실패하기 시작하면 해시가 바뀐다")은 이 absorbing
# state 안에서는 거짓이 되기 때문이다.
...
fingerprint() {   # $1 = 테스트 경로 → 정규화된 실패 지문
  # L3: 아래 `case "$t" in` 실행 루프는 확장자로 python3/bash를 가르는데 이
