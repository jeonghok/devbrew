...
3. **The feature is purely additive + opt-in.** When `DEVBREW_DISABLE_QG_CODEX=1` or codex is not installed, no new code paths execute. The structural checks above provide sufficient regression evidence without the harness investment.
...
5. After the feature lands, re-run `/qg` with `DEVBREW_DISABLE_QG_CODEX=1` on the same 3 PRs and diff against fixtures. Expect identity (modulo normalize).
