...
- **엔진 링크 셋이 PR 3 최종 리뷰 수정과 qg iter 3 으로 더 바뀌었다(`scripts/docreview_state.py` · `scripts/docreview_route.py` · `scripts/docreview_anchor.py`).** 얼림 검사와 permit · fix 적용 관측을 엔진이 원장의 라운드별 스냅숏으로 계산한다 — `observe-diff` · `finalize` 가 `--diff` 를 받지 않고, 관측을 건너뛴 라운드의 permit 을 뒤 라운드가 그 라운드의 스냅숏 쌍으로 따라잡으며, 필요한 스냅숏이 원장에 없으면 rc 1 `snapshot_missing` 이다(`diff_snapshots` 는 `docreview_anchor.py` 에서 `docreview_state.py` 로 옮겼다 — `diff` CLI 출력은 그대로). 비-UTF-8 critic 출력을 `prepare-recritic` 이 rc 1 `unreadable` 대신 critic 사망(rc 4 · 사유 `layer1 block undecodable`)으로 친다(설계 §9). 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
