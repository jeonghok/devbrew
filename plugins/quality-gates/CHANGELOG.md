...
- **엔진 링크 두 개가 PR 3 최종 리뷰 수정으로 더 바뀌었다(`scripts/docreview_state.py` · `scripts/docreview_route.py`).** 새 서브커맨드 `prev-snapshot`(7단계 얼림 검사의 직전 라운드 스냅숏을 원장에서 낸다) · 라운드 ≥ 2 에서 `--diff` 없이(또는 읽을 수 없는 `--diff` 로) `finalize` 하면 보고서 `advisory[]` 에 「얼림 검사 없음 — diff 미제공(라운드 n)」(공시이지 차단이 아니다). 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
