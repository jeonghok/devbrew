...
- **엔진 링크 `scripts/docreview_route.py` 가 PR 3 qg iter 1 로 한 번 더 바뀌었다(`scripts/docreview_state.py` 는 주석 한 줄).** `prepare-recritic` 이 critic 출력에도 codex 와 같은 시점 판별을 한다 — 라운드 시작 표식보다 먼저(또는 같은 시각에) 쓰인 critic 파일은 critic 사망(rc 4 · 사유 `layer1 block critic_predates_round`). 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **엔진 링크 `scripts/docreview_route.py` 가 PR 3 qg iter 2 로 한 번 더 바뀌었다.** 라운드 시작 표식이 없거나 정수가 아니면 `prepare-recritic` 이 critic 은 살린 채 「critic 시점 판별 불가 (<사유>)」를 공시한다. 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
