...
- **R8 원장(`runtime-evidence.md`)의 floor 5차원 중 하나라도 `degraded` 이거나 unclaimed 단위가 있으면** 사유 `silent-drop` 으로 `not-certified` 다 — `check_qa_ledger.py` 는 이 경우도 exit 0 을 낼 수 있어(원장 내부 일관성만 본다) 이 배선이 없으면 정직하게 degraded 를 적은 실행이 `clean` 으로 샌다.
...
- `scripts/runner_common.sh`(공용 사본)의 남은 두 주석 — 통일 범위의 러너 수(셋 → 둘)와, 이제 호출자가 없는 `codex_extract_or_fallback` 이 그렇다는 사실. 동작 변경 없음.
...
- **문서 리뷰 엔진 `shared/docreview/` 이 바뀌었다 — 이 플러그인은 `scripts/{docreview_state,docreview_route}.py` · `scripts/run_docreview_codex_reviewer.sh` 심볼릭 링크로 함께 배송한다(cache key).** 내용 전문은 `plugins/spec-distill/CHANGELOG.md` `[3.1.0]` 이다. minor 인 이유 — 링크로 받는 엔진 표면이 넓어졌다: 게이트 요약의 새 필드 넷(`unverified` · `approval_label` · `round_reviewed` · `unreviewed_reason`) · 새 서브커맨드 `state-dir-for` · `finalize` 거부 시 원장 표지(`rounds[n].finalize_failed`) · 러너의 새 fail-closed 사유(`profile_parse_ambiguous` · `profile_field_missing` · `ground_truth_empty` · `profile_body_empty` · `preamble_missing`) · codex 프롬프트가 프로필의 `ground_truth` 와 본문을 싣는 것. 이 플러그인이 물려받는 성질 하나 — 러너의 프로필 문법이 YAML 보다 좁아서(한 줄 완결 줄만) 그 밖의 프로필은 codex 축이 공시와 함께 멈춘다. 이 플러그인의 `references/docreview-profiles/generic.md` 는 무편집으로 맞는다. **호출자는 여전히 0** — `/qg critique` 전환은 후속 major PR 그대로다.
- **엔진 링크 셋이 PR 3 최종 리뷰 수정과 qg iter 3 으로 더 바뀌었다(`scripts/docreview_state.py` · `scripts/docreview_route.py` · `scripts/docreview_anchor.py`).** 얼림 검사와 permit · fix 적용 관측을 엔진이 원장의 라운드별 스냅숏으로 계산한다 — `observe-diff` · `finalize` 가 `--diff` 를 받지 않고, 관측을 건너뛴 라운드의 permit 을 뒤 라운드가 그 라운드의 스냅숏 쌍으로 따라잡으며, 필요한 스냅숏이 원장에 없으면 rc 1 `snapshot_missing` 이다(`diff_snapshots` 는 `docreview_anchor.py` 에서 `docreview_state.py` 로 옮겼다 — `diff` CLI 출력은 그대로). 따라잡는 것은 permit · fix 관측뿐이고, 건너뛴 라운드 자신의 얼림 검사(그 라운드 창의 `frozen_change`)는 따라잡지 않는다 — 이 릴리스 이전부터 같은 알려진 한계다. 비-UTF-8 critic 출력을 `prepare-recritic` 이 rc 1 `unreadable` 대신 critic 사망(rc 4 · 사유 `layer1 block undecodable`)으로 친다(설계 §9). 전문은 `plugins/spec-distill/CHANGELOG.md` `[3.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **엔진 링크 `scripts/docreview_route.py` 가 PR 3 qg iter 1 로 한 번 더 바뀌었다(`scripts/docreview_state.py` 는 주석 한 줄).** `prepare-recritic` 이 critic 출력에도 codex 와 같은 시점 판별을 한다 — 라운드 시작 표식보다 먼저(또는 같은 시각에) 쓰인 critic 파일은 critic 사망(rc 4 · 사유 `layer1 block critic_predates_round`). 전문은 `plugins/spec-distill/CHANGELOG.md` `[3.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **엔진 링크 `scripts/docreview_route.py` 가 PR 3 qg iter 2 로 한 번 더 바뀌었다.** 라운드 시작 표식이 없거나 정수가 아니면 `prepare-recritic` 이 critic 은 살린 채 「critic 시점 판별 불가 (<사유>)」를 공시한다. 전문은 `plugins/spec-distill/CHANGELOG.md` `[3.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **`scripts/adjudication.py`(심볼릭 링크)의 docstring 두 곳.** 지워진 spec-distill 테스트 `test_merge_review.py` 의 줄을 계약 근거로 대던 자리를 살아 있는 대조(`shared/tests/test_adjudication_behavior.sh` · 엔진 케이스 둘)로, 옛 카운터 이름을 쓰던 예시를 일반형으로 고쳤다. 동작 무변경.
...
- **TTL-GC 의 폴더 나이가 폴더 자신과 그 아래 모든 항목의 최신 mtime 이다 (공용 정본 `shared/gc/gc_common.py` · 사본 `scripts/gc_common.py`).**
  ...
  - **어떻게 재나.** 링크는 따라가지 않는다(`os.walk(followlinks=False)` · `os.lstat` — 링크 자신의 mtime 만 센다). 순회 중 항목이나 하위 디렉토리가 사라지면 스냅숏이 불안정하다고 본다(`SnapshotUnstable`). 그 실행은 그 폴더를 수집하지 않는다 — 걷기 도중 목록에 잡힌 tmp 가 lstat 전에 사라지는 원자적 교체를 늙은 값으로 걷지 않게. 두 번째 스캔 뒤 · rename 전의 쓰기는 여전히 못 본다(이 릴리스 이전부터의 창 — TTL 넘게 멈췄던 세션이 그 짧은 창에 재개할 때만).
...
- **`skills/quality-pipeline/SKILL.md` Review gate 스코프 도출의 awk 필드 토큰.** `awk '$1=="merge_base:"{print $2}'` 의 `$1` · `$2` 는 로드된 SKILL 본문에서 Skill 인자로 치환되는 토큰이다 — Claude Code 는 `$N` 을 0부터 센 호출 인자로 바꾸고 인자가 없는 자리는 그대로 둔다. 인자가 둘 이상인 호출이면 awk 프로그램이 인자 값으로 바뀌어 `MERGE_BASE` 가 비고, `git diff "$MERGE_BASE"..HEAD` 가 빈 범위가 되어 branch 스코프 (a) 가 조용히 0 이 될 수 있었다. 이 경로는 치환 규칙에서 도출한 것이고 이 자리의 런타임 관측은 없다 — 관측된 자리는 spec-distill `reviewing-brief` 다(`plugins/spec-distill/CHANGELOG.md` `[3.1.0]` Fixed). 위치 필드를 쓰지 않는 `sed -n 's/^merge_base: //p'` 로 바꿨고, `resolve-baseline.sh` 의 두 출력 모양(정상 · degraded)에서 옛 awk 와 같은 값을 낸다. 저장소 전체 락 `shared/tests/test_skill_body_no_positional_tokens.sh` 가 이 플러그인의 skill · command 본문도 잰다.
- **Law 2 락 `tests/test_agent_frontmatter_keys.sh` 가 frontmatter 의 파싱 가능성을 잰다 (규칙 L0, PR 3 최종 리뷰 F3).** 런타임 로더는 frontmatter 를 못 읽으면 오류 없이 파일명 이름 + 전 도구로 agent 를 싣는데(spec-distill PR 3 관측 T9 — 안 닫힌 따옴표 · U+2028 · U+2029), 락의 형태 화이트리스트는 column-0 줄 모양만 봐서 `description: "unclosed` 인 agent 를 통과시켰다 — `tools:` 가 멀쩡해 보여도 런타임에는 그 allowlist 가 없다. L0 는 모든 agent 의 frontmatter 를 PyYAML `safe_load` 해 예외 없음 · 매핑 · `tools` 키 있음을 요구하고, PyYAML 이 없으면 FAIL 이다. 한계: PyYAML 은 YAML 1.1 이라 U+2028 · U+2029 를 줄바꿈으로 읽어 파싱에 성공한다 — 그 둘은 `shared/tests/test_variant_of_contract.sh` 의 V4 가 막는다. `tests/test_agent_tools_lock_mutation.sh` 에 셀 넷(안 닫힌 큰따옴표 · 작은따옴표 · 안 닫힌 flow 값은 RED, 닫힌 같은 모양은 GREEN). 오늘 실제 agent 의 노출은 0 이다.
...
- **`agents/security-reviewer.md` 의 `## Hunt categories` 에 한 줄 — skill · command 본문의 위치 인자 치환 (PR 3 최종 리뷰 F4, Law 3).** spec-distill `reviewing-brief` 의 `rm -f "$1"` 이 로드된 본문에서 사용자 audit 경로로 치환된 결함(`plugins/spec-distill/CHANGELOG.md` `[3.1.0]` Fixed)은 세 도입 커밋과 그 태스크 리뷰들을 통과했다 — 이 리뷰어가 그 토큰을 셸 의미로만 읽었다. 불릿 하나를 **추가만** 했다(기존 줄 삭제 · 수정 0, 강화 편집). `tests/test_security_reviewer_persona.sh` 가 그 불릿의 body-unique 문구를 `## Hunt categories` 창 안에서 잰다.
