...
- **문서 리뷰 엔진 `shared/docreview/` 이 바뀌었다 — 이 플러그인은 `scripts/{docreview_state,docreview_route}.py` · `scripts/run_docreview_codex_reviewer.sh` 심볼릭 링크로 함께 배송한다(cache key).** 내용 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]` 이다. minor 인 이유 — 링크로 받는 엔진 표면이 넓어졌다: 게이트 요약의 새 필드 넷(`unverified` · `approval_label` · `round_reviewed` · `unreviewed_reason`) · 새 서브커맨드 `state-dir-for` · `finalize` 거부 시 원장 표지(`rounds[n].finalize_failed`) · 러너의 새 fail-closed 사유(`profile_parse_ambiguous` · `profile_field_missing` · `ground_truth_empty` · `profile_body_empty` · `preamble_missing`) · codex 프롬프트가 프로필의 `ground_truth` 와 본문을 싣는 것. 이 플러그인이 물려받는 성질 하나 — 러너의 프로필 문법이 YAML 보다 좁아서(한 줄 완결 줄만) 그 밖의 프로필은 codex 축이 공시와 함께 멈춘다. 이 플러그인의 `references/docreview-profiles/generic.md` 는 무편집으로 맞는다. **호출자는 여전히 0** — `/qg critique` 전환은 후속 major PR 그대로다.
- **엔진 링크 두 개가 PR 3 최종 리뷰 수정으로 더 바뀌었다(`scripts/docreview_state.py` · `scripts/docreview_route.py`).** 새 서브커맨드 `prev-snapshot`(7단계 얼림 검사의 직전 라운드 스냅숏을 원장에서 낸다) · 라운드 ≥ 2 에서 `--diff` 없이 `finalize` 하면 보고서 `advisory[]` 에 「얼림 검사 없음 — diff 미제공(라운드 n)」(공시이지 차단이 아니다) · 비-UTF-8 critic 출력을 `prepare-recritic` 이 rc 1 `unreadable` 대신 critic 사망(rc 4 · 사유 `layer1 block undecodable`)으로 친다(설계 §9). 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **엔진 링크 `scripts/docreview_route.py` 가 PR 3 qg iter 1 로 한 번 더 바뀌었다(`scripts/docreview_state.py` 는 주석 한 줄).** `prepare-recritic` 이 critic 출력에도 codex 와 같은 시점 판별을 한다 — 라운드 시작 표식보다 먼저(또는 같은 시각에) 쓰인 critic 파일은 critic 사망(rc 4 · 사유 `layer1 block critic_predates_round`). `finalize` 는 있는데 읽히지 않는 `--diff`(0바이트 · JSON 아님 · 매핑 아님)에서 죽지 않고 「얼림 검사 없음 — diff <사유>(라운드 n)」으로 공시한다(전에는 rc 1 로 죽어 라운드가 「미검증」이 됐다). 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
- **엔진 링크 `scripts/docreview_state.py` · `scripts/docreview_route.py` 가 PR 3 qg iter 2 로 한 번 더 바뀌었다.** diff 로더가 `docreview_state.load_diff` 한 자리로 옮겨 `observe-diff` 도 쓴다 — 읽히지 않는 diff(읽기 실패 포함)에서 `observe-diff` 는 rc 0 으로 permit 을 소비하지 않고 관측하지 못한 수를 원장에 세고, `finalize` 가 「permit 관측 불가 — diff <사유>(라운드 n): k건」을 공시한다(만료 · 재상승 없음). 라운드 시작 표식이 없거나 정수가 아니면 `prepare-recritic` 이 critic 은 살린 채 「critic 시점 판별 불가 (<사유>)」를 공시한다. 전문은 `plugins/spec-distill/CHANGELOG.md` `[2.1.0]`. 이 플러그인의 호출자는 여전히 0 이다.
...
- **`skills/quality-pipeline/SKILL.md` Review gate 스코프 도출의 awk 필드 토큰.** `awk '$1=="merge_base:"{print $2}'` 의 `$1` · `$2` 는 로드된 SKILL 본문에서 Skill 인자로 치환되는 토큰이다 — Claude Code 는 `$N` 을 0부터 센 호출 인자로 바꾸고 인자가 없는 자리는 그대로 둔다. 인자가 둘 이상인 호출이면 awk 프로그램이 인자 값으로 바뀌어 `MERGE_BASE` 가 비고, `git diff "$MERGE_BASE"..HEAD` 가 빈 범위가 되어 branch 스코프 (a) 가 조용히 0 이 될 수 있었다. 이 경로는 치환 규칙에서 도출한 것이고 이 자리의 런타임 관측은 없다 — 관측된 자리는 spec-distill `reviewing-brief` 다(`plugins/spec-distill/CHANGELOG.md` `[2.1.0]` Fixed). 위치 필드를 쓰지 않는 `sed -n 's/^merge_base: //p'` 로 바꿨고, `resolve-baseline.sh` 의 두 출력 모양(정상 · degraded)에서 옛 awk 와 같은 값을 낸다. 저장소 전체 락 `shared/tests/test_skill_body_no_positional_tokens.sh` 가 이 플러그인의 skill · command 본문도 잰다.
...
- **L0 의 PyYAML 은 스캔 루트 밖에서만 온다 (PR 3 qg iter 2 F-f).** 락은 스캔 루트로 cd 한 뒤 `python3 -c` 로 돌아 sys.path 첫 자리가 cwd 였다 — 스캔 루트에 심은 `yaml.py` · `yaml/` 가 PyYAML 보다 먼저 import 돼 위 완주 신호(`L0_DONE <파일 수> 0`)를 위조하고 끝낼 수 있었다. 이제 import 전에 `""` · `"."` · cwd 의 realpath 를 sys.path 에서 빼고, import 뒤 `yaml.__file__` 의 realpath 가 스캔 루트 안이면 stderr `PYYAML_SHADOWED` + rc 3 이다. `python3 -I` 는 쓰지 않는다(PYTHONPATH 까지 버려 변이 테스트의 주입 셀이 공허해진다). `tests/test_agent_tools_lock_mutation.sh` 에 셀 넷 — 스캔 루트 `yaml.py` 위조 · `yaml/__init__.py` 위조 · 스캔 루트 아래 PYTHONPATH 의 가짜 yaml 은 RED(셋 다 수정 전 락에서 GREEN), 심은 모듈이 없으면 GREEN.
