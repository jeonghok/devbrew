...
def state_file_for(session_id: str) -> Path:
    ...
    `state_path` import 를 함수 안에 둔다. 이 모듈의 `_yaml_scalar` 만 쓰는 소비자
    (`brief_review_state`)는 상태 경로를 전혀 해석하지 않는다 — 모듈 최상단에서
    import 하면 그 소비자가 같은 디렉토리의 `state_path.py` 에 import 시점으로 묶인다.
    """
