...
/interview ─→ [0] Trivia escape ─→ [1] Interview (문제공간 stage)
                                   ...
                                   [2] reviewing-brief → 문서 리뷰 엔진 (Law 2 분리 리뷰, brief 프로필, cost_class: medium)
                                       │  라운드마다 먼저: 진입 게이트 둘(check_brief.py · check_verbatim_coverage.py) → payload+audit 번들
                                       ├─ 층 1 방향성 · 층 2 충실도  doc-critic-web(DISABLE_WEB 이면 doc-critic) + codex → doc-recritic
                                       │                라운드 게이트 → 저자 수정 → 다음 라운드 (재리뷰 상한 2)
                                       └─ 냉독    brief-readback  (엔진 밖 advisory, G1–G6 gap)
                                       ▼ 산출물 셋 (엔진 게이트 결과 / 냉독 요약+gap / degrade 채널)
...
**v0.22.0**: [1] Interview 종료 driver를 고정 라운드 카운터에서 커버리지 원장(고정 floor 5 + 주제-도출 차원, status ∈ {open, in-progress, closed})으로 재구성 — 집요함·깊이·차원이 주제에 적응한다. tunneling 검출 에이전트는 `coverage-mapper`(주제-도출 차원 advisory 제안자)로 재명명·재목적화되었고, `blind-spot-prober`(적대적 premortem)가 blind-spot floor 차원 구현으로 신설되었다.
...
- **「공시와 차단은 다른 술어」 — 리뷰되지 않은 라운드 (3.1.0)** — 엔진이 critic 사망 두 번 · `finalize` 실패 · 라우팅 보고서가 없는 라운드를 결정론으로 알고 게이트 요약에 싣는다(`unverified` · `approval_label` · `round_reviewed` · `unreviewed_reason`). 「미검증」(critic 사망 · `finalize` 실패)은 승인 게이트를 그 라벨로 연다. 「미검증」이 아닌 `unrouted` 는 공시만 한다: 게이트 첫 줄이 `degrade 없음` 일 수 없고 「다음:」 줄에 꼬리가 붙는다. brief 자리는 같은 값을 Step B 로 싣는다.
...
- **P22 (Cost class)** — 모든 skill cost_class 선언 (conducting-interview: variable / framing-requests: variable / reviewing-spec: medium / reviewing-brief: medium — 3.1.0 에서 high 의 진입 지출 승인 게이트를 없앴고, 지출 상한은 엔진의 재리뷰 상한이 맡는다).
...
- **C4** coverage-mapper agent (`tools: Read, Grep, Glob, WebSearch, WebFetch` — advisory 주제-도출 차원 제안자) + **blind-spot-prober** agent (`tools: Read, Grep, Glob, WebSearch, WebFetch` — 적대적 premortem). 둘 다 dispatch 는 **자격 + 예산**이다(4.4.0): 자격 = 그 차원에 닿는 열린 결정이 아직 있는가, 예산 = `1 + 재개방`.
...
- `DEVBREW_SPEC_DISTILL_DISABLE_CODEX=1` (v0.20.0, v0.24.0 확대, 3.1.0 재배선) — codex 병렬 co-review만 skip. Claude 리뷰(탐지 · 재비판)는 정상 동작하고, codex 부재는 엔진이 `fin.json` 의 `advisory[]` 와 게이트 첫 줄로 loud 하게 공시한다. 전역 `DEVBREW_SPEC_DISTILL_DISABLE`과 독립. **적용 범위는 문서 리뷰 엔진 자리 셋 전부**: (a) design-doc 리뷰(`reviewing-spec`), (b) brief 리뷰(`reviewing-brief`), (c) seed 리뷰(`framing-requests`) — 세 자리 모두 codex 호출은 SKILL 의 리터럴 게이트 펜스(`codex-gate` 표지) 하나이고 매 라운드 4단계에서 돈다. 게이트는 **호출자 책임**이다 — `detect_codex.sh`가 이 스위치를 `codex_available: false`(`skip_reason: kill_switch`)로 옮기고 펜스가 그 값으로 러너 호출을 가르며, 러너(`run_docreview_codex_reviewer.sh`)는 이 변수를 보지 않는다. 펜스 밖에서 러너를 부르면 opt-out이 무시된 채 지출이 나간다.
...
- `DEVBREW_SPEC_DISTILL_DISABLE_WEB=1` (v0.12.0, AC21로 범위 확대, 3.1.0 재배선) — 이 kill switch 가
  **두 자리**의 웹 접근을 끈다: interview 웹 리서치(landscape · 웹 agent dispatch, v0.12.0), 그리고
  ...
  `doc-critic-web` 대신 웹 도구 없는 `doc-critic` 을 **지명**한다 — dispatch 는 지명된 블록을 따르고,
  그 선택을 강제하는 훅은 없다). 구조 게이트 `check_brief.py` 도 §4 sentinel 하나를 완화하며 자기
  advisory 로 공시한다. 어느 쪽이든 loud log(brief 자리는 degrade record 도)와 함께 생략, crash
...
- **Python 3.12+** — 이 플러그인의 훅이 요구하는 바닥입니다. 숫자는 도출된 값입니다 —
  ...
  바닥 미만이면 훅은 **막지 않고** 건너뜁니다. 다만 그 사실을 알리는 세션 시작 안내는
  `quality-gates` 의 `SessionStart` 훅 자리에서만 나갑니다 — devbrew 전체에서 그 자리는
  거기 하나뿐이라, 이 플러그인만 설치했다면 안내 없이 조용히 건너뜁니다.
...
- **codex CLI** (외부, optional) — 있으면 문서 리뷰 엔진 라운드(design doc · interview brief · interview seed)에 병렬 독립 co-reviewer로 참여(model diversity). 없거나 auth 미설정이면 Claude-only로 graceful degrade + loud advisory(crash 없음). kill switch `DEVBREW_SPEC_DISTILL_DISABLE_CODEX=1`.
