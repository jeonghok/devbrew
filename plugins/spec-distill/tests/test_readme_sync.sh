#!/usr/bin/env bash
# AC13/AC16 — README/plugin.json/CHANGELOG synced with v0.20.0 (codex parallel co-reviewer).
# plugin.json version은 0.20.x로 assert(patch digit는 의도적으로 unpin) — devbrew의
# "플러그인 건드리는 모든 PR은 patch-bump" 규칙 때문에 리터럴 0.20.0 pin은 다음 doc-only
# bump마다 stale-red. minor(0.20)는 v0.20.0 feature가 여전히 shipped임을 뜻하는 invariant라 pin 유지.
# CHANGELOG [0.20.0] 엔트리는 append-only 기록이라 리터럴 pin이 correct.
set -u -o pipefail
