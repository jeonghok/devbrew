...
# ── brief 자리 엔진 전환(v2.1.0) — 죽은 술어의 부재 + 양의 짝 ─────────────────
...
# 한계(재지 못하는 것 — 이 셋은 GREEN 으로 통과한다): (1) 은 이름을 리포 전체 파일시스템(git 인덱스 아님, 다른 플러그인 · 미추적 파일 포함)에서 풀어 다른 플러그인에만 있는 파일을 이 플러그인의 것처럼 대도 통과하고, `.py`·`.sh` 만 보므로 지워진 `.md` 를 현재형으로 대도 통과하며, (3) 은 리터럴 셋이라 옛 규칙을 다른 말로 되살린 문장(개념 별칭)을 못 잡는다.
