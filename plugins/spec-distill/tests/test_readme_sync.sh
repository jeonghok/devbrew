#!/usr/bin/env bash
# AC16 — README/plugin.json/CHANGELOG synced with v0.15.0 (approve-suppress symmetry).
set -u -o pipefail
...
grep -q '"version": "0.15.0"' "$PLUGIN_JSON" \
  && note PASS "AC16: plugin.json version 0.15.0" || note FAIL "AC16: plugin.json not 0.15.0"
grep -qE '^## \[0\.15\.0\] — 2026-[0-9]{2}-[0-9]{2}$' "$CHANGELOG" \
  && note PASS "AC16: CHANGELOG [0.15.0] entry with ISO date" || note FAIL "AC16: CHANGELOG [0.15.0] missing/!ISO"
grep -qE '^## \[0\.15\.0\].*XX' "$CHANGELOG" \
