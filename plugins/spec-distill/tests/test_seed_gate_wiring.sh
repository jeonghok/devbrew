...
# 전에는 아무것도 못 본다. 첫 줄이면 그 자리가 아예 없다. 형태만 재는 단언이 맞다:
# 오늘 동작을 안 바꾸는 이동도 그 보장을 없애므로 RED 여야 한다.
# (동작은 자유다 — 아래는 `rm -f "$CODEX_YAML"` 를 «포함하는지»만 보므로 평범한 `if`
#  로 다시 써도 통과한다.)
FIRST_EXEC="$(grep -vE '^[[:space:]]*(#|$)' "$GATE" | head -n 1)"
case "$FIRST_EXEC" in
  *'rm -f "$CODEX_YAML"'*)
    ok "0: 블록의 첫 실행 줄이 진입 클리어다 (그 앞에 분기가 설 자리가 없다)" ;;
  *)
    no "0: 블록의 첫 실행 줄이 진입 클리어가 아니다 — '$FIRST_EXEC' (클리어 앞에 자리가 생기면 그 자리의 분기는 이 실행이 만들지 않은 파일을 읽는다)" ;;
esac
