...
# 설계 당시 **다른** "zero-tool probe" 개념 때문에 `probe` 와 `백스톱` 을 **파일
# 전체에서는** 둘 다 담고 있었다(그 개념은 task-5(seam-channel-verification)가 제거해
# 지금은 그 파일의 `probe` 카운트가 0이다 — 파일-단위 co-occurrence 로 걸었다면 그
# 시점 기준 거짓 잔존이었다는 뜻으로 읽는다). 그래서 파일 전체가 아니라 **같은 줄
# 근접**으로 좁혔다 — 이 두 파일 어디에도 `probe` 와 `백스톱`이 한 줄 안에서 만나는
# 자리가 없음을 확인했다(실측, 이하 두 조건 전부 GREEN 그대로).
#
