...
grep -qE '^##.*[Dd]esign.*[Mm]ode' "$AGENT" \
  && ok "AC7: design mode section header exists" \
  || no "AC7 design mode section header missing"

# AC7b: 6 카테고리 모두 등장
for cat in "placeholder" "ambiguity" "scope.creep" "approaches.compar" "isolation" "testing"; do
  grep -qiE "$cat" "$AGENT" \
    && ok "AC7: category '$cat' present" \
    || no "AC7 category '$cat' missing"
done

# AC7c: spec mode regression — 기존 "11 필수 섹션" 표현 보존
grep -qE '11.*필수.*섹션|missing_section' "$AGENT" \
  && ok "AC7: spec mode 11-section table preserved (regression)" \
  || no "spec mode 11-section text accidentally removed"

# AC7d: Output 형식 (round N + Status + Issues + Stagnation_signal) 보존
grep -q 'Spec Review (round N)' "$AGENT" \
  && grep -q 'Stagnation_signal' "$AGENT" \
  && ok "AC7: Output 형식 (round N + Stagnation_signal) preserved" \
  || no "Output format regression"
...
grep -qE '^```spec-review-issues[ \t]*$' "$AGENT" \
  && ok "v0.20.0: sentinel fence 'spec-review-issues' documented (exact info-string)" \
  || no "v0.20.0: sentinel fence exact info-string missing"
...
[[ -n "$SENTINEL_BLOCK" ]] \
  && ok "v0.20.0: fenced spec-review-issues worked example found (non-empty body)" \
  || no "v0.20.0: fenced spec-review-issues worked example NOT FOUND (deleted?)"
...
for key in '"category"' '"target_section"' '"severity"' '"message"'; do
  printf '%s' "$SENTINEL_BLOCK" | grep -qF "$key" \
    && ok "v0.20.0: sentinel example body has key $key" \
    || no "v0.20.0: sentinel example body missing key $key"
done
...
' \
  && ok "v0.20.0: sentinel example parses as documented {issues:[...]} JSON shape" \
  || no "v0.20.0: sentinel example does NOT parse as documented JSON shape"

# (C) top-level **Status:** line still the verdict source of truth.
grep -qE '\*\*Status:\*\*' "$AGENT" \
  && ok "v0.20.0: **Status:** verdict line preserved" \
  || no "v0.20.0: **Status:** line removed"
...
grep -q 'sha256_short(category' "$AGENT" \
  && no "v0.20.0: issue_id self-report (sha256_short) still instructed" \
  || ok "v0.20.0: issue_id self-report removed"

# (E) compute_issue_id referenced as the id authority.
grep -q 'compute_issue_id' "$AGENT" \
  && ok "v0.20.0: compute_issue_id referenced" \
  || no "v0.20.0: compute_issue_id reference missing"
finish
