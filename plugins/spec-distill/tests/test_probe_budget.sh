...
python3 "$SCRIPT" check "$FX/state-probe-at-cap.md" >/dev/null 2>&1 \
  && no "AC12(a): probe_count=cap should block (check exit 1)" \
  || ok "AC12(a): probe_count=cap blocks (check exit 1)"

# within budget → exit 0
python3 "$SCRIPT" check "$FX/state-probe-within.md" >/dev/null 2>&1 \
  && ok "check within budget → exit 0" \
  || no "check within budget should pass"
...
grep -qE '^probe_cap_override: 12\b' "$tmp" \
  && ok "AC12(b): raise-cap set probe_cap_override to 12 (base_cap)" \
  || no "AC12(b): raise-cap did not persist probe_cap_override"
python3 "$SCRIPT" check "$tmp" >/dev/null 2>&1 \
  && ok "AC12(b): after raise-cap, effective_cap synthesized → check exit 0" \
  || no "AC12(b): raise-cap should lift effective_cap past probe_count"
rm -f "$tmp"
...
python3 "$SCRIPT" increment "$tmp2" >/dev/null 2>&1 \
  && ok "increment on well-formed state returns exit 0 (never cap-gates — C10)" \
  || no "increment on well-formed state should exit 0"
grep -qE '^probe_count: 1\b' "$tmp2" \
  && ok "increment advanced probe_count to 1" \
  || no "increment did not advance probe_count"
rm -f "$tmp2"
...
python3 "$SCRIPT" increment "$tmp3" >/dev/null 2>&1 \
  && ok "increment at cap still exits 0 (gating is check-only, C10)" \
  || no "increment must not gate even at cap"
rm -f "$tmp3"
...
python3 "$SCRIPT" increment "$tmp6" >/dev/null 2>&1 \
  && no "increment with absent probe_count should fail closed" \
  || ok "increment with absent probe_count fails closed (exit 1)"
rm -f "$tmp6"
...
DEVBREW_SPEC_DISTILL_PROBE_CAP=20 python3 "$SCRIPT" check "$tmp4" >/dev/null 2>&1 \
  && ok "env DEVBREW_SPEC_DISTILL_PROBE_CAP=20 → probe_count 12 within budget" \
  || no "env cap override not honored"
rm -f "$tmp4"

# malformed probe_count → fail closed (silent-0 금지).
tmp5="$(mktemp)"; printf -- '---\nprobe_count: abc\nprobe_cap_override: 0\n---\n' > "$tmp5"
python3 "$SCRIPT" check "$tmp5" >/dev/null 2>&1 \
  && no "malformed probe_count should fail closed" \
  || ok "malformed probe_count fails closed (exit 1)"
rm -f "$tmp5"

# missing state file → fail closed.
python3 "$SCRIPT" check "$FX/__no_such_state__.md" >/dev/null 2>&1 \
  && no "missing state should fail closed" \
  || ok "missing state fails closed (exit 1)"
finish
