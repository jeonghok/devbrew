...
#
# **정직한 경계 (fix round 3 — 이전 판이 여기 적은 "왜 안전한가"가 틀려서
# 문장만 고친다, 컨트롤러 지시로 코드는 안 건드린다).** sentinel 은 "codex 가
# 호출됐는가"만 잰다 — "그 호출의 **출력이 실제로 쓰였는가**"는 못 잰다.
# 실측 셋(전부 이 라운드에 재실행해 확인):
...
#      14/14 GREEN.** ②가 ①보다 싸다(호출 자체가 없다) — 그래서 "codex 호출의
#      부수효과(과금·요청 로그)가 방어선"이라는 이전 판 문장은 틀렸다. ①에서만
#      우연히 성립하고 ②에는 애초에 적용되지 않는다(②엔 부수효과 자체가 없다).
#      이 락이 실제로 못 잡는 것은 "응답을 버리는 러너"가 아니라 "sentinel
#      경로를 아는 러너" 전체다.
#   **왜 여기서 못 막는가**: `CODEX_MOCK_SENTINEL` 을 :146 에서 러너 프로세스
#   **전체**의 환경변수로 export 한다 — mock codex 자식 프로세스에만 scope
#   되지 않는다. 즉 **측정 대상인 러너 자신이 그 anchor 를 볼 수 있다.** 앵커를
#   볼 수 있는 대상은 그 앵커를 스스로 쓸 수 있다 — 이것이 이 sentinel 설계가
#   이 지점에서 못 닫는 이유이고, 더 강한 설계라면 이 가시성부터 끊어야 한다
#   (예: 러너 자신은 값을 모른 채 mock 자식에만 전달되는 채널 — fd 상속·
#   소켓 등). 이 축에서 더 쫓지 않는다(컨트롤러 지시) — 이 문단이 그 경계를
#   다음 독자가 찾을 자리에 남긴다.
