...
# ── I5(a) : 펜스 «전체 모양» — 「└ 천장」만으로는 판정 한 줄의 형식을 못 잰다 ────
# 위 has 는 '└ 천장' 한 문자열만 본다 — `<앵커>: <태그> <무엇이 과한가>. <더 단순한
# 대안>.` 템플릿 줄을 통째로 지워도 「└ 천장: <조건>」 잔해만 남으면 펜스가 비지
# 않아 위 양의 짝도, has 도 그대로 통과한다(실측 — mutation). 「판정 한 줄의 형식」
# 이라는 배너가 실제로 재는 것은 그 한 줄이 채우는 «이름 있는 슬롯 다섯 전부»여야
# 한다 — 부재 하나하나를 개별 실패로 보고한다(하나로 뭉치면 어느 슬롯이 없어졌는지
# 안 보인다).
for slot in '<앵커>' '<태그>' '<무엇이 과한가>' '<더 단순한 대안>' '<이 판정이 틀릴 조건>'; do
  has "$BFENCE" "$slot" "I5(a): brief 판정 한 줄 펜스에 슬롯 ${slot} 이 있다"
  has "$DFENCE" "$slot" "I5(a): design 판정 한 줄 펜스에 슬롯 ${slot} 이 있다"
done
...
for v in b d; do
  ...
  # 서술이 critic_dead(`docreview_route.cmd_prepare` 의 `if critic_why or not isinstance(l1, list)`
  # 갈래 — 층 1 블록 부재 → 라운드 「미검증」)를
  ...
  # I5(b) — 위 다섯 has/hasnt 는 숫자 «3」도 그 근거 문장도 안 읽는다. 「3」→「30」과
  # 근거 문장 삭제를 함께 해도(실측) 위가 전부 그대로 통과한다 — 「상한 N=3 과 그
  # 근거」 배너가 실제로 재는 것은 그 둘인데, 지금은 아무것도 안 잰다. 숫자와 근거
  # 문장을 각각 핀 — "3건" 은 "30건" 의 부분 문자열이 «아니다»(3 과 건 사이에 0 이
  # 끼어 연속이 깨진다)라 그 변이를 직접 잡고, 근거 문장은 결론 절만 따로 핀한다
  # (「게이트가 AskUserQuestion 을 4개씩」은 위쪽에서 `AskUserQuestion` 리터럴과
  # 겹치지 않는 결론부를 골랐다).
  # 맨 부분 문자열 `3건` 은 `13건` 안에도 있다 — 앞 글자가 숫자가 아님을 함께 핀한다.
  if printf '%s\n' "$CT" | grep -Eq '(^|[^0-9])3건'; then
    ok "I5(b): $N 「상한」절이 실제 상한 값 3건을 명시한다(30건·13건 등으로 바뀌면 실패)"
  else
    no "I5(b): $N 「상한」절에 앞이 숫자가 아닌 「3건」이 없다 — 상한 값이 바뀌었다"
  fi
  has   "$CT" '한 호출을 혼자 채우지 못한다' "I5(b): $N 「상한」절에 3건의 근거 문장(결론부)이 있다"
done
