...
# 아래 부재 검사(Step 2.5 가 Step 2 의 정지 문구를 재사용하지 않는가)는, 그 정지 문구가
# Step 2 블록에 **지금도 실재하는** 리터럴일 때만 의미가 있다 — 가드가 통제하지 않는
# 텍스트(Step 2 자신의 산문)에서 리터럴을 베꼈기 때문이다. Step 2 의 문구가 바뀌어 그
# 리터럴이 파일 전체에서 사라지면, 부재 검사는 무엇을 지워도 항상 통과하는 죽은 키가
# 된다. 그래서 같은 리터럴을 Step 2 블록 안에서 **양성으로도** 요구한다 — Step 2 의
# 문구가 바뀌면 이 양성 검사가 먼저 소리를 내고, 그 신호가 아래 부재 검사의 리터럴도
# 함께 갱신하라는 요구다. 이 가드가 잡는 것은 **이 한 문구**뿐이다 — 다른 표현의 차단
# 문장은 이 가드를 넘어간다(어휘를 열거해 일반화하지 않는다).
#
# 다른 절반 — 「막지 않는다」는 Step 2.5 가 조용하다는 것만으로는 성립하지 않는다. Step 3
# dispatch 줄이 **실재해야** 흐름이 실제로 이어진다(부재 검사만으로는 이 절반을 못 잡는다).
grep -qF '인터뷰를 시작하지 않습니다' <<<"$step2_flat" \
