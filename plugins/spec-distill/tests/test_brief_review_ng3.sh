...
grep -qiF "$OLD_EN" "$REVIEWER" && no "T13: spec-reviewer.md에 옛 NG3 문구 잔존" \
                                || ok "T13: spec-reviewer.md 옛 문구 부재"
grep -qF "$OLD_KO" "$REVIEWER" && no "T13: spec-reviewer.md 한국어 옛 문구 잔존" \
                               || ok "T13: spec-reviewer.md 한국어 옛 문구 부재"
grep -qF "$OLD_KO2" "$REVIEWER" && no "T13: spec-reviewer.md '분리 review 대상 아님' 잔존" \
                                || ok "T13: spec-reviewer.md 부정 서술 부재"
grep -qF 'brief-critic' "$REVIEWER" \
  && ok "T13: spec-reviewer.md가 brief 리뷰어를 가리킴" || no "T13: brief-critic 언급 부재"
# 역할 경계는 유지되어야 한다 — spec-reviewer는 여전히 design doc만 본다
grep -qE 'design doc (only|만)' "$REVIEWER" \
  && ok "T13: spec-reviewer 역할 경계 유지 (design doc only)" || no "T13: 역할 경계 서술 손실"
