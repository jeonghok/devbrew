...
audit_file: interview-brief-sentinel-in-statement.audit.md
user_sourced_items:
  - id: C1
    source: verbatim
    status: provisional
    statement: "게이트는 # confirmed 0건 — 사용자가 전부 잠정으로 판단 문자열을 값 안에서 인정하지 않는다"
    evidence: S1
  - id: D2
    source: chosen
    status: provisional
...
- 🗣 provisional **C1** — 게이트는 # confirmed 0건 — 사용자가 전부 잠정으로 판단 문자열을 값 안에서 인정하지 않는다 ⟨S1⟩
