...
[[ "$rc" == "0" ]] && ok "정상 payload → exit 0" || no "정상 payload가 exit $rc"

grep -qF '.audit.md' <<<"$BLOB" \
  && no "T24: blob에 '.audit.md' 잔존 (격리 위생 실패)" || ok "T24: blob에 '.audit.md' 부재"
grep -qE '^audit_file: <redacted>$' <<<"$BLOB" \
  && ok "T24: audit_file: <redacted>" || no "T24: audit_file redact 형태가 아님"
grep -qE '^name: <redacted>$' <<<"$BLOB" \
  && ok "T24: name: <redacted>" || no "T24: name redact 안 됨 (재구성 경로)"
grep -qE '^created_at: <redacted>$' <<<"$BLOB" \
  && ok "T24: created_at: <redacted>" || no "T24: created_at redact 안 됨"

# 본문은 온전해야 한다 — redaction이 §6 원문을 건드리면 충실도 판정 자체가 무의미해진다
grep -qF "브리프에 리뷰를 붙이고 싶다" <<<"$BLOB" \
  && ok "T24: §6 원문 보존" || no "T24: §6 원문이 손상됐다"
grep -qF "## 6. 사용자 원문" <<<"$BLOB" \
  && ok "T24: §6 헤딩 보존" || no "T24: §6 헤딩 손실"
grep -qF "Verbatim OK — Interview Brief" <<<"$BLOB" \
  && ok "T24: H1 제목 보존 (주제는 남는다 — readback 냉독 지장 없음)" || no "T24: H1 손실"

# session_id는 redact 대상이 아니다 (세 값만 — 과잉 redaction도 결함)
grep -qE '^session_id: 11111111' <<<"$BLOB" \
  && ok "T24: session_id는 그대로 (redact 대상 3값만)" || no "T24: 과잉 redaction"
...
for k in created_at session_id; do
  grep -qE "^${k}:" <<<"$BLOB3" \
    && ok "C1: 빈 redact 키 뒤의 '${k}:' 라인이 살아남는다" \
    || no "C1: '${k}:' 라인이 삭제됐다 — 빈 값 뒤 개행을 넘어 다음 줄을 먹었다"
done
# 원래 하려던 일은 그대로 해야 한다 — "아무것도 안 한다"로는 위 assert가 통과하므로
# redaction 자체가 살아 있는지 같은 입력에서 함께 확인한다.
grep -qE '^created_at: <redacted>$' <<<"$BLOB3" \
  && ok "C1: 값이 있는 created_at은 여전히 redact된다" \
  || no "C1: redaction이 죽었다 (no-op으로 위 assert를 만족시키는 퇴행)"
...
[[ "$n_lines_out" == "$n_lines_in" ]] \
  && ok "C1: frontmatter 키 ${n_lines_in}개가 전부 보존 (누락 0)" \
  || no "C1: frontmatter 키가 ${n_lines_in} → ${n_lines_out} 개로 줄었다"
rm -f "$tmp_empty"
...
rc3=$?
[[ "$rc3" == "2" ]] && ok "부재 파일 거부 (exit 2)" || no "부재 파일이 exit $rc3 (2가 아님)"
...
[[ "$rc" == "2" ]] \
  && ok "비-UTF-8 payload → exit 2 (문서화된 '디스패치 금지' 코드)" \
  || no "비-UTF-8 payload가 exit $rc — 표에 없는 코드는 빈 blob 디스패치로 샌다"
[[ -z "$out" ]] \
  && ok "비-UTF-8 payload → stdout 비어 있음 (부분 blob 아님)" \
  || no "비-UTF-8 payload가 stdout에 무언가를 냈다 — 잘린 blob이 디스패치될 수 있다"
...
grep -q 'payload' <<<"$err" \
  && ok "비-UTF-8 payload → stderr에 원인 명시 (조용한 실패 아님)" \
  || no "비-UTF-8 payload 실패가 stderr에 설명되지 않는다"
rm -f "$tmpbin"
...
if [[ ! -r "$tmpperm" ]]; then   # root로 돌리면 항상 읽히므로 그때는 건너뛴다
  ...
  [[ "$rc" == "2" ]] \
    && ok "읽기 불가 payload → exit 2 (1이 아님)" \
    || no "읽기 불가 payload가 exit $rc"
else
  ok "읽기 불가 케이스 건너뜀 (root 실행 — 권한이 적용되지 않음)"
fi
...
if [[ "$mutres" == "MUTATED" ]]; then
  ...
  [[ "$rcm" != "2" ]] \
    && ok "mutation: 읽기 가드 제거 → exit $rcm (2가 아님, 락에 이빨 있음)" \
    || no "mutation: 가드를 제거해도 exit 2 — 이 락은 다른 이유로 통과한다"
else
  no "mutation: 가드 텍스트를 못 찾아 치환이 일어나지 않았다 ($mutres) — 이 락은 vacuous하다"
fi
rm -f "$tmpmut" "$tmpbin2"
finish
