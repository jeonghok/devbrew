...
1. **기계적** — 이 계약을 채택한 **모든** skill 의 표면에 정지 어휘(턴을 끝낸다 · 그 다음
   턴에 사용자 트리거로만 진입한다)와 polite stop 금지가 실재하는지를 grep 이 잰다.
   재는 주체는 `spec-distill/tests/test_proceed_gate_adopters.sh` 이며, 채택자를
   **열거하지 않고 이 파일을 가리키는 포인터에서 도출**한다 — 세 번째 skill 이 이 계약을
   채택하면 같은 요구를 자동으로 받는다. 각 채택자는 **자기 표면**에서만 측정된다(아래).
