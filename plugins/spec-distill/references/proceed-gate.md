...
1. **기계적** — 이 계약을 채택한 **모든** skill 의 표면에 정지 어휘(턴을 끝낸다 · 그 다음
   ...
   재결정 규약(P23) 절의 이름이 실재하는지를 grep 이 잰다. 재는 주체는
...
**재결정 규약(P23) 절도 같은 도출 메커니즘이 요구하는 네 번째 항목이다** — 위 채택자 도출에
걸리는 모든 skill 은 자기 표면에 「재결정 규약」 절을 자기 어휘로 갖춰야 한다. 이 요건은
`test_proceed_gate_adopters.sh` 의 네 번째 채택자 단언(`재결정|반증` 리터럴)이 잰다. 채택자가
이 절을 갖추지 않으면, 정본만 읽고 넘어간 다음 채택자가 이 요건을 놓친 채 지나간다.
