...
- **게이트 술어 다섯** — 연결 ∀ · 연결 대상 실재 · 역참조 ∀(§3·§0 양쪽) · 이름 정확 일치
  `derived:internal_research` + `closed`(조건부 · 이름이 맞는 행 **전부**) · 확인 줄 ∀. **차단 판정에 개수가 없다** — 전부 ∀ 이고
  ...
  - 결정 연결은 조사 항목 줄 끝의 **넷** 중 하나다 — 레포 `[RC<n> → OQ<n>]` · `[RC<n> → 없음]`, 웹
    ...
    절마다 다르게 집행한다 — 레포 주장이 웹 형식 `[→ …]` 을 달면 역참조 ∀ 의 요구가 통째로 빠지기
    때문이다. **§5 는 줄 단위**다: §5 항목은 `RC<n>` 을 실어서 순회되므로 정의상 레포 주장 줄이고, 웹
    형식 연결이면 연결 없음이다(같은 id 의 버리는 `[RC3 → 없음]` 줄로 세탁되지 않는다). **§4 는 전역**
    이다: 웹 항목에 적힌 `RC<n>` 은 상호참조라, 그 id 가 어느 항목 줄의 레포 형식 연결에 한 번은 실려
    있으면 green 이다. 두 절 모두 어느 연결에도 안 실린 id 를 담은 줄(다른 id 의 연결 `[RC5 → …]` 뒤에
    숨는 모양)은 연결 없음이고, 연결 괄호는 줄마다 **정확히 하나**다 — 줄 끝 연결만 읽히므로 괄호가 둘이면
    앞 괄호의 결정이 대상 실재·역참조를 피한다(`RC3 [→ OQ1] [RC3 → 없음]`).
  - id 비교는 **토큰**이다 — 부분 문자열이면 `RC12` 가 `RC1` 역참조를 만족시켰다. id 의 끝 경계는
    `\b` 가 아니라 ASCII 낱말 문자의 부재라, 조사가 붙은 `RC3에` · `OQ1은` 도 그 id 로 읽는다. 선두 `OQ<n>` 은 강조 표기(`**OQ1**` ·
    `__OQ1__` · 백틱)를 건너뛰고 읽는다.
...
- **`contract: v2` 옵트인** — 술어 다섯이 이 frontmatter 필드가 있을 때만 발동한다. 기존 §4 보유
  ...
  `v2` 로 읽히지 않으면(중복 키 · `V2` · 모르는 값 · 빈 값 · `Contract:`·`contract :`·`"contract":` 같은 키 모양) 부재가
  아니라 red 다 — 부재로 읽으면 다섯 술어가
  조용히 꺼지고 advisory 는 「없다」고 오진한다.
- **`blind-spot-prober` 의 웹 근거도 계약 모양이다** — `hidden_assumptions[]`·`failure_modes[]` 의
  `evidence` 가 URL 문자열 목록이 아니라 계약 객체(`url`·`supports`·`claim`·`touches`·`decides`)다. 웹
  premortem 도 결정 연결을 대고, C44 면제 입력이 그 경로에서 비지 않는다(설계 §B — prober 의 출력 의무는
  coverage-mapper 와 같다).
- 락: `tests/test_research_claims_contract.sh` 여섯 축(A 산문 · B 양의 짝 · C 펜스 · X 차가운 셸
  ...
  다른 임계값(예: `>= 10`)엔 무력했던 것을 재비판이 드러내, 다섯 술어 함수 + 판정을 쥔 헬퍼 다섯
  (`_declared_decisions` · `_leading_oq` · 순회 정의 `_research_entries_split` · `research_entries` ·
  `payload_rc_ids`) + `contract: v2` 분기 전체를 대상으로 하는 임계값-무관 정적
...
- **조건부 발동이 피검자 산출물에 앵커돼 있다** — `RC<n>` 을 0건 내면 ④·⑤ 는 조건 자체가 공집합이라
  발동하지 않고(조건부 조기 반환), ①·② 는 §4 웹 항목만 순회해 레포 주장 쪽 절반이 잠든다. **③ 은 예외다** — forward
...
- **정본 밖에 계약 모양 사본이 넷 있다** — `steelman-builder` 의 인라인 사본은 락 축 D 가 필드 이름
  집합의 등호로 지키지만 **설명 산문의 갈라짐은 못 잡는다**. `coverage-mapper` 의 사본과
  `blind-spot-prober` 의 두 `evidence` 블록은 집합 락이 없다(prober 는 블록마다 `url`·`decides` 만
  잠근다). dispatch 는 매번 정본 전문을 `<claims_contract>` 로 싣으므로 사본이 갈라져도 subagent 가
  받는 계약은 바뀌지 않는다.
...
- **§H 표기층은 리뷰로 검증되지 않았다** — 설계 라운드 3 을 돌지 않았고, 이 릴리스의 red/green 짝과
  변이가 그 절의 유일한 검증이다.
- **웹 출처의 `RC<n>` 모양 문자열도 레포 id 로 읽힌다** — `PostgreSQL 17 RC1` 같은 릴리스 후보 표기는
  연결 ∀ 와 확인 줄 ∀ 에서 red 다(fail-closed). 연결 ∀ 메시지는 「release candidate 1」 처럼 풀어 쓰라고
  안내하고, 탐지기가 읽지 않는 표기는 알리지 않는다 — 차단 메시지는 막힌 모델이 읽는 채널이다. id 판독을
  경로 모양이나 연결 안으로 좁히면 레포 주장 우회가 다시 열려서 그러지 않았다. 반대로 `RC 3` · 소문자 ·
  전각 같은 근사 표기는 id 가 아니라 어느 술어에도 보이지 않는다.
- **연결 ∀ 는 형태 검사다** — 줄 끝 연결의 모양과 id 의 출현만 보고, 연결이 참인지는 V1(내용 확인)과
  확인 줄 ∀ 가 진다. 그래서 가르지 못하는 모양이 둘 남는다: 같은 id 의 버리는 레포 연결을 둔 채 다른 id
  의 연결로 실제 결정을 다는 §5 줄, 그리고 웹 항목으로 위장한 레포 주장 — 이미 선언된 «출처키» 를 다시
  쓰면 audit §7 을 고치지 않고도 통과한다. 위장한 줄이 `RC<n>` 을 실으면 확인 줄 ∀ 가 그 id 에 걸리지만,
  `RC<n>` 까지 빼면 어느 술어도 보지 못한다(위 조건부 발동 항목과 같은 뿌리). 「연결 괄호는 줄마다
  하나」도 **정확한 연결 문법에 한해**서다 — `->` · 쉼표 구분 · 전각 괄호 · 불릿 아닌 이어지는 줄에 적은
  결정은 산문으로 읽혀 어느 술어에도 보이지 않는다. 그 힘은 거짓 `[RC<n> → 없음]` 과 같아서 괄호 문법을
  넓히지 않았다(넓히면 정직한 산문 `(→ OQ1 참조)` 가 red 가 된다).
- **`contract` 키 줄 검색은 frontmatter 전체를 본다** — 다른 키 아래 들여쓴 `contract:` 도 존재로 읽혀
  옵트인 없는 brief 가 「판독 불가」 red 가 된다. fail-closed 방향이고 현재 코퍼스에 그런 키는 없다.
