...
- **엔진 서브커맨드 `docreview_state.py prev-snapshot --state-dir`.** 원장에 저장된 직전 라운드 스냅숏(`begin-round` 가 저장한 `anchor` · `title` · `level` · `hash` · `parents`)을 `docreview_anchor.py diff` 가 읽는 모양으로 낸다. 라운드 1 이면 `no_prev_snapshot`, 원장에 직전 스냅숏이 없으면 `prev_snapshot_missing` rc 1. 쓰임은 아래 Fixed(PR 3 최종 리뷰 F2).
...
- **7단계 얼림 검사의 입력(`prev.json`)을 만드는 자리가 없던 결함 (PR 3 최종 리뷰 F2).** 절차서 7단계가 `diff prev.json snap.json` 으로 직전 라운드 스냅숏을 읽는데 그것을 쓰는 줄이 절차서 · 두 skill · 엔진 어디에도 없었고, `finalize` 는 라운드 ≥ 2 에서 `--diff` 가 없으면 얼림 검사(`frozen_change` 자동 `decide`)를 공시 없이 건너뛰었다 — 그 라운드가 「degrade 없음」으로 렌더됐다. 이제 7단계가 `docreview_state.py prev-snapshot --state-dir D > prev.json` 로 원장의 직전 스냅숏을 먼저 쓰고, 라운드 ≥ 2 에서 `--diff` 가 없으면 `finalize` 가 `advisory[]` 에 「얼림 검사 없음 — diff 미제공(라운드 n)」을 싣는다(차단은 아니다 — 렌더 첫 줄에는 codex 가 있는 라운드에서만 오고, codex 부재 라운드의 첫 줄은 codex 공시다). 있는데 읽히지 않는 `--diff` 는 아래 qg iter 1 줄이다. 락: 엔진 케이스 둘(`case_F2_prev_snapshot_from_ledger` — prev-snapshot 으로 지은 diff 가 라운드 1 스냅숏 파일로 지은 diff 와 바이트로 같다 · `case_F2_finalize_round2_without_diff_discloses`)과 `shared/tests/test_docreview_procedure_paths.sh` 의 도출 ∀ — 절차서가 대는 `*.json` 마다 그것을 쓰는 `> <이름>` 이 같은 문서에 있다.
...
- **`tests/test_arm_ledger_timing.sh` 의 T12f 가 발동 조건과 극성을 잰다 (PR 3 최종 리뷰 F8).** 찾던 리터럴이 부분 문자열(`` `mark-reviewed` 대신 `clear-inflight` 를 부른다 ``)뿐이라, `reviewing-spec` 의 그 문장에서 조건 `(①/②)` 를 `(④)` 로 바꿔도 통과했다(변이로 확인). 이제 조건까지 붙인 한 줄(「이 예외가 걸린 라운드에서 사용자가 진행(①/②)을 고르면 …」)을 찾는다 — 조건 변이 · 극성 뒤집기 둘 다 RED.
...
- **읽히지 않는 `--diff` 가 `finalize` 를 죽여 라운드를 「미검증」으로 닫던 결함 (PR 3 qg iter 1).** 얼림 검사 부재 공시는 `--diff` 파일의 존재만 봤다 — 있는데 0바이트 · JSON 아님 · 매핑 아님이면 `finalize` 가 rc 1 `unreadable` 로 죽어 라운드가 「미검증」(`finalize_incomplete`)이 됐다. 공시여야 할 것이 차단이 되고 사유도 틀렸다. 이제 `finalize` 가 `--diff` 를 한 번 읽고, 읽히지 않으면 그 모양을 사유로 「얼림 검사 없음 — diff <사유>(라운드 n)」을 `advisory[]` 에 싣는다(파일 부재는 전과 같은 「diff 미제공」). 락: `case_F2_finalize_round2_unreadable_diff_discloses`(세 모양 각각 rc 0 · 그 이름의 공시 · `round_reviewed` 참). 파손 diff 로 `finalize` 를 죽이던 `case_T46_finalize_failed_unverified` 는 탈것을 비-UTF-8 재비판 출력으로 옮겼다(단언 무변경).
...
- **공시 채널 서술 두 자리를 사실대로 (PR 3 qg iter 1 · 재리뷰 NB-3).** 절차서 7단계와 위 F2 줄이 얼림 검사 부재를 「렌더 첫 줄이 공시한다」고 적었다 — codex 부재 라운드에서는 거짓이다. 공시는 `advisory[]` 로 간다. 렌더 첫 줄에는 codex 가 있는 라운드에서만 오고, codex 부재 라운드의 첫 줄은 codex 공시다.
- **읽히지 않는 diff 에서 7단계 `observe-diff` 가 원장 탓으로 죽거나 permit 을 공시 없이 잃던 결함 (PR 3 qg iter 2 F-a · F-d).** 위 `--diff` 줄은 `finalize` 만 고쳤다. 7단계는 같은 `diff.json` 으로 `observe-diff` 를 먼저 도는데, 그것은 0바이트 · `{` 에서 rc 1 `state_unreadable`(원장 탓)로, `[]` 에서 잡히지 않은 예외로 죽었다. 멈추면 라운드가 막혔고, 넘어가면 이번 라운드 permit 이 영영 소비되지 않아 채택된 결정이 적용도 만료도 재상승도 되지 않은 채 아무도 세지 않았다. 이제 diff 로더는 엔진의 한 자리(`docreview_state.load_diff`)이고 `observe-diff` · `finalize` 가 같은 판정을 쓴다 — 사유는 `미제공` · `읽기 실패:<사유>` · `0바이트` · `JSON 아님` · `매핑 아님`. 읽히지 않으면 `observe-diff` 는 rc 0 으로 permit 을 소비하지 않고 이번 라운드의 관측하지 못한 permit 수를 원장(`rounds[n].permit_unobserved`)에 세고, `finalize` 가 「permit 관측 불가 — diff <사유>(라운드 n): k건」을 `advisory[]` 에 싣는다. 공시만 한다 — 만료 · 재상승은 하지 않는다(사용자 결정). 읽기 권한이 없는 `--diff` 도 이제 traceback 이 아니라 「읽기 실패」 공시다. 절차서 7단계에 `observe-diff` 의 rc 규칙(읽히지 않는 diff 는 rc 0 · 공시, rc ≠ 0 은 원장 문제 → 「미검증」)이 섰다. 락: 엔진 케이스 `case_F2_observe_diff_unreadable_permit_disclosed`(실제 7단계 순서 × 세 모양, 양의 짝: 같은 라운드에서 읽히는 diff 로 재관측하면 공시 없음) · `case_F2_diff_read_failure_discloses`(mode 000 — root 로 돌면 건너뛰고 그 사실을 출력한다).
