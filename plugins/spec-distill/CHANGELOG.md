...
- **게이트 술어 다섯** — 연결 ∀ · 연결 대상 실재 · 역참조 ∀(§3·§0 양쪽) · 이름 정확 일치
  ...
  - 결정 연결은 조사 항목 줄 끝의 **넷** 중 하나다 — 레포 `[RC<n> → OQ<n>]` · `[RC<n> → 없음]`, 웹
    ...
    **전역으로** 집행한다: 모든 `RC<n>` 은 어느 항목 줄의 레포 형식 연결에 한 번은 실려야 하고, 어느
    연결에도 안 실린 id 를 담은 줄은 연결 없음이다 — 웹 형식 `[→ …]` 을 달든 다른 id 의 연결
    `[RC5 → …]` 을 달든 역참조 ∀ 의 요구가 통째로 빠지기 때문이다. 줄 단위가 아니라서, 이미 자기 연결을
    가진 id 를 웹 항목이 상호참조로 언급하는 것은 green 이다.
...
- **웹 출처의 `RC<n>` 모양 문자열도 레포 id 로 읽힌다** — `PostgreSQL 17 RC1` 같은 릴리스 후보 표기는
  연결 ∀ 와 확인 줄 ∀ 에서 red 다(fail-closed). 연결 ∀ 메시지가 `rc1` 로 쓰라고 안내한다. id 판독을
  경로 모양이나 연결 안으로 좁히면 레포 주장 우회가 다시 열려서 그러지 않았다. 반대로 `RC 3` · `rc3` ·
  전각 `ＲＣ3` 같은 근사 표기는 id 가 아니라 어느 술어에도 보이지 않는다.
