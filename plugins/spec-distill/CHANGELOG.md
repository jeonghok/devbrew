...
- **락 수치 셋이 아래 [3.0.0] 의 값에서 한 번 더 내려갔다.** `shared/tests/test_adjudication_wiring.sh` 의 `COMP_BASELINE` 48 → 40 — 이 릴리스가 지운 병합 스크립트 둘(`merge_brief_review.py` 2 · `merge_review.py` 6)의 컴프리헨션이 파일과 함께 빠졌다. `tools/adjudication/check_wiring.py` 의 `EXEMPT_BASELINE` 17 → 11 — 같은 두 파일의 면제 여섯(`merge_brief_review.py` 1 · `merge_review.py` 5)이 키째 사라졌다. 둘 다 병합 트리 스캔(`run_wiring_scan.py`) 실측이고, [3.0.0] 이 뺀 리뷰 훅의 몫(컴프리헨션 4 · 면제 10)과 파일이 달라 겹치지 않는다. `tests/test_brief_agents.sh` 의 격리 목록 4 → 3 — 옛 `brief-critic` 삭제. 세 락의 비교 방향(이하 · 등식)은 그대로다.
- **공유 절차서 8단계의 「리뷰 완료 기록은 `round_reviewed` 가 참일 때만 남긴다」 문장을 걷었다.** 그 기록자(설계문서 자리의 arm 원장)를 [3.0.0] 이 지웠고 완료를 기록하는 호스트가 남지 않았다. 남은 것은 정의 — `round_reviewed` 는 이번 라운드가 리뷰 완료인가다 — 이고, `shared/tests/test_docreview_procedure_paths.sh` 의 단언을 그 정의로 옮겼다. `reviewing-spec` 의 「미검증」 라운드 단락도 원장 문장만 뺀 채 남았다 — 라벨 · finalize rc 규칙 · `unrouted` 공시는 그대로다.
...
- **`reviewing-spec` 후보 펜스의 awk 가 `$0` 을 썼다([3.0.0]).** 로드된 skill 본문의 `$0` 은 첫 Skill 인자로 치환된다 — 인자 없이 불린 호출에서만 도는 펜스라 치환이 실제로 일어나지는 않지만, 저장소 전체 락 `shared/tests/test_skill_body_no_positional_tokens.sh` 가 그 토큰을 금지한다. 필드 구분자를 git 이 인용 없이 내지 않는 제어 문자 `\037` 로 두고 `$NF`(= 줄 전체)를 키로 쓴다 — 순서 보존 · 중복 제거 · 빈 줄 제거는 그대로다(`tests/test_reviewing_spec_entry_fence.sh` 의 후보 셀).
