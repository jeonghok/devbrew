...
- **`contract: v2` 옵트인** — 술어 다섯이 이 frontmatter 필드가 있을 때만 발동한다. 기존 §4 보유
  ...
  `v2` 로 읽히지 않으면(중복 키 · `V2` · 모르는 값 · 빈 값 · `Contract:`·`contract :` 같은 키 모양) 부재가
...
- **연결 ∀ 가 가르지 못하는 모양 둘** — 같은 id 의 버리는 레포 연결을 둔 채 다른 id 의 연결로 실제 결정을
  다는 §5 줄, 그리고 웹 항목으로 위장한 레포 주장(«출처키» 와 audit §7 선언이 필요해 비싸고 보인다).
  확인 줄 ∀ 는 두 경우 모두 그 id 에 걸린다.
