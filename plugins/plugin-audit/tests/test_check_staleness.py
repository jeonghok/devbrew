...
SCRIPTS_DIR = Path(__file__).resolve().parents[1] / "scripts"   # plugins/plugin-audit/scripts
...
def run_sweep(plugin_dir, repo_root=None, env=None):
    ...
    r = subprocess.run(cmd, capture_output=True, text=True, cwd=str(SCRIPTS_DIR), env=env)
