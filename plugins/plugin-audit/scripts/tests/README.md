...
## 테스트 실행
- Python: `python3 -m unittest discover -s plugins/plugin-audit/scripts/tests -t . -v` (리포 root에서)
- Node:   `node --test plugins/plugin-audit/scripts/tests/*.test.mjs` (bare 디렉토리 형태
  `node --test plugins/plugin-audit/scripts/tests/`는 이 harness가 검증한 Node 버전에서
