...
const CONTRACT = [
  ...
  '5. **D1–D5는 후보 단서다 — 사실이 아니다.** 각 전제를 직접 검증하고 confirmed/withdrawn/',
  '   reclassified/unverified로 판정하라. **이미 4건 중 3건의 전제가 틀렸다.**',
  ...
  '- **D5** AGENTS.md-canonical 설계가 2026 기준 정답인가? 근거는 블로그 1편 + gist 1개뿐이고 같은',
  '  등급의 주장이 이미 세 번 틀렸다. **`withdrawn`이면 갭을 올려라** — 가장 값진 발견일 수 있다.',
  '',
  '## Evidence Pack (사실만)',
  ...
  '`claude-code-setup`(**project-init과 정면으로 겹친다**), `skill-creator`, `hookify`, `commit-commands`.',
...
const AXES = [
  ...
  },
  {
    ...
    question: [
      ...
      '  읽는가?** 공식 문서로 직접 확인하라 (WebSearch/WebFetch 가용). 근거는 블로그 1편 + gist 1개뿐이고',
      '  같은 등급의 주장이 이미 세 번 틀렸다. **`withdrawn`이면 갭을 올려라.**',
      '  web 접근 불가면 `unverified` + **왜 불가였는지** (정직한 답이며 실패가 아니다).',
      '- **레퍼런스 코퍼스를 직접 읽어라.** `claude-code-setup`은 *"코드베이스를 분석해 hook·skill·MCP·',
      '  subagent 자동화를 추천"*한다 — **project-init과 정면으로 겹친다.** 무엇이 다른가?',
